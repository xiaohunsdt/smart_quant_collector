...
static std::vector<std::string> type = {"BOOL",      "CHAR",   "SHORT",    "STRING",   "LONG",   "NANOTIME", "NANOTIMESTAMP",
                                        "TIMESTAMP", "FLOAT",  "DOUBLE",   "INT",      "DATE",   "MONTH",    "TIME",
                                        "SECOND",    "MINUTE", "DATETIME", "DATEHOUR", "IPADDR", "INT128",   "SYMBOL"};
...
TEST_F(BatchTableWriterTest, test_batchTableWriter_insert) {
  ...
  script +=
      "t=table(100:0,`dbbool `dbchar `dbshort `dbstring_char `dbstring `dblong `dbnanotime `dbnanotimestamp "
      "`dbtimestamp `dbfloat `dbdouble `dbint `dbdate `dbmonth `dbtime `dbsecond `dbminute `dbdatetime `dbdatehour , "
      "[BOOL, CHAR, SHORT, STRING, STRING, LONG, NANOTIME, NANOTIMESTAMP, TIMESTAMP, FLOAT, DOUBLE, INT, DATE, MONTH, "
      "TIME, SECOND, MINUTE, DATETIME, DATEHOUR]);";
  script +=
      "share table(100:0,`dbbool `dbchar `dbshort `dbstring_char `dbstring `dblong `dbnanotime `dbnanotimestamp "
      "`dbtimestamp `dbfloat `dbdouble `dbint `dbdate `dbmonth `dbtime `dbsecond `dbminute `dbdatetime `dbdatehour , "
      "[BOOL, CHAR, SHORT, STRING, STRING, LONG, NANOTIME, NANOTIMESTAMP, TIMESTAMP, FLOAT, DOUBLE, INT, DATE, MONTH, "
      "TIME, SECOND, MINUTE, DATETIME, DATEHOUR]) as " +
      case_ + ";";
  ...
  std::string expected =
      "expected = table( bool(1) as dbbool, char('t') as dbchar, short(100) as dbshort, string('test') as "
      "dbstring_char, string('test') as dbstring, long(100) as dblong, nanotime(100) as dbnanotime, nanotimestamp(100) "
      "as dbnanotimestamp, timestamp(100) as dbtimestamp,float(100) as dbfloat, double(100) as dbdouble, int(100) as "
      "dbint, date(100) as dbdate, month(100) as dbmonth, time(100) as dbtime, second(100) as dbsecond, minute(100) as "
      "dbminute, datetime(100) as dbdatetime, datehour(100) as dbdatehour);";
  conn.run(expected);
  std::string expected1 =
      "expected1 = table( take(bool(1),3003000) as dbbool, take(char('t'),3003000) as dbchar, take(short(100),3003000) "
      "as dbshort, take(string('test'),3003000) as dbstring_char, take(string('test'),3003000) as dbstring, "
      "take(long(100),3003000) as dblong, take(nanotime(100),3003000) as dbnanotime, take(nanotimestamp(100),3003000) "
      "as dbnanotimestamp, take(timestamp(100),3003000) as dbtimestamp, take(float(100),3003000) as dbfloat, "
      "take(double(100),3003000) as dbdouble, take(int(100),3003000) as dbint, take(date(100),3003000) as dbdate, "
      "take(month(100),3003000) as dbmonth, take(time(100),3003000) as dbtime, take(second(100),3003000) as dbsecond, "
      "take(minute(100),3003000) as dbminute, take(datetime(100),3003000) as dbdatetime, take(datehour(100),3003000) "
      "as dbdatehour);";
  conn.run(expected1);

  ASSERT_ANY_THROW(btw.insert(case_, "", int(0), int(0), short(100), strtest, std::string("test"), testlong, testlong, testlong, testlong, float(100),
                              double(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100)));
  ASSERT_ANY_THROW(btw.insert(case_, "", char('0'), char('t'), short(100), strtest, std::string("test"), testlong, testlong, testlong, testlong,
                              float(100), double(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100)));

  // size1000
  for(int i = 0; i < 1000; i++) {
    btw.insert(case_, "", char('0'), char('t'), short(100), strtest, std::string("test"), testlong, testlong, testlong, testlong, float(100),
               double(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100));
  }
  {
  ...
  for(int i = 0; i < 1000; i++) {
    btw.insert(dbName, "dtable", char('0'), char('t'), short(100), strtest, std::string("test"), testlong, testlong, testlong, testlong, float(100),
               double(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100));
  }
  {
    while(btw.getAllStatus()->getColumn(3)->getString(0).c_str() != std::to_string(1000)) {
  ...
  for(int i = 0; i < 1000; i++) {
    btw.insert(dbName, "ptable", char('0'), char('t'), short(100), strtest, std::string("test"), testlong, testlong, testlong, testlong, float(100),
               double(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100));
  }
  {
    while(btw.getAllStatus()->getColumn(3)->getString(0).c_str() != std::to_string(1000)) {
...
TEST_F(BatchTableWriterTest, test_batchTableWriter_insert_symbol_in_memory) {
  ...
  script2 +=
      "expected=table(symbol(loop(take{, 300000}, c1v).flatten()) as c1, symbol(loop(take{, 300000}, c2v).flatten()) "
      "as c2, symbol(loop(take{, 300000}, c3v).flatten()) as c3);";
...
TEST_F(BatchTableWriterTest, test_batchTableWriter_insert_symbol_dfs) {
  ...
  script2 +=
      ...
      "as c2, symbol(loop(take{, 300000}, c3v).flatten()) as c3);";
  script2 +=
      "each(eqObj, (select * from expected order by c1, c2, c3).values(), (select * from pt order by c1, c2, "
      "c3).values()).all()";
  int result1;
  result1 = conn.run(script2)->getInt();
  ASSERT_EQ(result1, 1);
...
static void test_batchTableWriter_insert_thread_fuction(int id, dolphindb::BatchTableWriter &btw, std::string dbName, std::string tableName,
  ...
  for(int i = 0; i < dataRow; ++i) {
    ...
    btw.insert(dbName, tableName, dolphindb::Util::createInt(id), dolphindb::Util::createBool(boolPtr[i % maxIndex]),
               dolphindb::Util::createChar(charPtr[i % maxIndex]), dolphindb::Util::createShort(shortPtr[i % maxIndex]),
               dolphindb::Util::createString(std::string((char *)(stringPtr[i % maxIndex]))), dolphindb::Util::createLong(longPtr[i % maxIndex]),
               dolphindb::Util::createNanoTime(nanotimePtr[i % maxIndex]), dolphindb::Util::createNanoTimestamp(nanotimestampPtr[i % maxIndex]),
               dolphindb::Util::createTimestamp(timestampPtr[i % maxIndex]), dolphindb::Util::createFloat(floatPtr[i % maxIndex]),
               dolphindb::Util::createDouble(doublePtr[i % maxIndex]), dolphindb::Util::createInt(intPtr[i % maxIndex]),
               dolphindb::Util::createDate(datePtr[i % maxIndex]), dolphindb::Util::createMonth(monthPtr[i % maxIndex]),
               dolphindb::Util::createTime(timePtr[i % maxIndex]), dolphindb::Util::createSecond(secondPtr[i % maxIndex]),
               dolphindb::Util::createMinute(minutePtr[i % maxIndex]), dolphindb::Util::createDateTime(dateTimePtr[i % maxIndex]),
               dolphindb::Util::createDateHour(datehourPtr[i % maxIndex]), dolphindb::Util::createInt(iidPtr[i % maxIndex]));
  }
  connNew.close();
}

static void test_batchTableWriter_insert_thread_fuction_using_cpp_type(int id, dolphindb::BatchTableWriter &btw, std::string dbName,
                                                                       std::string tableName, dolphindb::TableSP data) {
  dolphindb::DBConnection connNew;
  connNew.connect(HOST, PORT, USER, PASSWD);
  size_t dataRow = data->rows();
  size_t dataColumn = data->columns();
  ...
  for(int i = 0; i < dataRow; ++i) {
    ...
    btw.insert(dbName, tableName, id, boolPtr[i % maxIndex], charPtr[i % maxIndex], shortPtr[i % maxIndex], (const char *)(stringPtr[i % maxIndex]),
               longPtr[i % maxIndex], nanotimePtr[i % maxIndex], nanotimestampPtr[i % maxIndex], timestampPtr[i % maxIndex], floatPtr[i % maxIndex],
               doublePtr[i % maxIndex], intPtr[i % maxIndex], datePtr[i % maxIndex], monthPtr[i % maxIndex], timePtr[i % maxIndex],
               secondPtr[i % maxIndex], minutePtr[i % maxIndex], dateTimePtr[i % maxIndex], datehourPtr[i % maxIndex], iidPtr[i % maxIndex],
...
static void test_fuc_getAllStatus(dolphindb::BatchTableWriter &btw, int cycles) {
  try {
    for(int i = 0; i < cycles; ++i) {
      ...
      for(int j = 0; j < allStatus->rows(); j++) ASSERT_EQ(allStatus->getColumn(2)->get(j)->getInt(), 0);
    }
  } catch(std::exception &e) {
    std::cout << e.what() << std::endl;
...
TEST_F(BatchTableWriterTest, test_multithread) {
  ...
  script +=
      "t=table(100:0,`id`dbbool `dbchar `dbshort  `dbstring `dblong `dbnanotime `dbnanotimestamp `dbtimestamp `dbfloat "
      "`dbdouble `dbint `dbdate `dbmonth `dbtime `dbsecond `dbminute `dbdatetime `dbdatehour `iid, [INT, BOOL, CHAR, "
      "SHORT, STRING, LONG, NANOTIME, NANOTIMESTAMP, TIMESTAMP, FLOAT, DOUBLE, INT, DATE, MONTH, TIME, SECOND, MINUTE, "
