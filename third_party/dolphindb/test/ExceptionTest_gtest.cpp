...
TEST_F(ExceptionTest, testIncompatibleTypeException) {
  ...
  try {
    ...
    std::cerr << e.what() << '\n';
    std::string ex_str =
        "Incompatible type. Expected: " + dolphindb::Util::getDataTypeString(dt1) + ", Actual: " + dolphindb::Util::getDataTypeString(dt2);
...
TEST_F(ExceptionTest, testIOException) {
  std::vector<dolphindb::IO_ERR> ioErrVec = {
      dolphindb::OK,         dolphindb::DISCONNECTED, dolphindb::NODATA,        dolphindb::NOSPACE,  dolphindb::TOO_LARGE_DATA,
      dolphindb::INPROGRESS, dolphindb::INVALIDDATA,  dolphindb::END_OF_STREAM, dolphindb::READONLY, dolphindb::WRITEONLY,
      dolphindb::NOTEXIST,   dolphindb::CORRUPT,      dolphindb::NOT_LEADER,    dolphindb::OTHERERR};
  std::string getMsg;
