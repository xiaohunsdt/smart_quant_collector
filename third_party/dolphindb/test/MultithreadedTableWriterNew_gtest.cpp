...
class MultithreadedTableWriterNewTest : public testing::Test {
 public:
  ...
  static void TearDownTestSuite() { pConn->close(); }
...
TEST_F(MultithreadedTableWriterNewTest, tableNameNull) { ASSERT_ANY_THROW(dolphindb::MTWConfig config(pConn, "")); }
...
TEST_F(MultithreadedTableWriterNewTest, userHasNotaccess) {
  ...
  conn_ctl->run("userName='" + userName +
                "';"
...
TEST_F(MultithreadedTableWriterNewTest, insertValuesMemoryTableDifferentTypeData) {
  ...
  bool flag = mtwsp->insert(pErrorInfo, false, char(12), (short)12, (long)12, dolphindb::Util::createDate(2012, 11, 12),
                            ...
                            dolphindb::Util::createNanoTime(532432), dolphindb::Util::createNanoTimestamp(85932494), 12.4f, 3412.3, "AAPL", "A", data,
                            data, data, 12, "0f0e0d0c0b0a09080706050403020100");
  mtwsp->waitForThreadCompletion();
  dolphindb::ConstantSP t1 = pConn->run(case_);
...
TEST_F(MultithreadedTableWriterNewTest, insertValuesTSDBPartitionTableDifferentTypeData) {
  ...
  bool flag = mtwsp->insert(pErrorInfo, false, char(12), (short)12, (long)12, dolphindb::Util::createDate(2012, 11, 12),
                            dolphindb::Util::createMonth(2012, 11), dolphindb::Util::createSecond(134),
                            dolphindb::Util::createDateTime(2012, 11, 13, 6, 12, 12), dolphindb::Util::createTimestamp(43241),
                            dolphindb::Util::createNanoTime(532432), dolphindb::Util::createNanoTimestamp(85932494), 12.4f, 3412.3, "A", "AAPL", data,
                            data, data, 12, "0f0e0d0c0b0a0908070605040302010");
  mtwsp->waitForThreadCompletion();
  dolphindb::ConstantSP t1 = pConn->run("select * from loadTable(dbName,`pt);");
  ASSERT_EQ(t1->getColumn(0)->getRow(0)->getString(), "0");
  ASSERT_EQ(t1->getColumn(1)->getRow(0)->getString(), "12");
...
TEST_F(MultithreadedTableWriterNewTest, insertValuesOlapDimensionTableDifferentType) {
  ...
  script +=
      ...
      std::to_string(scale64) + ")]);pt = db.createTable(t,`pt);";
  pConn->run(script);
  ...
  bool flag = mtwsp->insert(pErrorInfo, false, char(12), (short)12, (long)12, dolphindb::Util::createDate(2012, 11, 12),
                            ...
                            dolphindb::Util::createDateTime(2012, 11, 13, 6, 12, 12), dolphindb::Util::createTimestamp(43241),
                            dolphindb::Util::createNanoTime(532432), dolphindb::Util::createNanoTimestamp(85932494), 12.4f, 3412.3, "A", "AAPL", data,
                            data, data, 12, decimal32val, decimal64val, 2.36735, 4);
  mtwsp->waitForThreadCompletion();
...
TEST_F(MultithreadedTableWriterNewTest, TimeTypedata) {
  ...
  std::string script =
      ...
      "`cint`csym`cdate`cdatetime`cdatehour`cmonth`ctime`ctimestamp`cminute`second`nanotime`nanotimestamp,"
      "[INT,SYMBOL,DATE,DATETIME,DATEHOUR,MONTH,TIME,TIMESTAMP,MINUTE,SECOND,NANOTIME,NANOTIMESTAMP]) as " +
      case_;
  pConn->run(script);
  dolphindb::MTWConfig config(pConn, case_);
  dolphindb::SmartPointer<dolphindb::MultithreadedTableWriter> mtwsp = new dolphindb::MultithreadedTableWriter(config);
...
TEST_F(MultithreadedTableWriterNewTest, insertTodfsTablewithAlldataTypes) {
  ...
  for(auto i = 0; i < rowNum; i++) {
    if(!mtwsp->insert(pErrorInfo, datas[i * 25 + 0], datas[i * 25 + 1], datas[i * 25 + 2], datas[i * 25 + 3], datas[i * 25 + 4], datas[i * 25 + 5],
  ...
  dolphindb::VectorSP res = pConn->run("ex = exec * from loadTable(\"" + dbName +
                                       "\", `pt) order by col3;"
...
TEST_F(MultithreadedTableWriterNewTest, insertTo_inMemorytable_with_onDataWrite_success) {
  ...
  mtwsp->waitForThreadCompletion();
  dolphindb::VectorSP res = pConn->run("ex = exec * from " + case_ +
                                       " order by col0;"
                                       "res = exec * from tab1 order by col0;"
                                       "each(eqObj,res.values(), ex.values())");
...
TEST_F(MultithreadedTableWriterNewTest, test_insert_dfs_column_info_in_errMsg) {
  ...
  writer->waitForThreadCompletion();
  ASSERT_TRUE(pConn
                  ->run("eqObj((select * from loadTable('" + dbName +
                        "',`pt)).values(), [[false], ['1'], [short(1)], [int(1)], [long(1)], [date(1)], [month(1)], [time(1)], "
                        "[minute(1)], [second(1)], [datetime(1)], [timestamp(1)], [nanotime(1)], [nanotimestamp(1)], "
                        "[datehour(1)], [float(10.0)], [double(-0.123)], symbol([`123456msg]), [`123456msg], "
                        "[blob(`123456msg)], [ipaddr('192.168.2.1')], [uuid('0f0e0d0c-0b0a-0908-0706-050403020100')], "
                        "[int128('0f0e0d0c0b0a09080706050403020100')], decimal32([0.2313], 2), decimal64([-1], 11)])")
...
TEST_F(MultithreadedTableWriterNewTest, test_timeCol_position) {
  std::string case_ = getCaseName();
  pConn->run("try{dropStreamTable(`" + case_ +
             ")}catch(ex){};go;"
             "share streamTable(1:0, `sym`ind`val`ts, [SYMBOL, INT, DOUBLE, TIMESTAMP]) as " +
             case_ +
             ";"
  ...
  config.setStreamTableTimestamp()
      .setWriteMode(dolphindb::WriteMode::Append, {})
      .setBatching(1, std::chrono::milliseconds(10))
      .setWriteMode(dolphindb::WriteMode::Append, {});
  dolphindb::SmartPointer<dolphindb::MultithreadedTableWriter> writer = new dolphindb::MultithreadedTableWriter(config);
  const int rows = 1000;
