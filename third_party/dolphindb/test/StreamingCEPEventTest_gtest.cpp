...
std::vector<std::string> subInputTable(const std::string& t, const int target_rows, const std::vector<dolphindb::EventSchema>& EventSchemas,
                                       const std::vector<std::string>& eventTimeFields, const std::vector<std::string>& commonFields) {
  ...
  auto _handler = [&](const std::string& eventType, std::vector<dolphindb::ConstantSP>& attribute) {
    ...
    if(total_rows % 100 == 0) std::cout << "total_rows: " << total_rows << std::endl;
...
TEST_F(StreamingCEPEventTest, test_EventType_special_character) {
  std::string case_ = getCaseName();
  std::string script = "share streamTable(1:0, `eventType`event, [STRING,BLOB]) as " + case_ +
                       "_inputTable;\n"
                       "share streamTable(1:0, `market`code`price`qty`eventTime, [STRING,STRING,DOUBLE,INT,TIMESTAMP]) as " +
                       case_ + "_outputTable;";
  conn->run(script);
  dolphindb::EventSchema* schema = new dolphindb::EventSchema();
  ...
  sender->sendEvent(schema->eventType_,
                    {dolphindb::Util::createString("sz"), dolphindb::Util::createString("000001"), dolphindb::Util::createDouble(10.5),
                     dolphindb::Util::createInt(100), dolphindb::Util::createTimestamp(2021, 1, 1, 10, 30, 0, 100)});
...
TEST_F(StreamingCEPEventTest, test_eventTimeFields_two_column) {
  std::string case_ = getCaseName();
  conn->run("share streamTable(1:0, `time`eventType`event, [TIME,STRING,BLOB]) as " + case_ +
            "_inputTable;"
            "share streamTable(1:0, `market`time, [STRING,TIME]) as " +
            case_ + "_outputTable;");
  dolphindb::EventSchema* schema = new dolphindb::EventSchema();
  schema->eventType_ = "market";
...
TEST_F(StreamingCEPEventTest, test_EventClient_fields_null_but_subTable_has_column) {
  std::string case_ = getCaseName();
  conn->run("share streamTable(1:0, `time`eventType`event`common, [TIME,STRING,BLOB, INT]) as " + case_ +
            "_inputTable;"
            "share streamTable(1:0, `market`time, [STRING,TIME]) as " +
            case_ + "_outputTable;");
  dolphindb::EventSchema* schema = new dolphindb::EventSchema();
...
TEST_F(StreamingCEPEventTest, test_commonFields_one_column) {
  std::string case_ = getCaseName();
  conn->run("share streamTable(1:0, `time`eventType`event`time1, [TIME,STRING,BLOB,TIME]) as " + case_ +
            "_inputTable;"
            "share streamTable(1:0, `market`time, [STRING,TIME]) as " +
            case_ + "_outputTable;");
  dolphindb::EventSchema* schema = new dolphindb::EventSchema();
...
TEST_F(StreamingCEPEventTest, test_two_schemas_two_commonFields) {
  std::string case_ = getCaseName();
  conn->run("share streamTable(1:0, `eventType`event`time`sym, [STRING,BLOB,TIME,STRING]) as " + case_ +
            "_inputTable;"
            "share streamTable(1:0, `market`time`sym, [STRING,TIME,STRING]) as " +
            case_ + "_outputTable;");
  dolphindb::EventSchema* schema = new dolphindb::EventSchema();
  schema->eventType_ = "market";
  ...
  std::vector<dolphindb::ConstantSP> attributes = {dolphindb::Util::createString("sz"), dolphindb::Util::createTime(12, 45, 3, 100),
                                                   dolphindb::Util::createString("sz000001")};
  sender->sendEvent("market", attributes);
  std::vector<dolphindb::ConstantSP> attributes1 = {dolphindb::Util::createString("sh"), dolphindb::Util::createTime(8, 30, 59, 100),
                                                    dolphindb::Util::createString("sh000001")};
  sender->sendEvent("market1", attributes1);
  dolphindb::Util::sleep(2000);
  client->unsubscribe(HOST, PORT, case_ + "_inputTable", DEFAULT_ACTION_NAME);
  auto outs = subInputTable(case_ + "_inputTable", 2, EventSchemas, eventTimeFields, commonFields);
...
TEST_F(StreamingCEPEventTest, test_commonFields_eventTimeFields_with_same_col) {
  std::string case_ = getCaseName();
  conn->run("share streamTable(1:0, `time`eventType`event`time2`sym, [TIME,STRING,BLOB,TIME,STRING]) as " + case_ +
            "_inputTable;"
            "share streamTable(1:0, `market`time`sym, [STRING,TIME,STRING]) as " +
            case_ + "_outputTable;");
  dolphindb::EventSchema* schema = new dolphindb::EventSchema();
...
TEST_F(StreamingCEPEventTest, test_EventClient_commonFields_cols_not_match) {
  ...
  conn->run("share streamTable(1:0, `time`eventType`event, [TIME,STRING,BLOB]) as " + case_ +
            "_inputTable;"
            "share streamTable(1:0, `market`time`col1, [STRING,TIME, INT]) as " +
            case_ + "_outputTable;");
  dolphindb::EventSchema* schema = new dolphindb::EventSchema();
  schema->eventType_ = "market";
  schema->fieldNames_ = {"market", "time", "col1"};
...
TEST_F(StreamingCEPEventTest, test_EventSender_sendEvent_attributes_column_not_match) {
  ...
  std::string re = "";
  try {
    sender->sendEvent("market",
                      {dolphindb::Util::createString("sz"), dolphindb::Util::createTime(12, 45, 3, 100), dolphindb::Util::createString("sz")});
  } catch(const std::exception& ex) {
    re = ex.what();
  }
...
class StreamingCEPEventTest_EventSender_alltypes : public StreamingCEPEventTest,
                                                   public ::testing::WithParamInterface<std::tuple<dolphindb::DATA_TYPE, std::string>> {
 public:
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_scalar_data() {
    return {
        std::make_tuple(dolphindb::DT_BOOL, "array(BOOL).append!(true false NULL)"),
        std::make_tuple(dolphindb::DT_CHAR, "-127c 127c NULL"),
        std::make_tuple(dolphindb::DT_SHORT, "1000h NULL"),
        std::make_tuple(dolphindb::DT_INT, "10000 NULL"),
        std::make_tuple(dolphindb::DT_LONG, "1000000l NULL"),
        std::make_tuple(dolphindb::DT_FLOAT, "100.000f NULL"),
        std::make_tuple(dolphindb::DT_DOUBLE, "100.0000 NULL"),
        std::make_tuple(dolphindb::DT_DATE, "2023.01.01 NULL"),
        std::make_tuple(dolphindb::DT_MONTH, "2023.01M NULL"),
        std::make_tuple(dolphindb::DT_TIME, "23:45:03.100 NULL"),
        std::make_tuple(dolphindb::DT_MINUTE, "23:45m NULL"),
        std::make_tuple(dolphindb::DT_SECOND, "23:45:03 NULL"),
        std::make_tuple(dolphindb::DT_DATEHOUR, "datehour('2021.01.01T12'`)"),
        std::make_tuple(dolphindb::DT_STRING, "`szstr`"),
        std::make_tuple(dolphindb::DT_DATETIME, "2021.01.01T12:45:03 NULL"),
        std::make_tuple(dolphindb::DT_TIMESTAMP, "2021.01.01T12:45:03.100 NULL"),
        std::make_tuple(dolphindb::DT_NANOTIME, "23:45:03.100000000 NULL"),
        std::make_tuple(dolphindb::DT_NANOTIMESTAMP, "2021.01.01T12:45:03.100000000 NULL"),
        ...
        std::make_tuple(dolphindb::DT_DECIMAL32, "decimal32('1.123456789'`'-135.1', 5)"),
        std::make_tuple(dolphindb::DT_DECIMAL64, "decimal64('-11.456464'`'300.1', 15)"),
        std::make_tuple(dolphindb::DT_DECIMAL128, "decimal128('999.64621462333'`'-1326', 25)"),
    };
  };
};

INSTANTIATE_TEST_SUITE_P(EventSender_alltype, StreamingCEPEventTest_EventSender_alltypes,
                         testing::ValuesIn(StreamingCEPEventTest_EventSender_alltypes::get_scalar_data()));
TEST_P(StreamingCEPEventTest_EventSender_alltypes, test_EventSender_alltype) {
  ...
  if(typeString.compare(0, 9, "DECIMAL32") == 0) {
    ...
    typeString = "DECIMAL128(25)";
    extraParam = 25;
  }
  conn->run("share streamTable(1:0, `eventTime`eventType`event`commonCol, [TIMESTAMP,STRING,BLOB," + typeString + "]) as " + case_ +
            "_fromApi;"
            "share streamTable(1:0, `eventTime`eventType`event`commonCol, [TIMESTAMP,STRING,BLOB," +
            typeString + "]) as " + case_ +
            "_fromCEP;go;"
            "class md{"
            "market :: STRING\n"
            ...
            "def updateMarketData(event){"
            "emitEvent(event)}};"
            "try{dropStreamEngine(`" +
            case_ +
            "_os)}catch(ex){};go;"
            "outputSerializer = streamEventSerializer(name=`" +
            case_ + "_os, EventSchema=md, outputTable=" + case_ +
            "_fromCEP, eventTimeField = 'eventTime', commonField='commonCol');"
            "try{dropStreamEngine(`" +
            case_ +
            "_CEPengine)}catch(ex){};go;"
            "engine = createCEPEngine(`" +
            case_ + "_CEPengine, <MainMonitor()>, " + case_ + "_fromCEP, [md], 1, 'eventTime', 10000, outputSerializer);");
  dolphindb::EventSchema* schema = new dolphindb::EventSchema();
  ...
  for(int i = 0; i < rows; i++) {
    ...
    conn->run("ind=" + std::to_string(i) +
              ";rowData = md(market[ind], time[ind], testCol[ind], commonCol[ind]);appendEvent(outputSerializer, [rowData])");
  }
...
class StreamingCEPEventTest_EventSender_vector : public StreamingCEPEventTest,
                                                 public ::testing::WithParamInterface<std::tuple<dolphindb::DATA_TYPE, std::string>> {
 public:
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_vector_data() {
    return {
        std::make_tuple(dolphindb::DT_BOOL, "true false"),
        std::make_tuple(dolphindb::DT_CHAR, "rand(127c,2)"),
        std::make_tuple(dolphindb::DT_SHORT, "rand(1000h,2)"),
        std::make_tuple(dolphindb::DT_INT, "rand(10000,2)"),
        std::make_tuple(dolphindb::DT_LONG, "rand(1000000l,2)"),
        std::make_tuple(dolphindb::DT_FLOAT, "rand(100.000f,2)"),
        std::make_tuple(dolphindb::DT_DOUBLE, "rand(100.0000,2)"),
        std::make_tuple(dolphindb::DT_DATE, "rand(2023.01.01,2)"),
        std::make_tuple(dolphindb::DT_MONTH, "rand(2023.01M,2)"),
        std::make_tuple(dolphindb::DT_TIME, "rand(23:45:03.100,2)"),
        std::make_tuple(dolphindb::DT_MINUTE, "rand(23:45m,2)"),
        std::make_tuple(dolphindb::DT_SECOND, "rand(23:45:03,2)"),
        std::make_tuple(dolphindb::DT_DATEHOUR, "rand(datehour('2021.01.01T12'),2)"),
        std::make_tuple(dolphindb::DT_DATETIME, "rand(2021.01.01T12:45:03,2)"),
        std::make_tuple(dolphindb::DT_TIMESTAMP, "rand(2021.01.01T12:45:03.100,2)"),
        std::make_tuple(dolphindb::DT_NANOTIME, "rand(23:45:03.100000000,2)"),
        ...
        std::make_tuple(dolphindb::DT_BLOB, "blob(`blob`blob2)"),
        std::make_tuple(dolphindb::DT_DECIMAL32, "rand(decimal32('1.123456789'`0'-135.1', 5),2)"),
        std::make_tuple(dolphindb::DT_DECIMAL64, "rand(decimal64('-11.456464'`0'300.1', 15),2)"),
  ...
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_vector_allNull_data() {
    return {
        std::make_tuple(dolphindb::DT_BOOL, "array(BOOL).append!(take(bool(NULL), 2))"),
        std::make_tuple(dolphindb::DT_CHAR, "array(CHAR).append!(take(char(NULL), 2))"),
        std::make_tuple(dolphindb::DT_SHORT, "array(SHORT).append!(take(short(NULL), 2))"),
        std::make_tuple(dolphindb::DT_INT, "array(INT).append!(take(int(NULL), 2))"),
        std::make_tuple(dolphindb::DT_LONG, "array(LONG).append!(take(long(NULL), 2))"),
        std::make_tuple(dolphindb::DT_FLOAT, "array(FLOAT).append!(take(float(NULL), 2))"),
        std::make_tuple(dolphindb::DT_DOUBLE, "array(DOUBLE).append!(take(double(NULL), 2))"),
        std::make_tuple(dolphindb::DT_DATE, "array(DATE).append!(take(date(NULL), 2))"),
        std::make_tuple(dolphindb::DT_MONTH, "array(MONTH).append!(take(month(NULL), 2))"),
        std::make_tuple(dolphindb::DT_TIME, "array(TIME).append!(take(time(NULL), 2))"),
        std::make_tuple(dolphindb::DT_MINUTE, "array(MINUTE).append!(take(minute(NULL), 2))"),
        std::make_tuple(dolphindb::DT_SECOND, "array(SECOND).append!(take(second(NULL), 2))"),
        std::make_tuple(dolphindb::DT_DATEHOUR, "array(DATEHOUR).append!(take(datehour(NULL), 2))"),
        std::make_tuple(dolphindb::DT_DATETIME, "array(DATETIME).append!(take(datetime(NULL), 2))"),
        std::make_tuple(dolphindb::DT_TIMESTAMP, "array(TIMESTAMP).append!(take(timestamp(NULL), 2))"),
        std::make_tuple(dolphindb::DT_NANOTIME, "array(NANOTIME).append!(take(nanotime(NULL), 2))"),
        ...
        std::make_tuple(dolphindb::DT_UUID, "array(UUID).append!(take(uuid(''), 2))"),
        std::make_tuple(dolphindb::DT_IP, "array(IPADDR).append!(take(ipaddr(''), 2))"),
        std::make_tuple(dolphindb::DT_INT128, "array(INT128).append!(take(int128(''), 2))"),
        std::make_tuple(dolphindb::DT_BLOB, "array(BLOB).append!(take(blob(''), 2))"),
        std::make_tuple(dolphindb::DT_DECIMAL32, "array(DECIMAL32(5)).append!(take(decimal32(NULL,5), 2))"),
        std::make_tuple(dolphindb::DT_DECIMAL64, "array(DECIMAL64(15)).append!(take(decimal64(NULL,15), 2))"),
        std::make_tuple(dolphindb::DT_DECIMAL128, "array(DECIMAL128(25)).append!(take(decimal128(NULL,25), 2))"),
    };
  };
};

INSTANTIATE_TEST_SUITE_P(vector_noNull, StreamingCEPEventTest_EventSender_vector,
                         testing::ValuesIn(StreamingCEPEventTest_EventSender_vector::get_vector_data()));
INSTANTIATE_TEST_SUITE_P(vector_allNull, StreamingCEPEventTest_EventSender_vector,
                         testing::ValuesIn(StreamingCEPEventTest_EventSender_vector::get_vector_allNull_data()));
TEST_P(StreamingCEPEventTest_EventSender_vector, test_EventSender_vector) {
  ...
  conn->run("share streamTable(1:0, `eventTime`eventType`event`commonCol, [TIMESTAMP,STRING,BLOB," + formatStr1 + "]) as " + case_ +
            "_fromApi;"
            "share streamTable(1:0, `eventTime`eventType`event`commonCol, [TIMESTAMP,STRING,BLOB," +
            formatStr1 + "]) as " + case_ +
            "_fromCEP;go;"
  ...
  schema->fieldNames_ = {"market", "time", "testCol", "commonCol"};
  schema->fieldTypes_ = {
      dolphindb::DT_STRING, dolphindb::DT_TIMESTAMP, dataType,
      dataType == dolphindb::DT_SYMBOL || dataType == dolphindb::DT_STRING || dataType == dolphindb::DT_BLOB ? dolphindb::DT_INT : dataType};
...
class StreamingCEPEventTest_EventSender_arrayVector : public StreamingCEPEventTest,
                                                      public ::testing::WithParamInterface<std::tuple<dolphindb::DATA_TYPE, std::string>> {
 public:
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_arrayVector_data() {
    return {
        std::make_tuple(dolphindb::DT_BOOL_ARRAY, "array(BOOL[]).append!([true false])"),
        std::make_tuple(dolphindb::DT_CHAR_ARRAY, "array(CHAR[]).append!([1c NULL])"),
        std::make_tuple(dolphindb::DT_SHORT_ARRAY, "array(SHORT[]).append!([1000h NULL])"),
        std::make_tuple(dolphindb::DT_INT_ARRAY, "array(INT[]).append!([10000 NULL])"),
        std::make_tuple(dolphindb::DT_LONG_ARRAY, "array(LONG[]).append!([1000000l NULL])"),
        std::make_tuple(dolphindb::DT_FLOAT_ARRAY, "array(FLOAT[]).append!([100.000f NULL])"),
        std::make_tuple(dolphindb::DT_DOUBLE_ARRAY, "array(DOUBLE[]).append!([100.0000 NULL])"),
        std::make_tuple(dolphindb::DT_DATE_ARRAY, "array(DATE[]).append!([2023.01.01 NULL])"),
        std::make_tuple(dolphindb::DT_MONTH_ARRAY, "array(MONTH[]).append!([2023.01M NULL])"),
        std::make_tuple(dolphindb::DT_TIME_ARRAY, "array(TIME[]).append!([23:45:03.100 NULL])"),
        std::make_tuple(dolphindb::DT_MINUTE_ARRAY, "array(MINUTE[]).append!([23:45m NULL])"),
        std::make_tuple(dolphindb::DT_SECOND_ARRAY, "array(SECOND[]).append!([23:45:03 NULL])"),
  ...
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_arrayVector_allNull_data() {
    return {
        std::make_tuple(dolphindb::DT_BOOL_ARRAY, "array(BOOL[]).append!([take(bool(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_CHAR_ARRAY, "array(CHAR[]).append!([take(char(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_SHORT_ARRAY, "array(SHORT[]).append!([take(short(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_INT_ARRAY, "array(INT[]).append!([take(int(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_LONG_ARRAY, "array(LONG[]).append!([take(long(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_FLOAT_ARRAY, "array(FLOAT[]).append!([take(float(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_DOUBLE_ARRAY, "array(DOUBLE[]).append!([take(double(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_DATE_ARRAY, "array(DATE[]).append!([take(date(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_MONTH_ARRAY, "array(MONTH[]).append!([take(month(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_TIME_ARRAY, "array(TIME[]).append!([take(time(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_MINUTE_ARRAY, "array(MINUTE[]).append!([take(minute(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_SECOND_ARRAY, "array(SECOND[]).append!([take(second(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_DATEHOUR_ARRAY, "array(DATEHOUR[]).append!([take(datehour(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_DATETIME_ARRAY, "array(DATETIME[]).append!([take(datetime(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_TIMESTAMP_ARRAY, "array(TIMESTAMP[]).append!([take(timestamp(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_NANOTIME_ARRAY, "array(NANOTIME[]).append!([take(nanotime(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_NANOTIMESTAMP_ARRAY, "array(NANOTIMESTAMP[]).append!([take(nanotimestamp(NULL), 2)])"),
        std::make_tuple(dolphindb::DT_UUID_ARRAY, "array(UUID[]).append!([take(uuid(), 2)])"),
        std::make_tuple(dolphindb::DT_IP_ARRAY, "array(IPADDR[]).append!([take(ipaddr(), 2)])"),
...
INSTANTIATE_TEST_SUITE_P(EventSender_arrayVector, StreamingCEPEventTest_EventSender_arrayVector,
                         testing::ValuesIn(StreamingCEPEventTest_EventSender_arrayVector::get_arrayVector_data()));
INSTANTIATE_TEST_SUITE_P(EventSender_arrayVector_allNull, StreamingCEPEventTest_EventSender_arrayVector,
                         testing::ValuesIn(StreamingCEPEventTest_EventSender_arrayVector::get_arrayVector_allNull_data()));
TEST_P(StreamingCEPEventTest_EventSender_arrayVector,
       test_EventSender_arrayVector)  // commonFields不能包含array vector类型
{
  std::string case_ = getCaseName();
  dolphindb::DATA_TYPE dataType = std::get<0>(GetParam());
  std::string type_data_str = std::get<1>(GetParam());
...
class StreamingCEPEventTest_EventClient_alltypes : public StreamingCEPEventTest,
                                                   public ::testing::WithParamInterface<std::tuple<dolphindb::DATA_TYPE, std::string>> {
 public:
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_scalar_data() {
    return {
...
INSTANTIATE_TEST_SUITE_P(EventClient_alltype, StreamingCEPEventTest_EventClient_alltypes,
                         testing::ValuesIn(StreamingCEPEventTest_EventClient_alltypes::get_scalar_data()));
TEST_P(StreamingCEPEventTest_EventClient_alltypes, test_EventClient_alltype) {
  ...
  schema->fieldForms_ = {dolphindb::DF_SCALAR, dolphindb::DF_SCALAR, dolphindb::DF_SCALAR, dolphindb::DF_SCALAR};
  schema->fieldExtraParams_ = {0, 0, extraParam, extraParam};
  conn->run("share streamTable(1:0, `time`eventType`event`commonCol, [TIMESTAMP,STRING,BLOB," + typeString + "]) as " + case_ +
            "_fromCEP;"
            "share streamTable(1:0, `time`eventType`event`commonCol, [TIMESTAMP,STRING,BLOB," +
            typeString + "]) as " + case_ +
            "_fromApi;go;"
            "class md{"
            "market :: STRING\n"
            ...
            "try{dropStreamEngine(`" +
            case_ +
            "_EventSer)}catch(ex){};go;"
            "inputSerializer = streamEventSerializer(name=`" +
            case_ + "_EventSer, EventSchema=md, outputTable=" + case_ + "_fromCEP, eventTimeField = \"eventTime\", commonField = \"commonCol\");");
  dolphindb::Signal notify;
  dolphindb::Mutex mutex;
  ...
  conn->run("rows = " + std::to_string(test_rows) +
            ";"
            "val_col0=rand(`sz`sh`bj`adk, rows);val_col1=now()..temporalAdd(now(), rows-1, 'ms');val_col2=rand(" +
            type_data_str + ", rows);val_col3=rand(" + type_data_str +
            ", rows);"
            "for (i in 0:rows){"
            "data = md(val_col0[i], val_col1[i], val_col2[i], val_col3[i]);"
            "appendEvent(getStreamEngine(`" +
            case_ + "_EventSer), [data]);}");
  notify.wait();
  dolphindb::Util::sleep(1000);
...
class StreamingCEPEventTest_EventClient_alltypes_vector : public StreamingCEPEventTest,
                                                          public ::testing::WithParamInterface<std::tuple<dolphindb::DATA_TYPE, std::string>> {
 public:
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_vector_data() {
    return {
        std::make_tuple(dolphindb::DT_BOOL, "array(BOOL).append!(true false)"),
        std::make_tuple(dolphindb::DT_CHAR, "-127c 127c"),
        std::make_tuple(dolphindb::DT_SHORT, "1000h"),
        std::make_tuple(dolphindb::DT_INT, "10000"),
        std::make_tuple(dolphindb::DT_LONG, "1000000l"),
        std::make_tuple(dolphindb::DT_FLOAT, "100.000f"),
        std::make_tuple(dolphindb::DT_DOUBLE, "100.0000"),
        std::make_tuple(dolphindb::DT_DATE, "2023.01.01"),
        std::make_tuple(dolphindb::DT_MONTH, "2023.01M"),
        std::make_tuple(dolphindb::DT_TIME, "23:45:03.100"),
        std::make_tuple(dolphindb::DT_MINUTE, "23:45m"),
        std::make_tuple(dolphindb::DT_SECOND, "23:45:03"),
        std::make_tuple(dolphindb::DT_DATEHOUR, "datehour('2021.01.01T12'`)"),
        std::make_tuple(dolphindb::DT_DATETIME, "2021.01.01T12:45:03"),
        std::make_tuple(dolphindb::DT_TIMESTAMP, "2021.01.01T12:45:03.100"),
        std::make_tuple(dolphindb::DT_NANOTIME, "23:45:03.100000000"),
        std::make_tuple(dolphindb::DT_NANOTIMESTAMP, "2021.01.01T12:45:03.100000000"),
        std::make_tuple(dolphindb::DT_SYMBOL, "symbol(take(`sz`da`jt, 100000))"),
        std::make_tuple(dolphindb::DT_STRING, "`szstr`"),
        std::make_tuple(dolphindb::DT_UUID, "uuid('5d212a78-cc48-e3b1-4235-b4d91473ee87''5d212a78-cc48-e3b1-4235-b4d91473ee10')"),
        std::make_tuple(dolphindb::DT_IP, "ipaddr('127.0.0.1''1.1.1.1')"),
        std::make_tuple(dolphindb::DT_INT128, "int128(`e1671797c52e15f763380b45e841ec32`e1671797c52e15f763380b45e841ec42)"),
        std::make_tuple(dolphindb::DT_BLOB, "blob(`blob`blob2)"),
        std::make_tuple(dolphindb::DT_DECIMAL32, "decimal32('1.123456789'`0'-135.1', 5)"),
        std::make_tuple(dolphindb::DT_DECIMAL64, "decimal64('-11.456464'`0'300.1', 15)"),
        std::make_tuple(dolphindb::DT_DECIMAL128, "decimal128('999.64621462333'`0'-1326', 25)"),
    };
  };
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_vector_allNull_data() {
    return {
        std::make_tuple(dolphindb::DT_BOOL, "array(BOOL).append!([bool(NULL), bool(NULL)])"),
        std::make_tuple(dolphindb::DT_CHAR, "array(CHAR).append!([char(NULL), char(NULL)])"),
        std::make_tuple(dolphindb::DT_SHORT, "array(SHORT).append!([short(NULL), short(NULL)])"),
        std::make_tuple(dolphindb::DT_INT, "array(INT).append!([int(NULL), int(NULL)])"),
        std::make_tuple(dolphindb::DT_LONG, "array(LONG).append!([long(NULL), long(NULL)])"),
        std::make_tuple(dolphindb::DT_FLOAT, "array(FLOAT).append!([float(NULL), float(NULL)])"),
        std::make_tuple(dolphindb::DT_DOUBLE, "array(DOUBLE).append!([double(NULL), double(NULL)])"),
        std::make_tuple(dolphindb::DT_DATE, "array(DATE).append!([date(NULL), date(NULL)])"),
        std::make_tuple(dolphindb::DT_MONTH, "array(MONTH).append!([month(NULL), month(NULL)])"),
        std::make_tuple(dolphindb::DT_TIME, "array(TIME).append!([time(NULL), time(NULL)])"),
        std::make_tuple(dolphindb::DT_MINUTE, "array(MINUTE).append!([minute(NULL), minute(NULL)])"),
        std::make_tuple(dolphindb::DT_SECOND, "array(SECOND).append!([second(NULL), second(NULL)])"),
        std::make_tuple(dolphindb::DT_DATEHOUR, "array(DATEHOUR).append!([datehour(NULL), datehour(NULL)])"),
        std::make_tuple(dolphindb::DT_DATETIME, "array(DATETIME).append!([datetime(NULL), datetime(NULL)])"),
        std::make_tuple(dolphindb::DT_TIMESTAMP, "array(TIMESTAMP).append!([timestamp(NULL), timestamp(NULL)])"),
        std::make_tuple(dolphindb::DT_NANOTIME, "array(NANOTIME).append!([nanotime(NULL), nanotime(NULL)])"),
        std::make_tuple(dolphindb::DT_NANOTIMESTAMP, "array(NANOTIMESTAMP).append!([nanotimestamp(NULL), nanotimestamp(NULL)])"),
        std::make_tuple(dolphindb::DT_UUID, "array(UUID).append!([uuid(), uuid()])"),
        std::make_tuple(dolphindb::DT_IP, "array(IPADDR).append!([ipaddr(), ipaddr()])"),
...
INSTANTIATE_TEST_SUITE_P(vector_noNull, StreamingCEPEventTest_EventClient_alltypes_vector,
                         testing::ValuesIn(StreamingCEPEventTest_EventClient_alltypes_vector::get_vector_data()));
INSTANTIATE_TEST_SUITE_P(vector_allNull, StreamingCEPEventTest_EventClient_alltypes_vector,
                         testing::ValuesIn(StreamingCEPEventTest_EventClient_alltypes_vector::get_vector_allNull_data()));
...
#define NOT_LITERAL dataType != dolphindb::DT_SYMBOL&& dataType != dolphindb::DT_STRING&& dataType != dolphindb::DT_BLOB
  ...
  schema->fieldExtraParams_ = {0, 0, extraParam, extraParam};
  conn->run("share streamTable(1:0, `time`eventType`event`commonCol, [TIMESTAMP,STRING,BLOB," + formatCommonColstr + "]) as " + case_ +
            "_fromCEP;"
            "share streamTable(1:0, `time`eventType`event`commonCol, [TIMESTAMP,STRING,BLOB," +
            formatCommonColstr + "]) as " + case_ +
            "_fromApi;go;"
            "class md{"
            "market :: STRING\n"
            "eventTime :: TIMESTAMP\n"
  ...
  if(dataType == dolphindb::DT_SYMBOL) val_col2_scripts = {"symbol(string(rand([\"blob\"], 10)))", "symbol(string(rand([\"blob\"], 10)))"};
  conn->run("rows = " + std::to_string(test_rows) +
            ";"
            "val_col0=rand(`sz`sh`bj`adk, rows);val_col1=now()..temporalAdd(now(), rows-1, 'ms');val_col2=" +
            val_col2_scripts[0] + ";val_col3=rand(" + (NOT_LITERAL ? type_data_str : "1000") +
            ", rows);"
            "vector_size = rand(2000 0, 1)[0];"
            "for (i in 0:rows){"
            "col2 = " +
            val_col2_scripts[1] + ";col3=" + (NOT_LITERAL ? "rand(val_col3, vector_size)" : "val_col3[i]") +
            ";"
            "data = md(val_col0[i], val_col1[i], col2, col3);"
            "appendEvent(getStreamEngine(`" +
            case_ + "_EventSer), [data]);};go;");
  notify.wait();
  dolphindb::Util::sleep(1000);
  client->unsubscribe(HOST, PORT, case_ + "_fromCEP", DEFAULT_ACTION_NAME);
  client->exit();
...
class StreamingCEPEventTest_EventClient_arrayVector : public StreamingCEPEventTest,
                                                      public ::testing::WithParamInterface<std::tuple<dolphindb::DATA_TYPE, std::string>> {
 public:
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_arrayVector_data() {
    return {
        std::make_tuple(dolphindb::DT_BOOL_ARRAY, "array(BOOL[]).append!([rand(true false, 2)]).append!([rand(true false, 2)])"),
        std::make_tuple(dolphindb::DT_CHAR_ARRAY, "array(CHAR[]).append!([rand(127c, 2)]).append!([rand(127c, 2)])"),
        std::make_tuple(dolphindb::DT_SHORT_ARRAY, "array(SHORT[]).append!([rand(1000h, 2)]).append!([rand(1000h, 2)])"),
        std::make_tuple(dolphindb::DT_INT_ARRAY, "array(INT[]).append!([rand(10000, 2)]).append!([rand(10000, 2)])"),
        std::make_tuple(dolphindb::DT_LONG_ARRAY, "array(LONG[]).append!([rand(1000000l, 2)]).append!([rand(1000000l, 2)])"),
        std::make_tuple(dolphindb::DT_FLOAT_ARRAY, "array(FLOAT[]).append!([rand(100.000f, 2)]).append!([rand(100.000f, 2)])"),
        std::make_tuple(dolphindb::DT_DOUBLE_ARRAY, "array(DOUBLE[]).append!([rand(100.0000, 2)]).append!([rand(100.0000, 2)])"),
        std::make_tuple(dolphindb::DT_DATE_ARRAY, "array(DATE[]).append!([rand(2023.01.01, 2)]).append!([rand(2023.01.01, 2)])"),
        std::make_tuple(dolphindb::DT_MONTH_ARRAY, "array(MONTH[]).append!([rand(2023.01M, 2)]).append!([rand(2023.01M, 2)])"),
        std::make_tuple(dolphindb::DT_TIME_ARRAY, "array(TIME[]).append!([rand(23:45:03.100, 2)]).append!([rand(23:45:03.100, 2)])"),
        std::make_tuple(dolphindb::DT_MINUTE_ARRAY, "array(MINUTE[]).append!([rand(23:45m, 2)]).append!([rand(23:45m, 2)])"),
        std::make_tuple(dolphindb::DT_SECOND_ARRAY, "array(SECOND[]).append!([rand(23:45:03, 2)]).append!([rand(23:45:03, 2)])"),
        std::make_tuple(dolphindb::DT_DATEHOUR_ARRAY,
                        "array(DATEHOUR[]).append!([rand(datehour('2021.01.01T12'`), "
                        "2)]).append!([rand(datehour('2021.01.01T12'`), 2)])"),
        std::make_tuple(dolphindb::DT_DATETIME_ARRAY,
                        "array(DATETIME[]).append!([rand(2021.01.01T12:45:03, 2)]).append!([rand(2021.01.01T12:45:03, 2)])"),
        std::make_tuple(dolphindb::DT_TIMESTAMP_ARRAY,
                        "array(TIMESTAMP[]).append!([rand(2021.01.01T12:45:03.100, "
                        "2)]).append!([rand(2021.01.01T12:45:03.100, 2)])"),
        std::make_tuple(dolphindb::DT_NANOTIME_ARRAY,
                        "array(NANOTIME[]).append!([rand(23:45:03.100000000, 2)]).append!([rand(23:45:03.100000000, 2)])"),
        std::make_tuple(dolphindb::DT_NANOTIMESTAMP_ARRAY,
                        "array(NANOTIMESTAMP[]).append!([rand(2021.01.01T12:45:03.100000000, "
                        "2)]).append!([rand(2021.01.01T12:45:03.100000000, 2)])"),
        std::make_tuple(dolphindb::DT_UUID_ARRAY,
                        "array(UUID[]).append!([rand(uuid('5d212a78-cc48-e3b1-4235-b4d91473ee87''5d212a78-cc48-e3b1-"
                        "4235-b4d91473ee10'), "
                        "2)]).append!([rand(uuid('5d212a78-cc48-e3b1-4235-b4d91473ee44''5d212a78-cc48-e3b1-4235-"
                        "b4d91473ee15'), 2)])"),
        std::make_tuple(dolphindb::DT_IP_ARRAY,
                        "array(IPADDR[]).append!([rand(ipaddr('127.0.0.1''1.1.1.1'), "
                        "2)]).append!([rand(ipaddr('2.2.2.2''255.255.0.0'), 2)])"),
        std::make_tuple(dolphindb::DT_INT128_ARRAY,
                        "array(INT128[]).append!([rand(int128(`e1671797c52e15f763380b45e841ec32`e1671797c52e15f763380b45e841ec42), "
                        "2)]).append!([rand(int128(`e1671797c52e15f763380b45e841ec76`e1671797c52e15f763380b45e841ec89), 2)])"),
    };
  };
  static std::vector<std::tuple<dolphindb::DATA_TYPE, std::string>> get_arrayVector_allNull_data() {
    return {
...
INSTANTIATE_TEST_SUITE_P(arrayVector_noNull, StreamingCEPEventTest_EventClient_arrayVector,
                         testing::ValuesIn(StreamingCEPEventTest_EventClient_arrayVector::get_arrayVector_data()));
INSTANTIATE_TEST_SUITE_P(arrayVector_allNull, StreamingCEPEventTest_EventClient_arrayVector,
                         testing::ValuesIn(StreamingCEPEventTest_EventClient_arrayVector::get_arrayVector_allNull_data()));
TEST_P(StreamingCEPEventTest_EventClient_arrayVector, test_EventClient_arrayVector) {
  ...
  conn->run("rows = " + std::to_string(test_rows) +
            ";"
            "val_col0=rand(`sz`sh`bj`adk, rows);val_col1=now()..temporalAdd(now(), rows-1, 'ms');val_col2=" +
            val_col2_scripts[0] + ";val_col3=rand(" + type_data_str +
            ", rows);"
            "vector_size = rand(0 2000, 1)[0];"
            "for (i in 0:rows){"
            "col2 = " +
            val_col2_scripts[1] +
            ";col3=rand(val_col3, vector_size);"
            "data = md(val_col0[i], val_col1[i], col2, col3);"
            "appendEvent(getStreamEngine(`" +
            case_ + "_EventSer), [data]);};go;");
  notify.wait();
