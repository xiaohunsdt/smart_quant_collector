...
class HighAvailableTest : public testing::Test {
 ...
 protected:
  // Case
  virtual void SetUp() { restartAllNodes(HOST_CLUSTER, PORT_CONTROLLER); }
  virtual void TearDown() { restartAllNodes(HOST_CLUSTER, PORT_CONTROLLER); }
};
...
TEST_F(HighAvailableTest, test_connHA_basic_should_serial) {
  ...
  std::string nodeName =
      controller->run("exec name from getClusterPerf() where mode in 0 4 and port = " + std::to_string(nodePort))->get(0)->getString();
  controller->close();
...
TEST_F(HighAvailableTest, test_MTW_HA_should_serial) {
  ...
  mulwrite = new dolphindb::MultithreadedTableWriter(HOST_CLUSTER, PORT_DNODE1, USER_CLUSTER, PASSWD_CLUSTER, dbName, "pt", false, true, &sites, 100,
                                                     1, 1, "date");
...
TEST_F(HighAvailableTest, test_MTW_HA_2_should_serial) {
  ...
  dolphindb::SmartPointer<dolphindb::MultithreadedTableWriter> mulwrite;
  dolphindb::ErrorCodeInfo pErrorInfo;
  mulwrite = new dolphindb::MultithreadedTableWriter(HOST_CLUSTER, PORT_DNODE1, USER_CLUSTER, PASSWD_CLUSTER, dbName, "pt", false, true, &sites, 1000,
                                                     1, 4, "date");
  std::string sym[] = {"A", "B", "C", "D"};
  ...
  std::thread th1 = std::thread([&] { mulwrite->insertUnwrittenData(datas, pErrorInfo); });
...
TEST_F(HighAvailableTest, test_MTW_HA_3_should_serial) {
  ...
  script +=
      "t = table(1000:0, `symbol`date`value,[SYMBOL, TIME, INT]);"
      "pt = db.createPartitionedTable(t,`pt,`date);"
      "grant(`mtw_test, TABLE_WRITE, '" +
      dbName + "/pt');grant(`mtw_test, TABLE_READ, '" + dbName + "/pt');";
  connImp.run(script);
  dolphindb::SmartPointer<dolphindb::MultithreadedTableWriter> mulwrite;
  dolphindb::ErrorCodeInfo pErrorInfo;
  mulwrite = new dolphindb::MultithreadedTableWriter(HOST_CLUSTER, PORT_DNODE1, "mtw_test", PASSWD_CLUSTER, dbName, "pt", false, true, &sites, 100, 1,
                                                     1, "date");
  std::string sym[] = {"A", "B", "C", "D"};
...
struct CallBackStruct {
  ...
  bool operator==(const CallBackStruct& other) const { return state == other.state && host == other.host && port == other.port; }
};
