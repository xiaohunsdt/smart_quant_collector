...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_appendErrTableColNums) {
  ...
  std::vector<dolphindb::ConstantSP> cols = {dolphindb::Util::createString("zzz123中文a"), dolphindb::Util::createInt(7),
                                             dolphindb::Util::createInt(5)};
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_appendAllDataTypesToinMemoryTable) {
  ...
  for(int j = 0; j < colNum; j++) columnVecs[j]->setNull(rowNum - 1);
  ...
  for(int i = 0; i < result2->size(); i++) ASSERT_TRUE(result2->get(i)->getBool());
}
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_appendAllDataTypesTopartitionedTableOLAP) {
  ...
  for(int j = 0; j < colNum; j++) {
    if(j == 3)
      columnVecs[3]->set(rowNum - 1,
                         dolphindb::Util::createInt(rand() % INT_MAX));  // partition-column's value must be not null
    else
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_date) {
  ...
  std::vector<dolphindb::VectorSP> columnVecs;
  columnVecs.reserve(colNum);
  for(int i = 0; i < colNum; i++) columnVecs.emplace_back(tmp1->getColumn(i));
  ...
  std::string script2;
  script2 +=
      "expected = table(1969.01.01+0..9 as date1, 1969.01.01+0..9 as date2, 1969.01.01+0..9 as date3, 1969.01.01+0..9 "
      "as date4);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result = conn.run(script2);
  for(int i = 0; i < 4; i++) ASSERT_EQ(result->getInt(i), 1);
  ...
  for(int i = 0; i < colNum; i++) columnVecs1.emplace_back(tmp2->getColumn(i));
  for(int i = 0; i < rowNum; i++) {
  ...
  std::string script3;
  script3 +=
      "expected = table(2012.01.01+0..9 as date1, 2012.01.01+0..9 as date2, 2012.01.01+0..9 as date3, 2012.01.01+0..9 "
      "as date4);";
  script3 += "each(eqObj, (select * from st1 where date1>1970.01.01).values(), expected.values());";
  dolphindb::ConstantSP result2 = conn.run(script3);
  for(int i = 0; i < 4; i++) ASSERT_EQ(result2->getInt(i), 1);
}
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_month) {
  ...
  std::string script2;
  script2 +=
      "expected = table(1969.01M+0..9 as month1, 1969.01M+0..9 as month2, 1969.01M+0..9 as month3, 1969.01M+0..9 as "
      "month4, 1969.01M+0..9 as month5);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result1 = conn.run(script2);
  for(int i = 0; i < 5; i++) ASSERT_EQ(result1->getInt(i), 1);

  dolphindb::TableSP tmp2 = dolphindb::Util::createTable(colNames, colTypes, rowNum, rowNum);
  std::vector<dolphindb::VectorSP> columnVecs2;
  columnVecs2.reserve(colNum);
  for(int i = 0; i < colNum; i++) columnVecs2.emplace_back(tmp2->getColumn(i));
  for(int i = 0; i < rowNum; i++) {
  ...
  std::string script3;
  script3 +=
      "expected = table(2000.01M+0..9 as month1, 2000.01M+0..9 as month2, 2000.01M+0..9 as month3, 2000.01M+0..9 as "
      "month4, 2000.01M+0..9 as month5);";
  script3 += "each(eqObj,(select * from st1 where month1>1970.01M).values(), expected.values());";
  dolphindb::ConstantSP result2 = conn.run(script3);
  for(int i = 0; i < 5; i++) ASSERT_EQ(result2->getInt(i), 1);
}
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_datetime) {
  ...
  std::vector<dolphindb::VectorSP> columnVecs;
  columnVecs.reserve(colNum);
  for(int i = 0; i < colNum; i++) columnVecs.emplace_back(tmp->getColumn(i));
  for(int i = 0; i < rowNum; i++) {
  ...
  std::string script2;
  script2 +=
      "expected = table(1969.01.01T12:30:00+0..9 as col1, 1969.01.01T12:30:00+0..9 as col2, 1969.01.01T12:30:00+0..9 "
      "as col3, take(1969.01.01T00:00:00, 10) as col4);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result1 = conn.run(script2);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result1->getInt(i), 1);

  dolphindb::TableSP tmp1 = dolphindb::Util::createTable(colNames, colTypes, rowNum, rowNum);
  std::vector<dolphindb::VectorSP> columnVecs1;
  columnVecs1.reserve(colNum);
  for(int i = 0; i < colNum; i++) columnVecs1.emplace_back(tmp1->getColumn(i));
  for(int i = 0; i < rowNum; i++) {
  ...
  std::string script3;
  script3 +=
      "expected = table(2001.01.01T12:30:00+0..9 as col1, 2001.01.01T12:30:00+0..9 as col2, 2001.01.01T12:30:00+0..9 "
      "as col3, take(2001.01.01T00:00:00, 10) as col4);";
  script3 += "each(eqObj, (select * from st1 where col1>1970.01.01T00:00:00).values(), expected.values());";
  dolphindb::ConstantSP result2 = conn.run(script3);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result2->getInt(i), 1);
}
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_timestamp) {
  ...
  std::vector<dolphindb::DATA_TYPE> colTypes = {dolphindb::DT_DATETIME, dolphindb::DT_TIMESTAMP, dolphindb::DT_NANOTIMESTAMP,
                                                dolphindb::DT_NANOTIMESTAMP, dolphindb::DT_DATE};
  int colNum = 5, rowNum = 10;
  dolphindb::TableSP tmp1 = dolphindb::Util::createTable(colNames, colTypes, rowNum, rowNum);
  std::vector<dolphindb::VectorSP> columnVecs;
  columnVecs.reserve(colNum);
  ...
  std::string script2;
  script2 +=
      "expected = table(temporalAdd(1969.01.01T12:30:00.000, 0..9, \"s\") as col1, "
      "temporalAdd(1969.01.01T12:30:00.123, 0..9, \"s\") as col2, temporalAdd(1969.01.01T12:30:00.123, 0..9, \"s\") as "
      "col3, take(1960.01.01T12:30:10.008, 10) as col4, take(1969.01.01T00:00:00.000, 10) as col5);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result1 = conn.run(script2);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result1->getInt(i), 1);
  ...
  std::string script3;
  script3 +=
      "expected = table(temporalAdd(2000.01.01T12:30:00.000, 0..9, \"s\") as col1, "
      "temporalAdd(2000.01.01T12:30:00.123, 0..9, \"s\") as col2, temporalAdd(2000.01.01T12:30:00.123, 0..9, \"s\") as "
      "col3, take(2000.01.01T12:30:10.008, 10) as col4, take(2000.01.01T00:00:00.000, 10) as col5);";
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_nanotimestamp) {
  ...
  std::string script2;
  script2 +=
      "expected = table(temporalAdd(1969.01.01T12:30:00.000000000, 0..9, \"s\") as col1, "
      "temporalAdd(1969.01.01T12:30:00.123000000, 0..9, \"s\") as col2, temporalAdd(1969.01.01T12:30:00.123456789, "
      "0..9, \"s\") as col3, take(1969.01.01T00:00:00.000000000, 10) as col4);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result = conn.run(script2);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result->getInt(i), 1);
  ...
  std::string script3;
  script3 +=
      "expected = table(temporalAdd(2012.01.01T12:30:00.000000000, 0..9, \"s\") as col1, "
      "temporalAdd(2012.01.01T12:30:00.123000000, 0..9, \"s\") as col2, temporalAdd(2012.01.01T12:30:00.123456789, "
      "0..9, \"s\") as col3, take(2012.01.01T00:00:00.000000000, 10) as col4);";
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_minute) {
  ...
  std::vector<dolphindb::DATA_TYPE> colTypes = {dolphindb::DT_DATETIME, dolphindb::DT_TIMESTAMP, dolphindb::DT_NANOTIMESTAMP, dolphindb::DT_TIME,
                                                dolphindb::DT_SECOND,   dolphindb::DT_MINUTE,    dolphindb::DT_NANOTIME};
  ...
  std::string script2;
  script2 +=
      "expected = table(12:00m+0..9 as col1, 12:00m+0..9 as col2, 12:00m+0..9 as col3, 12:00m+0..9 as col4, "
      "12:00m+0..9 as col5, 12:00m+0..9 as col6, 12:00m+0..9 as col7);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result1 = conn.run(script2);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result1->getInt(i), 1);
  ...
  std::string script3;
  script3 +=
      "expected = table(12:10m+0..9 as col1, 12:10m+0..9 as col2, 12:10m+0..9 as col3, 12:10m+0..9 as col4, "
      "12:10m+0..9 as col5, 12:10m+0..9 as col6, 12:10m+0..9 as col7);";
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_time) {
  ...
  std::string script2;
  script2 +=
      "expected = table(temporalAdd(12:00:30.000, 0..9, \"m\") as col1, temporalAdd(12:00:30.123, 0..9, \"m\") as "
      "col2, temporalAdd(12:00:30.123, 0..9, \"m\") as col3, temporalAdd(12:00:30.123, 0..9, \"m\") as col4, "
      "temporalAdd(12:00:30.000, 0..9, \"m\") as col5, temporalAdd(12:00:00.000, 0..9, \"m\") as col6, "
      "temporalAdd(12:00:30.123, 0..9, \"m\") as col7);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result1 = conn.run(script2);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result1->getInt(i), 1);
  ...
  std::string script3;
  script3 +=
      "expected = table(temporalAdd(12:10:30.000, 0..9, \"m\") as col1, temporalAdd(12:10:30.123, 0..9, \"m\") as "
      "col2, temporalAdd(12:10:30.123, 0..9, \"m\") as col3, temporalAdd(12:10:30.123, 0..9, \"m\") as col4, "
      "temporalAdd(12:10:30.000, 0..9, \"m\") as col5, temporalAdd(12:10:00.000, 0..9, \"m\") as col6, "
      "temporalAdd(12:10:30.123, 0..9, \"m\") as col7);";
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_second) {
  ...
  std::string script2;
  script2 +=
      "expected = table(temporalAdd(12:00:30, 0..9, \"m\") as col1, temporalAdd(12:00:30, 0..9, \"m\") as col2, "
      "temporalAdd(12:00:30, 0..9, \"m\") as col3, temporalAdd(12:00:30, 0..9, \"m\") as col4, temporalAdd(12:00:30, "
      "0..9, \"m\") as col5, temporalAdd(12:00:00, 0..9, \"m\") as col6, temporalAdd(12:00:30, 0..9, \"m\") as col7);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result = conn.run(script2);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result->getInt(i), 1);
  ...
  std::string script3;
  script3 +=
      "expected = table(temporalAdd(12:10:30, 0..9, \"m\") as col1, temporalAdd(12:10:30, 0..9, \"m\") as col2, "
      "temporalAdd(12:10:30, 0..9, \"m\") as col3, temporalAdd(12:10:30, 0..9, \"m\") as col4, temporalAdd(12:10:30, "
      "0..9, \"m\") as col5, temporalAdd(12:10:00, 0..9, \"m\") as col6, temporalAdd(12:10:30, 0..9, \"m\") as col7);";
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_nanotime) {
  std::string script1;
  script1 +=
      "st1=table(100:0, `col1`col2`col3`col4`col5`col6`col7, [NANOTIME, NANOTIME, NANOTIME, NANOTIME, NANOTIME, "
      "NANOTIME, NANOTIME])";
  conn.run(script1);
  dolphindb::AutoFitTableAppender appender("", "st1", conn);
  std::vector<std::string> colNames = {"col1", "col2", "col3", "col4", "col5", "col6", "col7"};
  ...
  std::string script2;
  script2 +=
      "expected = table(temporalAdd(12:00:30.000000000, 0..9, \"m\") as col1, temporalAdd(12:00:30.123000000, 0..9, "
      "\"m\") as col2, temporalAdd(12:00:30.123456789, 0..9, \"m\") as col3, temporalAdd(12:00:30.123000000, 0..9, "
      "\"m\") as col4, temporalAdd(12:00:30.000000000, 0..9, \"m\") as col5, temporalAdd(12:00:00.000000000, 0..9, "
      "\"m\") as col6, temporalAdd(12:00:30.123456789, 0..9, \"m\") as col7);";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result = conn.run(script2);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result->getInt(i), 1);
  ...
  std::string script3;
  script3 +=
      "expected = table(temporalAdd(12:10:30.000000000, 0..9, \"m\") as col1, temporalAdd(12:10:30.123000000, 0..9, "
      "\"m\") as col2, temporalAdd(12:10:30.123456789, 0..9, \"m\") as col3, temporalAdd(12:10:30.123000000, 0..9, "
      "\"m\") as col4, temporalAdd(12:10:30.000000000, 0..9, \"m\") as col5, temporalAdd(12:10:00.000000000, 0..9, "
      "\"m\") as col6, temporalAdd(12:10:30.123456789, 0..9, \"m\") as col7);";
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_convert_to_datehour) {
  ...
  dolphindb::AutoFitTableAppender appender("", "st1", conn);
  std::vector<std::string> colNames = {"col1", "col2", "col3", "col4", "col5"};
  std::vector<dolphindb::DATA_TYPE> colTypes = {
      dolphindb::DT_DATE, dolphindb::DT_DATEHOUR, dolphindb::DT_DATETIME, dolphindb::DT_TIMESTAMP, dolphindb::DT_NANOTIMESTAMP,
  };
  int colNum = 5, rowNum = 10;
  dolphindb::TableSP tmp = dolphindb::Util::createTable(colNames, colTypes, rowNum, rowNum);
  std::vector<dolphindb::VectorSP> columnVecs;
  ...
  std::string script2;
  script2 +=
      "expected =table(take(datehour(1969.01.01T00:00:00),10) as col1, datehour(1969.01.01T01:00:00)+0..9 as col2, "
      "datehour(1969.01.01T01:00:00)+0..9 as col3, datehour(1969.01.01T01:00:00)+0..9 as col4, "
      "datehour(1969.01.01T01:00:00)+0..9 as col5  );";
  script2 += "each(eqObj, st1.values(), expected.values());";
  dolphindb::ConstantSP result1 = conn.run(script2);
  for(int i = 0; i < colNum; i++) ASSERT_EQ(result1->getInt(i), 1);
  ...
  std::string script3;
  script3 +=
      "expected =table(take(datehour(2001.01.01T00:00:00),10) as col1, datehour(2001.01.01T01:00:00)+0..9 as col2, "
      "datehour(2001.01.01T01:00:00)+0..9 as col3, datehour(2001.01.01T01:00:00)+0..9 as col4, "
      "datehour(2001.01.01T01:00:00)+0..9 as col5  );";
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_withCharArrayVectorToPartitionTableRangeTypeMorethan65535) {
  ...
  v1->setInt(1, 100);
  v1->setInt(2, 9999);
  for(unsigned int i = 0; i < 70000; i++) v2->setChar(i, i);
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_withFloatArrayVectorToPartitionTableRangeTypeMorethan65535) {
  ...
  v1->setInt(1, 100);
  v1->setInt(2, 9999);
  for(unsigned int i = 0; i < 70000; i++) v2->setFloat(i, i);
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_withDateArrayVectorToPartitionTableRangeTypeMorethan65535) {
  ...
  v1->setInt(1, 100);
  v1->setInt(2, 9999);
  for(unsigned int i = 0; i < 70000; i++) v2->set(i, dolphindb::Util::createDate(i));
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_withMonthArrayVectorToPartitionTableRangeTypeMorethan65535) {
  ...
  v1->setInt(1, 100);
  v1->setInt(2, 9999);
  for(unsigned int i = 0; i < 70000; i++) v2->set(i, dolphindb::Util::createMonth(i));
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_withTimeArrayVectorToPartitionTableRangeTypeMorethan65535) {
  ...
  v1->setInt(1, 100);
  v1->setInt(2, 9999);
  for(unsigned int i = 0; i < 70000; i++) v2->set(i, dolphindb::Util::createTime(i));
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_withSecondArrayVectorToPartitionTableRangeTypeMorethan65535) {
  ...
  v1->setInt(1, 100);
  v1->setInt(2, 9999);
  for(unsigned int i = 0; i < 70000; i++) v2->set(i, dolphindb::Util::createSecond(i));
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_withDatehourArrayVectorToPartitionTableRangeTypeMorethan65535) {
  ...
  v1->setInt(1, 100);
  v1->setInt(2, 9999);
  for(unsigned int i = 0; i < 70000; i++) v2->set(i, dolphindb::Util::createDateHour(i));
...
TEST_F(AutoFitTableappenderTest, test_AutoFitTableAppender_withUuidArrayVectorToPartitionTableRangeTypeMorethan65535) {
  ...
  v1->setInt(1, 100);
  v1->setInt(2, 9999);
  for(unsigned int i = 0; i < 70000; i++) v2->setString(i, "5d212a78-cc48-e3b1-4235-b4d91473ee87");
...
class AFTA_append_null : public AutoFitTableappenderTest, public testing::WithParamInterface<std::tuple<std::string, dolphindb::DATA_TYPE>> {
 public:
  static std::vector<std::tuple<std::string, dolphindb::DATA_TYPE>> data_prepare() {
    std::vector<std::string> testTypes = {"BOOL",
                                          "CHAR",
                                          "SHORT",
                                          "INT",
                                          "LONG",
                                          ...
                                          "DECIMAL64(15)[]",
                                          "DECIMAL128(25)[]"};
    std::vector<dolphindb::DATA_TYPE> dataTypes = {dolphindb::DT_BOOL,
                                                   dolphindb::DT_CHAR,
                                                   dolphindb::DT_SHORT,
                                                   dolphindb::DT_INT,
                                                   dolphindb::DT_LONG,
                                                   ...
                                                   dolphindb::DT_MINUTE,
                                                   dolphindb::DT_SECOND,
                                                   dolphindb::DT_DATETIME,
                                                   dolphindb::DT_TIMESTAMP,
                                                   dolphindb::DT_NANOTIME,
                                                   dolphindb::DT_NANOTIMESTAMP,
                                                   dolphindb::DT_DATEHOUR,
                                                   dolphindb::DT_FLOAT,
                                                   dolphindb::DT_DOUBLE,
                                                   dolphindb::DT_STRING,
                                                   dolphindb::DT_SYMBOL,
                                                   ...
                                                   dolphindb::DT_UUID,
                                                   dolphindb::DT_INT128,
                                                   dolphindb::DT_DECIMAL32,
                                                   dolphindb::DT_DECIMAL64,
                                                   dolphindb::DT_DECIMAL128,
                                                   dolphindb::DT_BOOL_ARRAY,
                                                   dolphindb::DT_CHAR_ARRAY,
                                                   dolphindb::DT_SHORT_ARRAY,
                                                   dolphindb::DT_INT_ARRAY,
                                                   dolphindb::DT_LONG_ARRAY,
                                                   dolphindb::DT_DATE_ARRAY,
                                                   dolphindb::DT_MONTH_ARRAY,
                                                   dolphindb::DT_TIME_ARRAY,
                                                   dolphindb::DT_MINUTE_ARRAY,
                                                   dolphindb::DT_SECOND_ARRAY,
                                                   dolphindb::DT_DATETIME_ARRAY,
                                                   dolphindb::DT_TIMESTAMP_ARRAY,
                                                   dolphindb::DT_NANOTIME_ARRAY,
                                                   dolphindb::DT_NANOTIMESTAMP_ARRAY,
                                                   dolphindb::DT_DATEHOUR_ARRAY,
                                                   dolphindb::DT_FLOAT_ARRAY,
                                                   dolphindb::DT_DOUBLE_ARRAY,
                                                   dolphindb::DT_IP_ARRAY,
                                                   dolphindb::DT_UUID_ARRAY,
                                                   dolphindb::DT_INT128_ARRAY,
                                                   dolphindb::DT_DECIMAL32_ARRAY,
                                                   dolphindb::DT_DECIMAL64_ARRAY,
                                                   dolphindb::DT_DECIMAL128_ARRAY};
    std::vector<std::tuple<std::string, dolphindb::DATA_TYPE>> data;
    for(auto i = 0; i < testTypes.size(); i++) data.push_back(make_tuple(testTypes[i], dataTypes[i]));
    return data;
  }
};
