...
TEST_F(ConcurrentTest, test_HightlyConcurrentConn_insertToPartitionedTable) {
  ...
  if(createTable(conn, db, tab)) std::cout << "create database and partitionedtable successfully" << std::endl;
...
TEST_F(ConcurrentTest, test_HightlyConcurrentConn_insertToinMemoryTable) {
  ...
  if(createTable(conn, "", tab)) std::cout << "create in-memory table successfully" << std::endl;
  std::vector<std::thread> th(bf_num);
  int sum = 0;
  for(int i = 0; i < bf_num; i++) {
...
TEST_F(ConcurrentTest, test_threadedClient_multi_client_subscribe_concurrent) {
  ...
  std::string replayScript = "n = " + std::to_string(test_rows) +
            ...
            replay(inputTables=table1_STCT, outputTables=`" +
                             tab + ", dateColumn=`timestampv, timeColumn=`timestampv)";
  ...
  for(int i = 0; i < subNum; i++) {
    ...
    conn.run("share table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]) as " + tab + "_" +
             std::to_string(i));
  ...
  do {
    flag = conn.run(
                   "res_rows = array(INT);\
            for (i in 0:" +
                   std::to_string(subNum) + "){res_rows.append!(objByName('" + tab +
                   "_'+string(i)).rows())};\
            all(res_rows == " +
  ...
  ASSERT_TRUE(conn.run("tableRes = array(BOOL);\
            for (i in 0:" +
                       std::to_string(subNum) + "){tableRes.append!(each(eqObj, objByName('" + tab + "_'+string(i)).values(), " + tab +
                       ".values()))};\
            all(tableRes);")
                  ->getBool());
}
