...
class StreamingUDPThreadedClientTester : public testing::Test {
 public:
  ...
  static void TearDownTestSuite() { connSp->close(); }
...
static void createSharedTableAndReplay(const std::string& st, int rows) {
  std::string script =
      "\
            share streamTable(100:0, `datetimev`timestampv`sym`price1`price2,[DATETIME,TIMESTAMP,SYMBOL,DOUBLE,DOUBLE]) as " +
      st +
      ";\n\
            go;\n\
  ...
  std::string replayScript = "n = " + std::to_string(rows) +
                             ";table1_UDPCT = table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]);\
            share table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]) as " +
                             st +
                             "_res_UDPCT;go;\
            tableInsert(table1_UDPCT, 2012.01.01T01:21:23 + 1..n, 2018.12.01T01:21:23.000 + 1..n, take(`a`b`c,n), rand(100,n)+rand(1.0, n), rand(100,n)+rand(1.0, n));share table1_UDPCT as " +
                             st +
                             "_ex_UDPCT;\
...
static void createSharedTableAndReplay_withAllDataType(const std::string& st) {
  ...
  std::string replayScript =
      "colName =  "
      "`ind`cbool`cchar`cshort`cint`clong`cdate`cmonth`ctime`cminute`csecond`cdatetime`ctimestamp`cnanotime`"
      "cnanotimestamp`cfloat`cdouble`csymbol`cstring`cblob`cipaddr`cuuid`cint128`cdecimal32`cdecimal64`cdecimal128;"
      "colType = [INT, BOOL, CHAR, SHORT, INT,LONG, DATE, MONTH, TIME, MINUTE, SECOND, DATETIME, TIMESTAMP, NANOTIME, "
      "NANOTIMESTAMP, FLOAT, DOUBLE, SYMBOL, STRING, BLOB, IPADDR, UUID, INT128, DECIMAL32(" +
      std::to_string(scale32) + "), DECIMAL64(" + std::to_string(scale64) + "), DECIMAL128(" + std::to_string(scale128) +
      ")];"
      "share streamTable(100:0,colName, colType) as " +
      st +
      ";go;"
      "setStreamTableFilterColumn(" +
      st +
      ...
      "row_num = 1000;"
      "share table(1:0, colName, colType) as " +
      st +
      "_res_UDPCT;go;"
      "col1 = rand(2 ,row_num);col2 = rand(256 ,row_num);col3 = rand(-row_num..row_num ,row_num);col4 = "
      "rand(-row_num..row_num ,row_num);col5 = rand(-row_num..row_num ,row_num);col6 = rand(0..row_num ,row_num);col7 "
      "= rand(0..row_num ,row_num);"
      "col8 = rand(0..row_num ,row_num);col9 = rand(0..row_num ,row_num);col10 = rand(0..row_num ,row_num);col11 = "
      "rand(0..row_num ,row_num);col12 = rand(0..row_num ,row_num);col13 = rand(0..row_num ,row_num);col14= "
      "rand(0..row_num ,row_num);"
      "col15 = rand(round(row_num,2) ,row_num);col16 = rand(round(row_num,2) ,row_num);col17 "
      "=rand(`a`s`sym`d`zdg`f`y`ddvb,row_num);col18 =rand(`a`s`sym`d`zdg`f`y`ddvb,row_num);col19 "
      "=rand(`a`s`sym`d`zdg`f`y`ddvb,row_num);col20 = take(ipaddr(\"192.168.1.13\"),row_num);"
      "col21 = take(uuid(\"5d212a78-cc48-e3b1-4235-b4d91473ee87\"),row_num);col22 = "
      "take(int128(\"e1671797c52e15f763380b45e841ec32\"),row_num);col23=rand((-1000..1000)/1000.0000$DECIMAL32(" +
      std::to_string(scale32) + "),row_num);col24=rand((-1000..1000)/1000.0000$DECIMAL64(" + std::to_string(scale64) +
      "),row_num);col25=rand((-1000..1000)/1000.0000$DECIMAL128(" + std::to_string(scale128) +
      "),row_num);go;"
      "for (i in "
      "0..(row_num-1)){tableInsert(table1_UDPCT,i,col1[i],col2[i],col3[i],col4[i],col5[i],col6[i],col7[i],col8[i],col9["
      "i],col10[i],col11[i],col12[i],col13[i],col14[i],col15[i],col16[i],col17[i],col18[i],col19[i],col20[i],col21[i],"
      "col22[i],col23[i],col24[i],col25[i])};"
      "share table1_UDPCT as " +
      st +
      "_ex_UDPCT;go;"
      "replay(inputTables=table1_UDPCT, outputTables=`" +
...
static void createSharedTableAndReplay_withArrayVector(const std::string& st) {
  std::string replayScript =
      "colName = "
      "`ts`cbool`cchar`cshort`cint`clong`cdate`cmonth`ctime`cminute`csecond`cdatetime`ctimestamp`cnanotime`"
      "cnanotimestamp`cdatehour`cfloat`cdouble`cipaddr`cuuid`cint128`cdecimal32`cdecimal64`cdecimal128;"
      "colType = [TIMESTAMP, BOOL[], CHAR[], SHORT[], INT[],LONG[], DATE[], MONTH[], TIME[], MINUTE[], SECOND[], "
      "DATETIME[], TIMESTAMP[], NANOTIME[], NANOTIMESTAMP[], DATEHOUR[], FLOAT[], DOUBLE[], IPADDR[], UUID[], "
      "INT128[],DECIMAL32(6)[],DECIMAL64(16)[],DECIMAL128(26)[]];"
      "share streamTable(1:0,colName, colType) as " +
      st +
      ";go;"
      "share table(1:0, colName, colType) as " +
      st +
      ...
      "cbool= arrayVector(ind, bool(take(0 1 ,row_num)));cchar = arrayVector(ind, char(take(256 ,row_num)));cshort = "
      "arrayVector(ind, short(take(-10000..10000 ,row_num)));cint = arrayVector(ind, int(take(-10000..10000 "
      ",row_num)));"
      "clong = arrayVector(ind, long(take(-10000..10000 ,row_num)));cdate = arrayVector(ind, date(take(10000 "
      ",row_num)));cmonth = arrayVector(ind, month(take(23640..25000 ,row_num)));ctime = arrayVector(ind, "
      "time(take(10000 ,row_num)));"
      "cminute = arrayVector(ind, minute(take(100 ,row_num)));csecond = arrayVector(ind, second(take(100 "
      ",row_num)));cdatetime = arrayVector(ind, datetime(take(10000 ,row_num)));ctimestamp = arrayVector(ind, "
      "timestamp(take(10000 ,row_num)));"
      "cnanotime = arrayVector(ind, nanotime(take(10000 ,row_num)));cnanotimestamp = arrayVector(ind, "
      "nanotimestamp(take(10000 ,row_num)));cdatehour = arrayVector(ind, datehour(take(10000 ,row_num)));cfloat = "
      "arrayVector(ind, float(rand(10000.0000,row_num)));cdouble = arrayVector(ind, rand(10000.0000, row_num));"
      ...
      "cdecimal32 = array(DECIMAL32(6)[], 0, 10).append!(decimal32([1..50, [], rand(100.000000, 10), rand(1..100, 10), "
      "take(00i, 3)], 6));"
      "cdecimal64 = array(DECIMAL64(16)[], 0, 10).append!(decimal64([1..50, [], rand(100.000000, 10), rand(1..100, "
      "10), take(00i, 3)], 16));"
      "cdecimal128 = array(DECIMAL128(26)[], 0, 10).append!(decimal128([1..50, [], rand(100.000000, 10), rand(1..100, "
      "10), take(00i, 3)], 26));"
      "for(i in 1..(row_num-5)){"
      "    cbool.append!([bool(take(0 1 ,50))]);"
      "    cchar.append!([char(take(256 ,50))]);cshort.append!([short(take(-10000..10000 "
      ",50))]);cint.append!([int(take(-10000..10000 ,50))]);"
      "    clong.append!([long(take(-10000..10000 ,50))]);cdate.append!([date(take(10000 "
      ",50))]);cmonth.append!([month(take(23640..25000 ,50))]);"
      "    ctime.append!([time(take(10000 ,50))]);cminute.append!([minute(take(100 "
      ",50))]);csecond.append!([second(take(100 ,50))]);"
      ...
      "table1_UDPCT=table(ts,cbool,cchar,cshort,cint,clong,cdate,cmonth,ctime,cminute,csecond,cdatetime,ctimestamp,"
      "cnanotime,cnanotimestamp,cdatehour,cfloat,cdouble,cipaddr,cuuid,cint128,cdecimal32,cdecimal64,cdecimal128);"
      "share table1_UDPCT as " +
      st +
      "_ex_UDPCT;go;"
      "replay(inputTables=table1_UDPCT, outputTables=`" +
...
dolphindb::TableSP UDPmsgToTable(dolphindb::VectorSP tupleVal) {
  ...
  for(auto i = 0; i < col_count; i++) {
    ...
    dolphindb::DATA_TYPE type =
        form == dolphindb::DF_VECTOR && _t < dolphindb::ARRAY_TYPE_BASE ? static_cast<dolphindb::DATA_TYPE>(_t + dolphindb::ARRAY_TYPE_BASE) : _t;
...
TEST_F(StreamingUDPThreadedClientTester, tableNotExist_should_serial) {
  ...
  ASSERT_ANY_THROW(
      client.subscribe(HOST, PORT, [](dolphindb::Message msg) {}, "ajwdhwjkae", DEFAULT_ACTION_NAME, 0, true, nullptr, false, false, USER, PASSWD));
}
...
TEST_F(StreamingUDPThreadedClientTester, subscribe_host_error_should_serial) {
  ...
  dolphindb::ThreadedClient client(scfg);
  ASSERT_ANY_THROW(
      client.subscribe("abcd", PORT, [](dolphindb::Message msg) {}, st, DEFAULT_ACTION_NAME, 0, true, nullptr, false, false, USER, PASSWD));
}
...
TEST_F(StreamingUDPThreadedClientTester, subscribe_username_error_should_serial) {
  ...
  ASSERT_ANY_THROW(
      client.subscribe(HOST, PORT, [](dolphindb::Message msg) {}, st, DEFAULT_ACTION_NAME, 0, true, nullptr, false, false, "sjkdwae", "123456"));
}
...
TEST_F(StreamingUDPThreadedClientTester, subscribe_password_error_should_serial) {
  ...
  ASSERT_ANY_THROW(
      client.subscribe(HOST, PORT, [](dolphindb::Message msg) {}, st, DEFAULT_ACTION_NAME, 0, true, nullptr, false, false, USER, "111111"));
}
...
TEST_F(StreamingUDPThreadedClientTester, subscribe_offset_negative_should_serial) {
  ...
  ASSERT_TRUE(connSp
                  ->run("ex = exec * from extraData order by timestampv;"
                        "res = exec * from " +
                        st +
                        "_res_UDPCT order by timestampv;"
...
TEST_F(StreamingUDPThreadedClientTester, subscribe_offset_gt0_should_serial) {
  ...
  ASSERT_TRUE(connSp
                  ->run("ex = (exec * from " + st +
                        "_ex_UDPCT order by datetimev)[10:];"
                        "res = exec * from " +
                        st +
                        "_res_UDPCT order by timestampv;"
                        "all(each(eqObj, ex.values(), res.values()))")
...
TEST_F(StreamingUDPThreadedClientTester, subscribe_filter_should_serial) {
  ...
  ASSERT_TRUE(connSp
                  ->run("ex = exec * from " + st +
                        "_ex_UDPCT where sym = `a order by timestampv;"
                        "res = exec * from " +
                        st +
                        "_res_UDPCT order by timestampv;"
                        "all(each(eqObj, ex.values(), res.values()))")
...
TEST_F(StreamingUDPThreadedClientTester, not_unsubscribe_after_subscribe_should_serial) {
  ...
  connSp->run(
      "\
        info = exec * from getStreamingStat().udpPubTables where tableName=`" +
      st +
      ";\
        ip1 = (exec channel.split(':')[0] from info)[0];\
        port1 = (exec channel.split(':')[1] from info)[0];\
        stopPublishTable(ip1, port1, `" +
      st + ",`" + DEFAULT_ACTION_NAME + ",,true)");
  dolphindb::Util::sleep(1000);
  ASSERT_FALSE(connSp->run("`" + st + " in (exec tableName from getStreamingStat().udpPubTables)")->getBool());
...
TEST_F(StreamingUDPThreadedClientTester, subscribe_multithread_should_serial) {
  ...
  std::vector<std::string> res_tabs = {"res_UDPCT0", "res_UDPCT1", "res_UDPCT2", "res_UDPCT3", "res_UDPCT4",
                                       "res_UDPCT5", "res_UDPCT6", "res_UDPCT7", "res_UDPCT8", "res_UDPCT9"};
...
TEST_F(StreamingUDPThreadedClientTester, subscribe_with_alldataTypes_should_serial) {
  ...
  ASSERT_TRUE(connSp
                  ->run("ex = exec * from " + st +
                        "_ex_UDPCT order by ind;"
                        "res = exec * from " +
                        st +
                        "_res_UDPCT order by ind;"
                        "all(each(eqObj, ex.values(), res.values()))")
                  ->getBool());
  ASSERT_FALSE(connSp->run("`" + st + " in (exec tableName from getStreamingStat().udpPubTables)")->getBool());
}

class StreamingUDPThreadedClientTester_realtime : public testing::Test,
                                                  public ::testing::WithParamInterface<std::pair<dolphindb::DATA_TYPE, std::string>> {
 public:
 ...
 public:
  static std::vector<std::pair<dolphindb::DATA_TYPE, std::string>> getAVData() {
    return {
        {dolphindb::DT_BOOL_ARRAY, "take([true false], row_num)"},
        {dolphindb::DT_BOOL_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_CHAR_ARRAY, "take([10c 23c], row_num)"},
        {dolphindb::DT_CHAR_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_SHORT_ARRAY, "take([10s 23s], row_num)"},
        {dolphindb::DT_SHORT_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_INT_ARRAY, "take([10 23], row_num)"},
        {dolphindb::DT_INT_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_LONG_ARRAY, "take([10l 23l], row_num)"},
        {dolphindb::DT_LONG_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_DATE_ARRAY, "take([2021.01.01 2021.01.02], row_num)"},
        {dolphindb::DT_DATE_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_MONTH_ARRAY, "take([2021.01M 2021.02M], row_num)"},
        {dolphindb::DT_MONTH_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_TIME_ARRAY, "take([10:10:10 11:11:11], row_num)"},
        {dolphindb::DT_TIME_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_MINUTE_ARRAY, "take([10:10m 11:11m], row_num)"},
        {dolphindb::DT_MINUTE_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_SECOND_ARRAY, "take([10:10:10 11:11:11], row_num)"},
        {dolphindb::DT_SECOND_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_DATETIME_ARRAY, "take([2021.01.01T10:10:10 2021.01.02T11:11:11], row_num)"},
        {dolphindb::DT_DATETIME_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_TIMESTAMP_ARRAY, "take([2021.01.01T10:10:10.000 2021.01.02T11:11:11.000], row_num)"},
        {dolphindb::DT_TIMESTAMP_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_NANOTIME_ARRAY, "take([10:10:10.000000000 11:11:11.000000000], row_num)"},
        {dolphindb::DT_NANOTIME_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_NANOTIMESTAMP_ARRAY, "take([2021.01.01T10:10:10.000000000 2021.01.02T11:11:11.000000000], row_num)"},
        {dolphindb::DT_NANOTIMESTAMP_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_DATEHOUR_ARRAY, "take([datehour(1000 2000)], row_num)"},
        {dolphindb::DT_DATEHOUR_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_FLOAT_ARRAY, "take([10.00f 23.00f], row_num)"},
        {dolphindb::DT_FLOAT_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_DOUBLE_ARRAY, "take([10.314 23.445], row_num)"},
        {dolphindb::DT_DOUBLE_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_IP_ARRAY, "take([rand(ipaddr(), 2)], row_num)"},
        {dolphindb::DT_IP_ARRAY, "take([[ipaddr()]], row_num)"},
        {dolphindb::DT_UUID_ARRAY, "take([rand(uuid(), 2)], row_num)"},
        {dolphindb::DT_UUID_ARRAY, "take([[uuid()]], row_num)"},
        {
            dolphindb::DT_INT128_ARRAY,
            "take([rand(int128(), 2)], row_num)",
        },
        {dolphindb::DT_INT128_ARRAY, "take([[int128()]], row_num)"},
        {dolphindb::DT_DECIMAL32_ARRAY, "take([decimal32(rand('-1.123''''2.23468965412', 2), 8)], row_num)"},
        {dolphindb::DT_DECIMAL32_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_DECIMAL64_ARRAY, "take([decimal64(rand('-1.123''''2.123', 2), 15)], row_num)"},
        {dolphindb::DT_DECIMAL64_ARRAY, "take([[00i]], row_num)"},
        {dolphindb::DT_DECIMAL128_ARRAY, "take([decimal128(rand('-1.123''''2.123', 2), 25)], row_num)"},
        {dolphindb::DT_DECIMAL128_ARRAY, "take([[00i]], row_num)"},
    };
  };
  static std::vector<std::pair<dolphindb::DATA_TYPE, std::string>> getData() {
    return {
        {dolphindb::DT_BOOL, "rand(true false, row_num)"},
        {dolphindb::DT_BOOL, "take(bool(), row_num)"},
        {dolphindb::DT_CHAR, "rand(127c, row_num)"},
        {dolphindb::DT_CHAR, "take(char(), row_num)"},
        {dolphindb::DT_SHORT, "rand(32767h, row_num)"},
        {dolphindb::DT_SHORT, "take(short(), row_num)"},
        {dolphindb::DT_INT, "rand(2147483647, row_num)"},
        {dolphindb::DT_INT, "take(int(), row_num)"},
        {dolphindb::DT_LONG, "rand(1000l, row_num)"},
        {dolphindb::DT_LONG, "take(long(), row_num)"},
        {dolphindb::DT_DATE, "rand(2019.01.01, row_num)"},
        {dolphindb::DT_DATE, "take(date(), row_num)"},
        {dolphindb::DT_MONTH, "rand(2019.01M, row_num)"},
        {dolphindb::DT_MONTH, "take(month(), row_num)"},
        {dolphindb::DT_TIME, "rand(12:00:00.123, row_num)"},
        {dolphindb::DT_TIME, "take(time(), row_num)"},
        {dolphindb::DT_MINUTE, "rand(12:00m, row_num)"},
        {dolphindb::DT_MINUTE, "take(minute(), row_num)"},
        {dolphindb::DT_SECOND, "rand(12:00:00, row_num)"},
        {dolphindb::DT_SECOND, "take(second(), row_num)"},
        {dolphindb::DT_DATETIME, "rand(2019.01.01 12:00:00, row_num)"},
        {dolphindb::DT_DATETIME, "take(datetime(), row_num)"},
        {dolphindb::DT_DATEHOUR, "rand(datehour(1000), row_num)"},
        {dolphindb::DT_DATETIME, "take(datehour(), row_num)"},
        {dolphindb::DT_TIMESTAMP, "rand(2019.01.01 12:00:00.123, row_num)"},
        {dolphindb::DT_TIMESTAMP, "take(timestamp(), row_num)"},
        {dolphindb::DT_NANOTIME, "rand(12:00:00.123456789, row_num)"},
        {dolphindb::DT_NANOTIME, "take(nanotime(), row_num)"},
        {dolphindb::DT_NANOTIMESTAMP, "rand(2019.01.01 12:00:00.123456789, row_num)"},
        {dolphindb::DT_NANOTIMESTAMP, "take(nanotimestamp(), row_num)"},
        {dolphindb::DT_DATEHOUR, "rand(datehour(100), row_num)"},
        {dolphindb::DT_DATEHOUR, "take(datehour(), row_num)"},
        {dolphindb::DT_FLOAT, "rand(10.00f, row_num)"},
        {dolphindb::DT_FLOAT, "take(float(), row_num)"},
        {dolphindb::DT_DOUBLE, "rand(10.00, row_num)"},
        {dolphindb::DT_DOUBLE, "take(double(), row_num)"},
        {dolphindb::DT_IP, "rand(ipaddr(), row_num)"},
        {dolphindb::DT_IP, "take(ipaddr(), row_num)"},
        {dolphindb::DT_UUID, "rand(uuid(), row_num)"},
        {dolphindb::DT_UUID, "take(uuid(), row_num)"},
        {dolphindb::DT_INT128, "rand(int128(), row_num)"},
        {dolphindb::DT_INT128, "take(int128(), row_num)"},
        {dolphindb::DT_DECIMAL32, "decimal32(rand('-1.123''''2.23468965412', row_num), 8)"},
        {dolphindb::DT_DECIMAL32, "take(decimal32(NULL, 8), row_num)"},
        {dolphindb::DT_DECIMAL64, "decimal64(rand('-1.123''''2.123123123123123123', row_num), 15)"},
        {dolphindb::DT_DECIMAL64, "take(decimal64(NULL, 15), row_num)"},
        {dolphindb::DT_DECIMAL128, "decimal128(rand('-1.123''''2.123', row_num), 25)"},
        {dolphindb::DT_DECIMAL128, "take(decimal128(NULL, 25), row_num)"},
        {dolphindb::DT_STRING, "rand(`str1`str2, row_num)"},
        {dolphindb::DT_STRING, "take(string(), row_num)"},
        {dolphindb::DT_SYMBOL, "rand(symbol(`sym1`sym2), row_num)"},
        {dolphindb::DT_SYMBOL, "symbol(take(string(), row_num))"},
        {dolphindb::DT_BLOB, "rand(blob(`b1`b2`b3), row_num)"},
        {dolphindb::DT_BLOB, "take(blob(''), row_num)"},
    };
  };
  ...
  void insertData(dolphindb::DBConnection& conn, const std::string& name, const std::string& colScript) {
    std::string s =
        ...
        colScript +
        ";"
        "for (i in 0..(row_num-1)){insert into " +
        name + " values([col0[i]], [col1[i]]);};";
    conn.run(s);
  };
};
...
INSTANTIATE_TEST_SUITE_P(basicTypes, StreamingUDPThreadedClientTester_realtime,
                         testing::ValuesIn(StreamingUDPThreadedClientTester_realtime::getData()));
INSTANTIATE_TEST_SUITE_P(arrayVectorTypes, StreamingUDPThreadedClientTester_realtime,
                         testing::ValuesIn(StreamingUDPThreadedClientTester_realtime::getAVData()));
TEST_P(StreamingUDPThreadedClientTester_realtime, test_realtime_should_serial) {
  ...
  std::thread th = std::thread([&]() { insertData(*connSp, st, dataScript); });
  auto thread = client.subscribe(HOST, PORT, handler, st, DEFAULT_ACTION_NAME, 0, true, nullptr, false, false, USER, PASSWD);
  th.join();
  notify.wait();
  dolphindb::Util::sleep(1000);
  ...
  client.unsubscribe(HOST, PORT, st, DEFAULT_ACTION_NAME);
  ASSERT_TRUE(connSp
                  ->run("re = select * from " + st + "_res_UDPCT order by ts;\
                        ex = select * from " +
                        st + " order by ts;\
                        all(each(eqObj, re.values(), ex.values()))")
                  ->getBool());
  ASSERT_FALSE(connSp->run("`" + st + " in (exec tableName from getStreamingStat().udpPubTables)")->getBool());
}
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_onehandler_should_serial) {
  ...
  dolphindb::Util::sleep(1000);
  ASSERT_TRUE(connSp
                  ->run("re = select * from " + st + "_res_UDPCT order by datetimev;\
                            ex = select * from " +
                        st + "_ex_UDPCT order by datetimev;\
                            all(each(eqObj, re.values(), ex.values()))")
                  ->getBool());
  dolphindb::Util::sleep(1000);
  ASSERT_TRUE(connSp->run("(exec count(*) from getStreamingStat().udpPubTables where tableName =`" + st + ") ==0")->getBool());
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribeUnsubscribe_notExistTable_should_serial) {
  dolphindb::ThreadedClient threadedClient = dolphindb::ThreadedClient(scfg);
  ASSERT_ANY_THROW(
      threadedClient.subscribe(HOST, PORT, [](dolphindb::Message msg) {}, "st_notExist", "actionTest", 0, true, nullptr, false, false, USER, PASSWD));
}
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_onehandler_tableNameNull_should_serial) {
  ...
  ASSERT_ANY_THROW(auto thread =
                       threadedClient.subscribe(HOST, PORT, onehandler, "", "cppStreamingAPI", 0, true, nullptr, false, false, USER, PASSWD));
}
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_batchhandler_tableNameNull_should_serial) {
  ...
  dolphindb::ThreadedClient threadedClient = dolphindb::ThreadedClient(scfg);
  ASSERT_ANY_THROW(
      auto thread = threadedClient.subscribe(HOST, PORT, batchhandler, "", "cppStreamingAPI", 0, false, nullptr, 1, 1.0, false, "admin", "123456"));
}
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_onehandler_offsetInMiddle_should_serial) {
  ...
  ASSERT_TRUE(connSp
                  ->run("re = select * from " + st + "_res_UDPCT order by datetimev;\
                            ex = select * from " +
                        st + "_ex_UDPCT order by datetimev limit 5, 5;\
                            all(each(eqObj, re.values(), ex.values()))")
                  ->getBool());
  dolphindb::Util::sleep(1000);
  ASSERT_TRUE(connSp->run("(exec count(*) from getStreamingStat().udpPubTables where tableName =`" + st + ") ==0")->getBool());
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_onehandler_msgAsTable_should_serial) {
  ...
  ASSERT_TRUE(threadedClient.isExit());
  ASSERT_TRUE(connSp
                  ->run("re = select * from " + case_ + "_res_UDPCT order by datetimev;\
                            ex = select * from " +
                        case_ + "_ex_UDPCT order by datetimev;\
                            all(each(eqObj, re.values(), ex.values()))")
                  ->getBool());
  dolphindb::Util::sleep(1000);
  ASSERT_TRUE(connSp->run("(exec count(*) from getStreamingStat().udpPubTables where tableName =`" + st + ") ==0")->getBool());
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_batchhandler_allowExists_should_serial) {
  ...
  auto batchhandler = [&](std::vector<dolphindb::Message> msgs) {
    ...
    for(auto& msg : msgs) {
      msg_total += 1;
      // std::cout  << msg->getString() << std::endl;
      if(msg.getOffset() == 999) notify.set();
    }
  };
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_resub_true_should_serial) {
  ...
  auto onehandler = [&](dolphindb::Message msg) { msgs.push_back(msg); };
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_onehandler_portNull_should_serial) {
  ...
  auto onehandler = [&](dolphindb::Message msg) { msg_total += 1; };
...
TEST_F(StreamingUDPThreadedClientTester, tes_onehandler_subscribe_twice_should_serial) {
  ...
  ASSERT_ANY_THROW(auto thread2 =
                       threadedClient.subscribe(HOST, PORT, onehandler, st, "mutiSchemaOne", 0, true, nullptr, false, false, USER, PASSWD));
  notify.wait();
  std::cout << "total size: " << msg_total << std::endl;
  threadedClient.unsubscribe(HOST, PORT, st, "mutiSchemaOne");
  dolphindb::Util::sleep(1000);
...
TEST_F(StreamingUDPThreadedClientTester, test_batchhandler_subscribe_twice_should_serial) {
  ...
  ASSERT_ANY_THROW(
      auto thread2 = threadedClient.subscribe(HOST, PORT, batchhandler, st, "mutiSchemaBatch", 0, true, nullptr, false, true, 1, 1.0, USER, PASSWD));
  notify.wait();
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_onehandler_withAllDataType_should_serial) {
  ...
  ASSERT_TRUE(connSp
                  ->run("re = select * from " + case_ + "_res_UDPCT order by ind;\
                            ex = select * from " +
                        case_ + "_ex_UDPCT order by ind;\
                            all(each(eqObj, re.values(), ex.values()))")
                  ->getBool());
  dolphindb::Util::sleep(1000);
  ASSERT_TRUE(connSp->run("(exec count(*) from getStreamingStat().udpPubTables where tableName =`" + st + ") ==0")->getBool());
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_onehandler_arrayVector_should_serial) {
  ...
  ASSERT_TRUE(connSp
                  ->run("re = select * from " + case_ + "_res_UDPCT order by ts;\
                            ex = select * from " +
                        case_ + "_ex_UDPCT order by ts;\
                            all(each(eqObj, re.values(), ex.values()))")
                  ->getBool());
  dolphindb::Util::sleep(1000);
  ASSERT_TRUE(connSp->run("(exec count(*) from getStreamingStat().udpPubTables where tableName =`" + st + ") ==0")->getBool());
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_hugetable_onehandler_should_serial) {
  ...
  auto onehandler = [&](dolphindb::Message msg) {
    dolphindb::LockGuard<dolphindb::Mutex> lock(&mutex);
    msg_total += 1;
    if(msg_total % 100000 == 0) std::cout << "now subscribed rows: " << msg_total << std::endl;
    // handle msg
...
TEST_F(StreamingUDPThreadedClientTester, test_subscribe_hugetable_batchhandler_msgAsTable_batchSize_throttle_should_serial) {
  ...
  dolphindb::ThreadedClient threadedClient = dolphindb::ThreadedClient(scfg);
  auto thread = threadedClient.subscribe(HOST, PORT, batchhandler, st, "mutiSchemaBatch", 0, true, nullptr, false, test_batchSize, test_throttle,
                                         true, USER, PASSWD);
  notify.wait();
...
TEST_F(StreamingUDPThreadedClientTester, test_unsubscribe_in_handler_should_serial) {
  ...
  // notify.wait();
  ASSERT_EQ(msg_total,
            1);  // only one message is in handler, because unsubscribe is called before the first message received
  dolphindb::Util::sleep(1000);
  ASSERT_TRUE(connSp->run("(exec count(*) from getStreamingStat().udpPubTables where tableName =`" + st + ") ==0")->getBool());
}
...
TEST_F(StreamingUDPThreadedClientTester, test_resub_true_with_resubscribeTimeout_should_serial) {
  ...
  auto t = client1.subscribe(
      HOST, PORT, [](dolphindb::Message msg) {}, st, DEFAULT_ACTION_NAME, 0, true, nullptr, false, false, USER, PASSWD, nullptr, {}, 100, false,
      resubscribeTimeout);
