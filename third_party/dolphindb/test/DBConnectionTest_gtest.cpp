...
void StopCurNode(std::string cur_node) {
  ...
  conn1.run(
      "startDataNode(exec name from getClusterPerf() where mode !=1 and state != 1);go;"
...
TEST_F(DBConnectionTest, test_connect_withStartupScript) {
  ...
  for(auto i = 0; i < resVec->size(); i++) ASSERT_TRUE(resVec->get(i)->getBool());
...
TEST_F(DBConnectionTest, test_connection_python_script) {
  std::string script1 =
      ...
      "\ttypes=[int,long,short,float,double,char,bool,date,minute,month,second,datetime,timestamp,nanotime,"
      "nanotimestamp,datehour]\n"
...
TEST_F(DBConnectionTest, test_connectionPool_compress) {
  ...
  std::vector<std::string> colName = {"cbool",   "cchar",   "cshort",    "cint",       "cdate",     "cmonth",         "ctime",
                                      "cminute", "csecond", "cdatetime", "ctimestamp", "cnanotime", "cnanotimestamp", "cfloat",
                                      "cdouble", "csymbol", "cstring",   "cipaddr",    "cblob",     "cdecimal32",     "cdecimal64"};
  std::vector<dolphindb::ConstantSP> colVector{boolVector,          charVector,      shortVector,    intVector,      dateVector,      monthVector,
                                               timeVector,          minuteVector,    secondVector,   datetimeVector, timestampVector, nanotimeVector,
                                               nanotimestampVector, floatVector,     doubleVector,   symbolVector,   stringVector,    ipaddrVector,
                                               blobVector,          decimal32Vector, decimal64Vector};
  dolphindb::TableSP table = dolphindb::Util::createTable(colName, colVector);
  std::vector<dolphindb::COMPRESS_METHOD> typeVec{
      dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA,
      dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA,
      dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,
      dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,
      dolphindb::COMPRESS_LZ4};
  ...
  std::string script =
      "colName =  "
      "`cbool`cchar`cshort`cint`cdate`cmonth`ctime`cminute`csecond`cdatetime`ctimestamp`cnanotime`cnanotimestamp`"
      "cfloat`cdouble`csymbol`cstring`cipaddr`cblob`cdecimal32`cdecimal64;\n"
      "colType = [BOOL, CHAR, SHORT, INT, DATE, MONTH, TIME, MINUTE, SECOND, DATETIME, TIMESTAMP, NANOTIME, "
      "NANOTIMESTAMP, FLOAT, DOUBLE, SYMBOL, STRING,IPADDR,BLOB,DECIMAL32(" +
  ...
  for(int i = 0; i < res->size(); i++) ASSERT_TRUE(res->get(i)->getBool());
  pool_demo.shutDown();
}
...
TEST_F(DBConnectionTest, test_DBconnectionPool) {
  ...
  conn.run(srcipt1);
  ASSERT_ANY_THROW(
      pool_demo.run("for(i in 1..100){tableInsert(" + case_ + ",rand(100,1),rand(100,1),rand(100,1));};select * from " + case_ + "", -1));
...
TEST_F(DBConnectionTest, test_DBconnectionPoolwithFetchSize) {
  ...
  std::string s =
      "rows = 20000;t=table(rand(1..100,20000) as c1, rand(`a`b`c`d`e`f`g, 20000) as c2, rand(timestamp(10001..50100), "
      "20000) as c3, rand(100.0000, 20000) as c4, arrayVector(1..20000*3, rand(100.0000, 60000)) as c5);"
      "loadTable('" +
      dbName + "', `" + tab +
      ").append!(t);"
      "go;select * from loadTable('" +
      dbName + "', `" + tab + ")";
  ...
  ASSERT_TRUE(conn.run("ex_t=select * from loadTable(dbpath,`" + tab +
                       ");"
...
TEST_F(DBConnectionTest, test_connection_concurrent_insert_datas) {
  ...
  std::string s =
      "t=table(rand(1..100,10) as c1, rand(`a`b`c`d`e`f`g, 10) as c2, rand(timestamp(10001..10100), 10) as c3);"
      "loadTable('" +
      dbName + "', `" + tab +
      ").append!(t);"
      "go;"
  ...
  for(int i = 0; i < 50; i++) threads.emplace_back(query);
...
TEST_F(DBConnectionTest, test_connectionPool_concurrent_insert_datas) {
  ...
  std::string s =
      "t=table(rand(1..100,100) as c1, rand(`a`b`c`d`e`f`g, 100) as c2, rand(timestamp(10001..10100), 100) as c3);"
      "loadTable('" +
      dbpath + "', `" + tab +
      ").append!(t);"
      "go;"
  ...
  std::vector<std::thread> threads;
  for(int i = 0; i < 5; i++) threads.emplace_back(query);
...
TEST_F(DBConnectionTest, test_connectionPool_condition_variable_run_script) {
  ...
  cv->wait(lk, [id_1, &pool] { return pool.isFinished(id_1); });
  cv->wait(lk, [id_2, &pool] { return pool.isFinished(id_2); });
  cv->wait(lk, [id_3, &pool] { return pool.isFinished(id_3); });
  cv->wait(lk, [id_4, &pool] { return pool.isFinished(id_4); });
  ASSERT_EQ(pool.getData(id_1)->getString(), "1");
...
TEST_F(DBConnectionTest, test_connection_SCRAM) {
  ...
  {  // login with no scram auth user admin
    dolphindb::DBConnection _c = dolphindb::DBConnection(false, false, 7200, false, false, false, true);
    _c.connect(HOST, PORT, USER,
               PASSWD);  // stdout: [debug] [140591328847872] : user 'admin' doesn't support scram authMode.
  }
