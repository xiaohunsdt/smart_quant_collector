...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_value_int) {
  ...
  for(int i = 0; i < colNum; i++) columnVecs.emplace_back(t1->getColumn(i));
  ...
  for(int i = 0; i < 3; i++) ASSERT_EQ(result->getInt(i), 1);
  pool.shutDown();
}
...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_value_date1) {
  ...
  std::string script2;
  script2 +=
      "tmp = table(take(0..9, 1000000) as id, take(\"A\"+string(0..999), 1000000) as sym, take(2012.01.01..2012.01.10, "
      "1000000) as date, 0..999999 as value);";
...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_value_date2) {
  ...
  std::string script2;
  script2 +=
      "tmp = table(take(0..9, 1000000) as id, take(\"A\"+string(0..999), 1000000) as sym, "
      "concatDateTime(take(2012.01.01..2012.01.10, 1000000), 09:30:30) as time, 0..999999 as value);";
...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_value_date3) {
  ...
  script2 +=
      "tmp = table(take(0..9, 1000000) as id, take(\"A\"+string(0..999), 1000000) as sym, "
      "concatDateTime(take(2012.01.01..2012.01.10, 1000000), 09:30:30.125) as time, 0..999999 as value);";
  script2 += "expected = select *  from tmp order by time, id, sym, value;";
  script2 += "re = select * from loadTable(dbPath, `pt) order by time, id, sym, value;";
  script2 += "each(eqObj, re.values(), expected.values()).int();";
  dolphindb::ConstantSP result = conn.run(script2);
...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_value_date4) {
  ...
  script2 +=
      "tmp = table(take(0..9, 1000000) as id, take(\"A\"+string(0..999), 1000000) as sym, "
      "concatDateTime(take(2012.01.01..2012.01.10, 1000000), 09:30:30.000000000) as time, 0..999999 as value);";
  script2 += "expected = select *  from tmp order by time, id, sym, value;";
  script2 += "re = select * from loadTable(dbPath, `pt) order by time, id, sym, value;";
  script2 += "each(eqObj, re.values(), expected.values()).int();";
  dolphindb::ConstantSP result = conn.run(script2);
...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_value_month1) {
  ...
  std::string script2;
  script2 +=
      "tmp = table(take(0..9, 1000000) as id, take(\"A\"+string(0..999), 1000000) as sym, take(temporalAdd(2012.01.10, "
      "0..9, \"M\"), 1000000) as date, 0..999999 as value);";
  script2 += "expected = select *  from tmp order by date, id, sym, value;";
  script2 += "re = select * from loadTable(dbPath, `pt) order by date, id, sym, value;";
  script2 += "each(eqObj, re.values(), expected.values()).int();";
  dolphindb::ConstantSP result = conn.run(script2);
...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_value_month2) {
  ...
  script2 +=
      "tmp = table(take(0..9, 1000000) as id, take(\"A\"+string(0..999), 1000000) as sym, "
      "concatDateTime(take(temporalAdd(2012.01.10, 0..9, \"M\"), 1000000), 09:30:30) as time, 0..999999 as value);";
  script2 += "expected = select *  from tmp order by time, id, sym, value;";
  script2 += "re = select * from loadTable(dbPath, `pt) order by time, id, sym, value;";
  script2 += "each(eqObj, re.values(), expected.values()).int();";
  dolphindb::ConstantSP result = conn.run(script2);
...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_value_month3) {
  ...
  script2 +=
      "tmp = table(take(0..9, 1000000) as id, take(\"A\"+string(0..999), 1000000) as sym, "
      "concatDateTime(take(temporalAdd(2012.01.10, 0..9, \"M\"), 1000000), 09:30:30.125) as time, 0..999999 as value);";
  script2 += "expected = select *  from tmp order by time, id, sym, value;";
  script2 += "re = select * from loadTable(dbPath, `pt) order by time, id, sym, value;";
  script2 += "each(eqObj, re.values(), expected.values()).int();";
  dolphindb::ConstantSP result = conn.run(script2);
  ...
  for(int j = 0; j < colNum; j++) {
    if(j == 3)
      columnVecs[3]->set(rowNum - 1,
                         dolphindb::Util::createInt(rowNum - 1));  // partition-column's value must be not null
    else
...
TEST_P(PTA_append_null, test_append_empty_table) {
  ...
  std::string script1 = "colName = [`ind,`time, `" + colName +
                        "];"
                        "colType = [INT, DATETIME, " +
                        type +
                        "];"
                        "t=table(1:0, colName, colType);"
                        "if(existsDatabase('" +
                        dbName + "')) dropDatabase('" + dbName +
                        "');go;"
                        "db = database('" +
                        dbName +
                        "', VALUE, 1..10,,'TSDB');"
...
TEST_F(PartitionedTableAppenderTest, test_PartitionedTableAppender_SCRAM_user) {
  ...
  script += "db = database(dbPath,COMPO,[db1, db2, db3]);";
  script +=
      "share table(1..100 as id, \"A\"+string(0..99) as sym, take(2012.01.01..2013.01.10, 100) as date, 1..100 as "
      "value) as " +
      case_ + ";";
  script += "pt = db.createPartitionedTable(" + case_ +
            ",`pt,`date`sym`id);"
  ...
  ASSERT_EQ(rows, 100);
  ASSERT_TRUE(conn.run("res = select * from " + case_ + " order by id;ex = select * from loadTable('" + dbName +
                       "', `pt) order by id;all(each(eqObj, res.values(), ex.values()))")
                  ->getBool());
  pool.shutDown();
}