...
class ScalarTest : public testing::Test {
 public:
  ...
  static void TearDownTestSuite() { conn.close(); }
...
TEST_F(ScalarTest, testScalar1) {
  ...
  std::vector<dolphindb::ConstantSP> vals = {boolval,   charval,   intval,      shortval,     longval,     floatval,        doubleval, stringval,
                                             symbolval, blobval,   int128val,   ipval,        uuidval,     dateval,         monthval,  timeval,
                                             minuteval, secondval, datetimeval, timestampval, nanotimeval, nanotimestampval};
  std::vector<std::string> names = {"boolval",     "charval",      "intval",      "shortval",        "longval",   "floatval",
                                    "doubleval",   "stringval",    "symbolval",   "blobval",         "int128val", "ipval",
                                    "uuidval",     "dateval",      "monthval",    "timeval",         "minuteval", "secondval",
                                    "datetimeval", "timestampval", "nanotimeval", "nanotimestampval"};
...
TEST_F(ScalarTest, test_decimal64) {
  ...
  ASSERT_ANY_THROW(dolphindb::ConstantSP decimalval1 =
                       dolphindb::Util::createDecimal64(0, 10000000000000000000.0));  // scale + value's places must less than 20
...
TEST_F(ScalarTest, testScalarFunction) {
  ...
  unsigned char int128[16];
  for(auto i = 0; i < 16; i++) int128[i] = rand() % CHAR_MAX;
  dolphindb::ConstantSP int128val = dolphindb::Util::createConstant(dolphindb::DT_INT128);
  int128val->setBinary(int128, sizeof(dolphindb::Guid));
  ...
  unsigned char uuid[16];
  for(auto i = 0; i < 16; i++) uuid[i] = rand() % CHAR_MAX;
  dolphindb::ConstantSP uuidval = dolphindb::Util::createConstant(dolphindb::DT_UUID);
  uuidval->setBinary(uuid, sizeof(dolphindb::Guid));
  ...
  unsigned char ip[16];
  for(auto i = 0; i < 16; i++) ip[i] = rand() % CHAR_MAX;
  dolphindb::ConstantSP ipval = dolphindb::Util::createConstant(dolphindb::DT_IP);
  ipval->setBinary(ip, sizeof(dolphindb::Guid));
...
TEST_F(ScalarTest, testFunctionCastTemporal_datatime) {
  ...
  ASSERT_EQ(dolphindb::Util::createDateTime(INT_MIN)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(datetime(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(INT_MIN)->castTemporal(dolphindb::DT_MONTH)->getString(),
            conn.run("month(datetime(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(INT_MIN)->castTemporal(dolphindb::DT_TIME)->getString(),
            conn.run("time(datetime(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(INT_MIN)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(datetime(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(INT_MIN)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(datetime(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(INT_MIN)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(datetime(" + std::to_string(INT_MIN) + "))")->getString());
  ...
  ASSERT_EQ(dolphindb::Util::createDateTime(INT_MIN)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(datetime(" + std::to_string(INT_MIN) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createDateTime(negSec)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(datetime(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(negSec3600)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(datetime(" + std::to_string(negSec3600) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(datetime(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec3600)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(datetime(" + std::to_string(posSec3600) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createDateTime(negSec)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(datetime(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(negSec86400)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(datetime(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(datetime(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec86400)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(datetime(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createDateTime(negSec)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(datetime(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(negSec86400)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(datetime(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(datetime(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec86400)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(datetime(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createDateTime(negSec)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(datetime(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(negSec86400)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(datetime(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(datetime(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec86400)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(datetime(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createDateTime(negSec)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(datetime(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(negSec86400)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(datetime(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(datetime(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateTime(posSec86400)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(datetime(" + std::to_string(posSec86400) + "))")->getString());
}
...
TEST_F(ScalarTest, testFunctionCastTemporal_NanoTimestamp) {
  ...
  // std::cout<<negSec<<std::endl<<posSec<<std::endl<<negSec3600<<std::endl<<posSec3600<<std::endl;
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_NANOTIMESTAMP)->getString(),
            conn.run("nanotimestamp(nanotimestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(nanotimestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_MONTH)->getString(),
            conn.run("month(nanotimestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_TIME)->getString(),
            conn.run("time(nanotimestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(nanotimestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(nanotimestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(nanotimestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ...
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(nanotimestamp(" + std::to_string(LLONG_MIN) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(nanotimestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec3600)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(nanotimestamp(" + std::to_string(negSec3600) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(nanotimestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec3600)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(nanotimestamp(" + std::to_string(posSec3600) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(nanotimestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec86400)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(nanotimestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(nanotimestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec86400)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(nanotimestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(nanotimestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec86400)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(nanotimestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(nanotimestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec86400)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(nanotimestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(nanotimestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec86400)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(nanotimestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(nanotimestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec86400)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(nanotimestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(nanotimestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec86400)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(nanotimestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(nanotimestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec86400)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(nanotimestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(nanotimestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec86400)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(nanotimestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(nanotimestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec86400)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(nanotimestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec)->castTemporal(dolphindb::DT_TIMESTAMP)->getString(),
            conn.run("timestamp(nanotimestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(negSec86400)->castTemporal(dolphindb::DT_TIMESTAMP)->getString(),
            conn.run("timestamp(nanotimestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec)->castTemporal(dolphindb::DT_TIMESTAMP)->getString(),
            conn.run("timestamp(nanotimestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTimestamp(posSec86400)->castTemporal(dolphindb::DT_TIMESTAMP)->getString(),
            conn.run("timestamp(nanotimestamp(" + std::to_string(posSec86400) + "))")->getString());
}
...
TEST_F(ScalarTest, testFunctionCastTemporal_Timestamp) {
  ...
  ASSERT_EQ(dolphindb::Util::createTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(timestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_MONTH)->getString(),
            conn.run("month(timestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_TIME)->getString(),
            conn.run("time(timestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(timestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(timestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(timestamp(" + std::to_string(LLONG_MIN) + "))")->getString());
  ...
  ASSERT_EQ(dolphindb::Util::createTimestamp(LLONG_MIN)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(timestamp(" + std::to_string(LLONG_MIN) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(timestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec3600)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(timestamp(" + std::to_string(negSec3600) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(timestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec3600)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(timestamp(" + std::to_string(posSec3600) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(timestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec86400)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(timestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(timestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec86400)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(timestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(timestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec86400)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(timestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(timestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec86400)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(timestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(timestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec86400)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(timestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(timestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec86400)->castTemporal(dolphindb::DT_NANOTIME)->getString(),
            conn.run("nanotime(timestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(timestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec86400)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(timestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(timestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec86400)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(timestamp(" + std::to_string(posSec86400) + "))")->getString());

  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(timestamp(" + std::to_string(negSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(negSec86400)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(timestamp(" + std::to_string(negSec86400) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(timestamp(" + std::to_string(posSec) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTimestamp(posSec86400)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(timestamp(" + std::to_string(posSec86400) + "))")->getString());
}
...
TEST_F(ScalarTest, testFunctionCastTemporal_datehour) {
  ...
  ASSERT_EQ(dolphindb::Util::createDateHour(INT_MIN)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(datehour(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateHour(INT_MIN)->castTemporal(dolphindb::DT_MONTH)->getString(),
            conn.run("month(datehour(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateHour(INT_MIN)->castTemporal(dolphindb::DT_TIME)->getString(),
            conn.run("time(datehour(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateHour(INT_MIN)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(datehour(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateHour(INT_MIN)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(datehour(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDateHour(INT_MIN)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(datehour(" + std::to_string(INT_MIN) + "))")->getString());
  ...
  ASSERT_EQ(dolphindb::Util::createDateHour(INT_MIN)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(datehour(" + std::to_string(INT_MIN) + "))")->getString());
}
...
TEST_F(ScalarTest, testFunctionCastTemporal_date) {
  ...
  ASSERT_EQ(dolphindb::Util::createDate(INT_MIN)->castTemporal(dolphindb::DT_DATE)->getString(),
            conn.run("date(date(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDate(INT_MIN)->castTemporal(dolphindb::DT_MONTH)->getString(),
            conn.run("month(date(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDate(INT_MIN)->castTemporal(dolphindb::DT_DATETIME)->getString(),
            conn.run("datetime(date(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createDate(INT_MIN)->castTemporal(dolphindb::DT_TIMESTAMP)->getString(), "");
  ASSERT_EQ(dolphindb::Util::createDate(INT_MIN)->castTemporal(dolphindb::DT_DATEHOUR)->getString(),
            conn.run("datehour(date(" + std::to_string(INT_MIN) + "))")->getString());
}
...
TEST_F(ScalarTest, testFunctionCastTemporal_second) {
  ...
  // std::cout<<negSec<<std::endl<<posSec<<std::endl<<negSec3600<<std::endl<<posSec3600<<std::endl;
  ASSERT_EQ(dolphindb::Util::createSecond(INT_MIN)->castTemporal(dolphindb::DT_TIME)->getString(),
            conn.run("time(second(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createSecond(INT_MIN)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(second(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createSecond(INT_MIN)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(second(" + std::to_string(INT_MIN) + "))")->getString());
...
TEST_F(ScalarTest, testFunctionCastTemporal_minute) {
  ...
  // std::cout<<negSec<<std::endl<<posSec<<std::endl<<negSec3600<<std::endl<<posSec3600<<std::endl;
  ASSERT_EQ(dolphindb::Util::createMinute(INT_MIN)->castTemporal(dolphindb::DT_TIME)->getString(),
            conn.run("time(minute(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createMinute(INT_MIN)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(minute(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createMinute(INT_MIN)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(minute(" + std::to_string(INT_MIN) + "))")->getString());
...
TEST_F(ScalarTest, testFunctionCastTemporal_time) {
  ...
  // std::cout<<negSec<<std::endl<<posSec<<std::endl<<negSec3600<<std::endl<<posSec3600<<std::endl;
  ASSERT_EQ(dolphindb::Util::createTime(INT_MIN)->castTemporal(dolphindb::DT_TIME)->getString(),
            conn.run("time(time(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTime(INT_MIN)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(time(" + std::to_string(INT_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createTime(INT_MIN)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(time(" + std::to_string(INT_MIN) + "))")->getString());
...
TEST_F(ScalarTest, testFunctionCastTemporal_nanotime) {
  ...
  // std::cout<<negSec<<std::endl<<posSec<<std::endl<<negSec3600<<std::endl<<posSec3600<<std::endl;
  ASSERT_EQ(dolphindb::Util::createNanoTime(LLONG_MIN)->castTemporal(dolphindb::DT_TIME)->getString(),
            conn.run("time(nanotime(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTime(LLONG_MIN)->castTemporal(dolphindb::DT_MINUTE)->getString(),
            conn.run("minute(nanotime(" + std::to_string(LLONG_MIN) + "))")->getString());
  ASSERT_EQ(dolphindb::Util::createNanoTime(LLONG_MIN)->castTemporal(dolphindb::DT_SECOND)->getString(),
            conn.run("second(nanotime(" + std::to_string(LLONG_MIN) + "))")->getString());
...
class ScalarTest_download : public ScalarTest, public testing::WithParamInterface<std::tuple<std::string, std::string, dolphindb::DATA_TYPE>> {
 public:
  static std::vector<std::tuple<std::string, std::string, dolphindb::DATA_TYPE>> getData() {
    return {std::make_tuple(std::string("x=false;x"), std::string("0"), dolphindb::DT_BOOL),
            ...
            std::make_tuple(std::string("x=int128(`0123456789abcdef0123456789abcdef);x"), std::string("0123456789abcdef0123456789abcdef"),
                            dolphindb::DT_INT128),
            std::make_tuple(std::string("x=ipaddr('192.168.1.1');x"), std::string("192.168.1.1"), dolphindb::DT_IP),
            std::make_tuple(std::string("x=uuid('5d212a78-cc48-e3b1-4235-b4d91473ee87');x"), std::string("5d212a78-cc48-e3b1-4235-b4d91473ee87"),
                            dolphindb::DT_UUID),
            ...
            std::make_tuple(std::string("x=nanotimestamp(2022.01.01T13:30:00.123456789);x"), std::string("2022.01.01T13:30:00.123456789"),
                            dolphindb::DT_NANOTIMESTAMP)};
  }
};
