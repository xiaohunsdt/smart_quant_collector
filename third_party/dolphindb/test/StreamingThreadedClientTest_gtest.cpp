...
static void createSharedTableAndReplay(const std::string& st, int rows) {
  ...
  std::string replayScript = "n = " + std::to_string(rows) +
                             ";table1_STCT = table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]);\
                share table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]) as " +
                             st +
                             "_res_STCT;go;\
                tableInsert(table1_STCT, 2012.01.01T01:21:23 + 1..n, 2018.12.01T01:21:23.000 + 1..n, take(`a`b`c,n), rand(100,n)+rand(1.0, n), rand(100,n)+rand(1.0, n));share table1_STCT as " +
                             st +
                             "_ex_STCT;\
                replay(inputTables=table1_STCT, outputTables=`" +
...
static void createSharedTableAndReplay_withAllDataType(const std::string& st) {
  ...
  std::string replayScript =
      ...
      "),row_num);go;"
      "for (i in "
      "0..(row_num-1)){tableInsert(table1_STCT,i,col1[i],col2[i],col3[i],col4[i],col5[i],col6[i],col7[i],col8[i],col9["
...
static void createSharedTableAndReplay_withArrayVector(const std::string& st) {
  std::string replayScript =
      ...
      "table1_STCT=table(ts,cbool,cchar,cshort,cint,clong,cdate,cmonth,ctime,cminute,csecond,cdatetime,ctimestamp,"
...
TEST_P(StreamingThreadedClientTester, test_subscribe_onehandler) {
  ...
  ASSERT_TRUE(conn.run("re = select * from " + case_ + "_res_STCT order by datetimev;\
                            ex = select * from " +
                       case_ + "_ex_STCT order by datetimev;\
...
TEST_P(StreamingThreadedClientTester, test_subscribe_onehandler_tableNameNull) {
  ...
  ASSERT_ANY_THROW(auto thread =
                       threadedClient.subscribe(HOST, PORT, onehandler, "", DEFAULT_ACTION_NAME, 0, false, nullptr, false, false, USER, PASSWD));
}
...
TEST_P(StreamingThreadedClientTester, test_subscribe_batchhandler_tableNameNull) {
  ...
  dolphindb::ThreadedClient threadedClient = dolphindb::ThreadedClient(listenport);
  ASSERT_ANY_THROW(auto thread = threadedClient.subscribe(HOST, PORT, batchhandler, "", DEFAULT_ACTION_NAME, 0, false, nullptr, false, 1, 1.0, false,
                                                          USER, PASSWD));
}
...
TEST_P(StreamingThreadedClientTester, test_subscribe_onehandler_offsetInMiddle) {
  ...
  ASSERT_TRUE(conn.run("re = select * from " + case_ + "_res_STCT order by datetimev;\
                            ex = select * from " +
                       case_ + "_ex_STCT order by datetimev limit 5, 5;\
                            all(each(eqObj, re.values(), ex.values()))")
                  ->getBool());
  dolphindb::Util::sleep(1000);
  ASSERT_TRUE(conn.run("(exec count(*) from getStreamingStat()[`pubConns] where tables =`" + st + ") ==0")->getBool());
...
TEST_P(StreamingThreadedClientTester, test_subscribe_resub_false) {
  ...
  dolphindb::ThreadedClient threadedClient = dolphindb::ThreadedClient(listenport);
  ASSERT_ANY_THROW(auto th =
                       threadedClient.subscribe(HOST, PORT, batchhandler, "st", "actionTest", 0, false, nullptr, false, 1, 1.0, false, USER, PASSWD));
}
...
TEST_P(StreamingThreadedClientTester, test_subscribe_resub_true) {
  ...
  dolphindb::ThreadedClient threadedClient = dolphindb::ThreadedClient(listenport);
  auto thread =
      threadedClient.subscribe(HOST, PORT, batchhandler, "nonExistTable", "resubTest", 0, true, nullptr, false, 1, 1.0, false, USER, PASSWD);
  dolphindb::Util::sleep(1000);
...
TEST_P(StreamingThreadedClientTester, tes_onehandler_subscribe_twice) {
  ...
  ASSERT_ANY_THROW(auto thread2 =
                       threadedClient.subscribe(HOST, PORT, onehandler, st, "mutiSchemaOne", 0, false, nullptr, false, true, USER, PASSWD));
...
TEST_P(StreamingThreadedClientTester, test_batchhandler_subscribe_twice) {
  ...
  ASSERT_ANY_THROW(auto thread2 = threadedClient.subscribe(HOST, PORT, batchhandler, st, "mutiSchemaBatch", 0, false, nullptr, false, 1, 1.0, false,
                                                           USER, PASSWD));
  notify.wait();
...
TEST_P(StreamingThreadedClientTester, test_subscribe_hugetable_batchhandler_msgAsTable_batchSize_throttle) {
  ...
  dolphindb::ThreadedClient threadedClient = dolphindb::ThreadedClient(listenport);
  auto thread = threadedClient.subscribe(HOST, PORT, batchhandler, st, "mutiSchemaBatch", 0, false, nullptr, false, test_batchSize, test_throttle,
                                         true, USER, PASSWD);
  notify.wait();
...
class StreamingThreadedClientTester_realtime : public testing::Test,
                                               public ::testing::WithParamInterface<std::tuple<int, std::pair<dolphindb::DATA_TYPE, std::string>>> {
 public:
  static dolphindb::DBConnection conn;
  // Suite
  static void SetUpTestSuite() {
...
INSTANTIATE_TEST_SUITE_P(basicTypes, StreamingThreadedClientTester_realtime,
                         testing::Combine(testing::Values(0, rand() % 1000 + 13000),
                                          testing::ValuesIn(StreamingThreadedClientTester_realtime::getData())));
INSTANTIATE_TEST_SUITE_P(arrayVectorTypes, StreamingThreadedClientTester_realtime,
                         testing::Combine(testing::Values(0, rand() % 1000 + 13000),
...
TEST_P(StreamingThreadedClientTester_realtime, test_realtime) {
  ...
  std::thread th = std::thread([&]() { insertData(conn, st, dataScript); });

  auto thread =
      threadedClient.subscribe(HOST, PORT, onehandler, st, "test_realtime", 0, true, nullptr, false, false, USER, PASSWD, nullptr, {}, 100, false, 0);
  th.join();
  notify.wait();
  std::cout << "total size: " << msg_total << std::endl;
...
TEST_P(StreamingThreadedClientTester, test_resub_true_with_resubscribeTimeout) {
  ...
  auto t = threadedClient1.subscribe(
...
TEST_F(StreamingThreadedClientTester, test_new_subscribe_resub_true) {
  ...
  dolphindb::Util::sleep(2000);
  conn.run("subinfo = (exec subscriber from getStreamingStat().pubTables where tableName=`" + st +
           ")[0].split(':');"
           "subClient = subinfo[0];"
           "subPort=int(subinfo[1]);go;"
           "stopPublishTable(subClient, subPort, `" +
           st + ", `resubTest)");
