...
TEST_F(DataformMatrixTest, testBlobMatrix) { ASSERT_ANY_THROW(dolphindb::VectorSP v1 = dolphindb::Util::createMatrix(dolphindb::DT_BLOB, 2, 2, 4)); }

TEST_F(DataformMatrixTest, testAnyMatrix) { ASSERT_ANY_THROW(dolphindb::VectorSP v1 = dolphindb::Util::createMatrix(dolphindb::DT_ANY, 2, 2, 4)); }
...
TEST_F(DataformMatrixTest, testBoolMatrix) {
  ...
  ASSERT_EQ(winMatrix1->getValue(2)->getString(),
            "[" + winMatrix1->getRow(0)->get(0)->getString() + "," + winMatrix1->getRow(0)->get(1)->getString() + "]");
}
...
TEST_F(DataformMatrixTest, testDatehourMatrix) {
  ...
  vals->append(dolphindb::Util::createDateHour(0));
  v1->set(cols, vals);
  ASSERT_EQ(v1->getString(), "#0            #1           \n------------- -------------\n" + v1->getColumn(0)->get(0)->getString() + " " +
                                 v1->getColumn(1)->get(0)->getString() + "\n" + v1->getColumn(0)->get(1)->getString() + " " +
                                 v1->getColumn(1)->get(1)->getString() + "\n");
  cols->append(dolphindb::Util::createInt(-1));
  ASSERT_ANY_THROW(v1->set(cols, vals));
  cols->set(2, dolphindb::Util::createInt(3));
  ASSERT_ANY_THROW(v1->set(cols, vals));
...
TEST_F(DataformMatrixTest, test_Indexedmatrix) {
  dolphindb::VectorSP mat = conn.run(
      "m=matrix(1..5, 11..15);\
...
TEST_F(DataformMatrixTest, test_IndexedSeries) {
  dolphindb::VectorSP series = conn.run(
 ...
 public:
  static const std::vector<std::string> getData() {
    return {
        ...
        "matrix(2020.01.01T00:00:01.000000000 2020.01.01T00:00:02.000000000, 2020.01.01T00:00:03.000000000 "
        "2020.01.01T00:00:04.000000000)",
    };
  }
};
