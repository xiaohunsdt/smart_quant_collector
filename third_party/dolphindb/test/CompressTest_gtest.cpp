...
TEST_F(CompressTest, CompressIncludeNull) {
  ...
  std::vector<dolphindb::ConstantSP> colVector{timeVector, valueVector, nameVector};
  dolphindb::TableSP table = dolphindb::Util::createTable(colName, colVector);
  std::vector<dolphindb::COMPRESS_METHOD> typeVec{dolphindb::COMPRESS_METHOD::COMPRESS_DELTA, dolphindb::COMPRESS_METHOD::COMPRESS_LZ4,
                                                  dolphindb::COMPRESS_METHOD::COMPRESS_LZ4};
  table->setColumnCompressMethods(typeVec);
...
TEST_F(CompressTest, insertTableCompressWithAllType) {
  ...
  std::vector<std::string> colName = {"ind",     "cbool",   "cchar",     "cshort",     "cint",       "cdate",          "cmonth",     "ctime",
                                      "cminute", "csecond", "cdatetime", "ctimestamp", "cnanotime",  "cnanotimestamp", "cfloat",     "cdouble",
                                      "csymbol", "cstring", "cipaddr",   "cblob",      "cdecimal32", "cdecimal64",     "cdecimal128"};
  std::vector<dolphindb::ConstantSP> colVector{
      indV,         boolVector,   charVector,     shortVector,     intVector,       dateVector,          monthVector,     timeVector,
      minuteVector, secondVector, datetimeVector, timestampVector, nanotimeVector,  nanotimestampVector, floatVector,     doubleVector,
      symbolVector, stringVector, ipaddrVector,   blobVector,      decimal32Vector, decimal64Vector,     decimal128Vector};
  dolphindb::TableSP table = dolphindb::Util::createTable(colName, colVector);
  std::vector<dolphindb::COMPRESS_METHOD> typeVec{
      dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA,
      ...
      dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_DELTA, dolphindb::COMPRESS_LZ4,
      dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,
      dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4,   dolphindb::COMPRESS_LZ4};
  table->setColumnCompressMethods(typeVec);

  std::vector<dolphindb::ConstantSP> args{table};
  std::string script =
      "colName =  "
      "`ind`cbool`cchar`cshort`cint`cdate`cmonth`ctime`cminute`csecond`cdatetime`ctimestamp`cnanotime`cnanotimestamp`"
      "cfloat`cdouble`csymbol`cstring`cipaddr`cblob`cdecimal32`cdecimal64`cdecimal128;\n"
      "colType = [INT, BOOL, CHAR, SHORT, INT, DATE, MONTH, TIME, MINUTE, SECOND, DATETIME, TIMESTAMP, NANOTIME, "
      ...
      "dbname = '" +
      dbName +
      "';tableName = 'insertTableCompressWithAllType';"
      "if(existsDatabase(dbname)){dropDatabase(dbname)};go;"
      "db = database(dbname,HASH,[INT, 2],,'TSDB');"
      "compress_methods={cbool:`lz4, cchar:`lz4, cshort:`delta, cint:`delta, cdate:`delta, cmonth:`delta, "
      "ctime:`delta, cminute:`delta, csecond:`delta, cdatetime:`delta, ctimestamp:`lz4, cnanotime:`lz4, "
      "cnanotimestamp:`lz4, cfloat:`lz4, cdouble:`lz4, csymbol:`lz4, cstring:`lz4, cipaddr:`lz4, cblob:`lz4, "
      "cdecimal32:`lz4, cdecimal64:`lz4, cdecimal128:`lz4};"
  ...
  dolphindb::ConstantSP ans = conn.run(
      "ex= select * from loadTable(dbname,tableName) order by ind;"
...
TEST_F(CompressTest, uploadArrayVectorwithCompress) {
  ...
  ASSERT_EQ(objs->getColumn(1)->getString(),
            "[\"BOOL[]\",\"CHAR[]\",\"SHORT[]\",\"LONG[]\",\"INT[]\",\"FLOAT[]\",\"DOUBLE[]\",\"DATE[]\",\"TIMESTAMP[]"
            "\",\"DATETIME[]\",\"DATEHOUR[]\",\"TIME[]\",\"MINUTE[]\",\"MONTH[]\",\"SECOND[]\",\"NANOTIME[]\","
            "\"NANOTIMESTAMP[]\",\"INT128[]\",\"UUID[]\",\"IPADDR[]\"]");
}
...
TEST_F(CompressTest, uploadArrayVectorTablewithCompress) {
  ...
  std::vector<dolphindb::ConstantSP> colVector{timeVector,         valueVector, boolArray,   charArray,  shortArray,     longArray,
                                               intArray,           floatArray,  doubleArray, dateArray,  timestampArray, datetimeArray,
                                               datehourArray,      timeArray,   minuteArray, monthArray, secondeArray,   nanotimeArray,
                                               nanotimestampArray, int128Array, uuidArray,   ipaddrArray};
  dolphindb::TableSP table = dolphindb::Util::createTable(colName, colVector);
  ...
  conn.run(
      ...
      "}\n"
      "colType = "
      "[DATE,LONG,BOOL[],CHAR[],SHORT[],INT[],LONG[],FLOAT[],DOUBLE[],DATE[],TIMESTAMP[],DATEHOUR[],DATETIME[],\n"
...
TEST_F(CompressTest, insertTableCompressWithDecimal128ArrayVector) {
  ...
  std::vector<dolphindb::ConstantSP> args{table};
  std::string script =
      "colName =  [`decimal128av];"
      "colType = [DECIMAL128(" +
      std::to_string(scale128) +
      ")[]];"
      "table1=streamTable(1:0,colName,colType)";
  conn.run(script);
  int success = conn.run("tableInsert{table1}", args)->getInt();
...
TEST_F(CompressTest, upload_other_dataforms_with_compress) {
  ...
  conn.run(
      "assert 1, d.size() == val3.size();"
      "assert 2, 1 in d;"
      "assert 3, 2 in d;"
      "assert 4, 3 in d");
  conn.run(
      "assert 5, each(eqObj, sort(val4.keys()), sort(ee.keys()));"
 ...
 public:
  static std::vector<std::vector<std::string>> get_data() {
    return {
        ...
        {"compress(int128(`e1671797c52e15f763380b45e841ec32`e1671797c52e15f763380b45e841ec33))",
         "[e1671797c52e15f763380b45e841ec32,e1671797c52e15f763380b45e841ec33]"},
        {"compress(uuid('5d212a78-cc48-e3b1-4235-b4d91473ee87''5d212a78-cc48-e3b1-4235-b4d91473ee88'))",
         "[5d212a78-cc48-e3b1-4235-b4d91473ee87,5d212a78-cc48-e3b1-4235-b4d91473ee88]"},
        ...
        {"compress(table(rand(`APPL`GOOL`MSFT`TSLA, 70000) as sym, rand(100.0000, 70000) as price, rand(100, 70000) as "
         "qty, arrayVector(1..70000*5, rand(100.0000,     350000)) as value))",
         ""},
        // delta compression
        ...
        {"compress(table(rand(`APPL`GOOL`MSFT`TSLA, 70000) as sym, rand(100.0000, 70000) as price, rand(100, 70000) as "
         "qty, arrayVector(1..70000*5, rand(100.0000,     350000)) as value), 'delta')",
         ""},
    };
  }
};
