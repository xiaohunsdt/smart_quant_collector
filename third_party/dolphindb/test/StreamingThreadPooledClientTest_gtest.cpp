...
static void createSharedTableAndReplay(const std::string &st, int rows) {
  ...
  std::string replayScript = "n = " + std::to_string(rows) +
                             ";table1_STPCT = table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]);\
                share table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]) as " +
                             st +
                             "_res_STPCT;go;\
                tableInsert(table1_STPCT, 2012.01.01T01:21:23 + 1..n, 2018.12.01T01:21:23.000 + 1..n, take(`a`b`c,n), rand(100,n)+rand(1.0, n), rand(100,n)+rand(1.0, n));share table1_STPCT as " +
                             st +
                             "_ex_STPCT;\
                replay(inputTables=table1_STPCT, outputTables=`" +
...
static void createSharedTableAndReplay_withAllDataType(const std::string &st) {
  ...
  std::string replayScript =
      ...
      "),row_num);go;"
      "for (i in "
      "0..(row_num-1)){tableInsert(table1_STPCT,i,col1[i],col2[i],col3[i],col4[i],col5[i],col6[i],col7[i],col8[i],col9["
...
static void createSharedTableAndReplay_withArrayVector(const std::string &st) {
  std::string replayScript =
      ...
      "table1_STPCT=table(ts,cbool,cchar,cshort,cint,clong,cdate,cmonth,ctime,cminute,csecond,cdatetime,ctimestamp,"
...
TEST_P(StreamingThreadPooledClientTester, test_client_threadCount_1) {
  ...
  ASSERT_TRUE(conn.run("re = select * from " + case_ + "_res_STPCT order by datetimev;\
                            ex = select * from " +
                       case_ + "_ex_STPCT order by datetimev;\
...
TEST_P(StreamingThreadPooledClientTester, test_client_resub_true) {
  ...
  auto onehandler = [&](dolphindb::Message m) { msgs.push_back(m); };
...
class StreamingThreadPooledClientTester_realtime
    : public testing::Test,
...
INSTANTIATE_TEST_SUITE_P(basicTypes, StreamingThreadPooledClientTester_realtime,
                         testing::Combine(testing::Values(0, rand() % 1000 + 13000),
                                          testing::ValuesIn(StreamingThreadPooledClientTester_realtime::getData())));
INSTANTIATE_TEST_SUITE_P(arrayVectorTypes, StreamingThreadPooledClientTester_realtime,
                         testing::Combine(testing::Values(0, rand() % 1000 + 13000),
