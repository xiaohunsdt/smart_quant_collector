...
static void createSharedTableAndReplay(const std::string& st, int rows) {
  ...
  std::string replayScript = "n = " + std::to_string(rows) +
                             ";table1_SPCT = table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]);\
                share table(1:0, `datetimev`timestampv`sym`price1`price2, [DATETIME, TIMESTAMP, SYMBOL, DOUBLE, DOUBLE]) as " +
                             st +
                             "_res_SPCT;go;\
                tableInsert(table1_SPCT, 2012.01.01T01:21:23 + 1..n, 2018.12.01T01:21:23.000 + 1..n, take(`a`b`c,n), rand(100,n)+rand(1.0, n), rand(100,n)+rand(1.0, n));share table1_SPCT as " +
                             st +
                             "_ex_SPCT;\
                replay(inputTables=table1_SPCT, outputTables=`" +
...
static void createSharedTableAndReplay_withAllDataType(const std::string& st) {
  ...
  std::string replayScript =
      ...
      "),row_num);go;"
      "for (i in "
      "0..(row_num-1)){tableInsert(table1_SPCT,i,col1[i],col2[i],col3[i],col4[i],col5[i],col6[i],col7[i],col8[i],col9["
...
static void createSharedTableAndReplay_withArrayVector(const std::string& st) {
  std::string replayScript =
      ...
      "table1_SPCT=table(ts,cbool,cchar,cshort,cint,clong,cdate,cmonth,ctime,cminute,csecond,cdatetime,ctimestamp,"
...
TEST_P(StreamingPollingClientTester, test_subscribeUnsubscribe_normal) {
  ...
  ASSERT_TRUE(conn.run("re = select * from " + case_ + "_res_SPCT order by datetimev;\
                            ex = select * from " +
                       case_ + "_ex_SPCT order by datetimev;\
  ...
  std::thread th1 = std::thread([&] {
    while(true) {
      ...
      if(!msg.isNull()) {
        ...
        msg_total2 += 1;
      } else if(msg.isNull() && msg2.isNull()) {
        break;
      }
      if(msg.getOffset() == 999 && msg2.getOffset() == 999) notify.set();
    }
  });
...
TEST_P(StreamingPollingClientTester, test_subscribeUnsubscribe_hugetable_msgAsTable) {
  ...
  std::thread th1 = std::thread([&] {
    ...
    while(true) {
      ...
      int first_offset = msg.getOffset();
      int last_offset = first_offset + msg->rows() - 1;
      if(last_offset == 9999999) notify.set();
    }
  });
...
class StreamingPollingClientTester_realtime : public testing::Test,
...
INSTANTIATE_TEST_SUITE_P(basicTypes, StreamingPollingClientTester_realtime,
                         testing::Combine(testing::Values(0, rand() % 1000 + 13000),
                                          testing::ValuesIn(StreamingPollingClientTester_realtime::getData())));
INSTANTIATE_TEST_SUITE_P(arrayVectorTypes, StreamingPollingClientTester_realtime,
                         testing::Combine(testing::Values(0, rand() % 1000 + 13000),
...
TEST_P(StreamingPollingClientTester, test_resub_true_with_resubscribeTimeout) {
  ...
  auto queue1 =
      client1.subscribe(HOST, PORT, st, DEFAULT_ACTION_NAME, 0, true, nullptr, false, false, USER, PASSWD, nullptr, {}, 100, resubscribeTimeout);
