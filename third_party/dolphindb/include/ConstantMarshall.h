...
class EXPORT_DECL VectorMarshall : public ConstantMarshallImp {
 ...
 private:
  bool writeMetaValues(BufferWriter<DataOutputStreamSP>& output, const ConstantSP& target, bool includeMeta, size_t offset, bool blocking,
                       IO_ERR& ret);
  INDEX nextStart_{0};
  int partial_{0};
...
class EXPORT_DECL SymbolBaseUnmarshall {
 public:
  ...
  void reset();
  SymbolBaseSP getSymbolBase() const { return obj_; }
