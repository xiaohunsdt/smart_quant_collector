...
class EXPORT_DECL TemporalFormat {
    ...
    for(int i = 0; i < segmentCount_; ++i) {
      ...
      if(index == 5) {
        ...
        for(int j = 0; j < unitDemandlength; ++j) {
          ...
          tmpNumber /= 10;
          if(cntDigits >= formatDemandLength && tmpNumber == 0) break;
        }
        for(int j = 0; j < formatDemandLength - cntDigits; ++j) tmpString += '0';
 ...
 private:
  void initialize(const std::string& format) {
    if(formatMap.empty()) formatMap = TemporalFormat::initFormatMap();
    int len = static_cast<int>(format.length());
    if(len == 0) throw RuntimeException("The format string can't be empty.");
    if(len > 128) throw RuntimeException("The format string is too big.");
    format_.reserve(len);
    ...
    while(i < len) {
      char ch = format[i];
      if(ch == '\\') {
        if(i == len - 1) throw RuntimeException("Invalid escape (\\)in the end of the format string.");
