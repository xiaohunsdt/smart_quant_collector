...
#define NOT_IMPLEMENT                                                                                                                             \
  throw RuntimeException("Data type [" + std::to_string(static_cast<int>(getType())) + "] form [" + std::to_string(static_cast<int>(getForm())) + \
...
class EXPORT_DECL Constant {
 public:
  ...
  void setTemporary(bool val) {
    if(val)
      flag_ |= FLAG_TEMPORARY;
    else
      flag_ &= (uint16_t)~FLAG_TEMPORARY;
  }
  bool isIndependent() const { return flag_ & FLAG_INDEPENDENT; }
  void setIndependent(bool val) {
    if(val)
      flag_ |= FLAG_INDEPENDENT;
    else
      flag_ &= (uint16_t)~FLAG_INDEPENDENT;
  }
