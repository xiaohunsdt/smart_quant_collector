...
class EXPORT_DECL ErrorCodeInfo {
 public:
  ...
  void clearError() { errorCode.clear(); }
  bool hasError() { return errorCode.empty() == false; }
  bool succeed() { return errorCode.empty(); }
  static std::string formatApiCode(int code) {
    if(code != EC_None) return "A" + std::to_string(code);
    return "";
  }
