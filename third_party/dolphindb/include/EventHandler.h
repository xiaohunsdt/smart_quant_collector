...
class EventHandler {
 public:
  EventHandler(const std::vector<EventSchema>& eventSchemas, const std::vector<std::string>& eventTimeFields,
               const std::vector<std::string>& commonFields);
  bool checkOutputTable(const TableSP& outputTable, std::string& errMsg);
  bool serializeEvent(const std::string& eventType, const std::vector<ConstantSP>& attributes, std::vector<ConstantSP>& serializedEvent,
                      std::string& errMsg);
  bool deserializeEvent(const ConstantSP& obj, std::vector<std::string>& eventTypes, std::vector<std::vector<ConstantSP>>& attributes,
                        ErrorCodeInfo& errorInfo);

 private:
  bool checkSchema(const std::vector<EventSchema>& eventSchemas, const std::vector<std::string>& expandTimeKeys,
...
class EXPORT_DECL EventSender {
 public:
  EventSender(const DBConnectionSP& conn, const std::string& tableName, const std::vector<EventSchema>& eventSchema,
