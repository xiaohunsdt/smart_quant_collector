...
class DBConnectionImpl {
 public:
  explicit DBConnectionImpl(bool sslEnable = false, bool asynTask = false, int keepAliveTime = 7200, bool compress = false, bool python = false,
                            bool isReverseStreaming = false, bool enableSCRAM = false);
  ~DBConnectionImpl();
  bool connect(const std::string& hostName, int port, const std::string& userId = "", const std::string& password = "", bool sslEnable = false,
               bool asynTask = false, int keepAliveTime = -1, bool compress = false, bool python = false);
  ...
  ConstantSP run(const std::string& funcName, std::vector<ConstantSP>& args, int priority = 4, int parallelism = 64, int fetchSize = 0,
                 bool clearMemory = false, long seqNum = 0);
  ...
  void getHostPort(std::string& host, int& port) {
    host = hostName_;
    port = port_;
  }
 ...
 private:
  ...
  ConstantSP run(const std::string& script, const std::string& scriptType, std::vector<ConstantSP>& args, int priority = 4, int parallelism = 64,
                 int fetchSize = 0, bool clearMemory = false, long seqNum = 0);
  bool connect();
  void login();
  void scramLogin();
