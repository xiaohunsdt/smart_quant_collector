...
class SmartPointer {
 public:
  ...
  T& operator*() const { return *((T*)counterP_->p_); }
  ...
  bool operator==(const SmartPointer<T>& sp) const { return counterP_ == sp.counterP_; }
  ...
  bool isNull() const { return counterP_->p_ == nullptr; }

  int count() const { return counterP_->getCount(); }

  T* get() const { return (T*)counterP_->p_; }
