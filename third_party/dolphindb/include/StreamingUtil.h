...
class EXPORT_DECL StreamDeserializer {
 public:
  ...
  explicit StreamDeserializer(const std::unordered_map<std::string, std::pair<std::string, std::string>> &sym2tableName,
 ...
 private:
  class TableInfo {
   public:
    ...
    ConstantSP newTuple();
    void setLimit(INDEX limit) { queuelimit_ = limit; }
