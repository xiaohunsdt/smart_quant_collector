...
class SharedMemStream {
 public:
  ...
  void closeShm() { close(shmFd_); }

  /* Unlink the shared memory object. Even if the peer process
      is still using the object, this is okay. The object will
      be removed only after all open references are closed. */
  void unlinkShm() { shm_unlink(path_.c_str()); }

  size_t getBufSize() { return shmpHeader_->bufSize_; }
...
class IPCInMemTable : public BasicTable {
 public:
  IPCInMemTable(bool create, std::string shmPath, std::string tableName, const std::vector<ConstantSP>& cols,
