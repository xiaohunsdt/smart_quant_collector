...
class ConstantFactory {
  using ConstantParser = Constant* (ConstantFactory::*)(const std::string&);
  using ConstantFunc = Constant* (ConstantFactory::*)(int extraParam);
  using ConstantVectorFunc = Vector* (ConstantFactory::*)(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment,
                                                          int segmentSizeInBit, bool containNull);
  using ConstantArrayVectorFunc = Vector* (ConstantFactory::*)(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex,
                                                               void** dataSegment, int segmentSizeInBit, bool containNull);
  using ConstantMatrixFunc = Vector* (ConstantFactory::*)(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment,
                                                          int segmentSizeInBit, bool containNull);
  using ConstantRptVectorFunc = Vector* (ConstantFactory::*)(const ConstantSP& scalar, INDEX size);

 public:
  ConstantFactory() { init(); }

  Constant* parseConstant(int type, const std::string& word) {
    if(type < 0 || type >= TYPE_COUNT) throw RuntimeException("Invalid data type value " + Util::getDataTypeString((DATA_TYPE)type));
  ...
  Constant* createConstant(DATA_TYPE type, int extraParam) {
    if(type < 0 || type >= TYPE_COUNT) throw RuntimeException("Invalid data type value " + Util::getDataTypeString(type));
    ConstantFunc func = arrConstFactory[type];
    if(func == nullptr) throw RuntimeException("Not allowed to create a scalar with type " + Util::getDataTypeString(type));
    return (this->*func)(extraParam);
  }

  Vector* createConstantVector(DATA_TYPE type, INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment,
                               int segmentSizeInBit, bool containNull) {
    if(type < 0 || type >= TYPE_COUNT) throw RuntimeException("Invalid Vector data type value " + Util::getDataTypeString(type));
    ConstantVectorFunc func = arrConstVectorFactory[type];
    if(func == nullptr) throw RuntimeException("Not allowed to create a vector with type " + Util::getDataTypeString(type));
    return (this->*func)(size, capacity, fastMode, extraParam, data, dataSegment, segmentSizeInBit, containNull);
  }

  Vector* createConstantArrayVector(DATA_TYPE type, INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, INDEX* pindex,
                                    void** dataSegment, int segmentSizeInBit, bool containNull) {
  ...
  Vector* createConstantMatrix(DATA_TYPE type, int cols, int rows, int colCapacity, int extraParam, void* data = nullptr,
                               void** dataSegment = nullptr, int segmentSizeInBit = 0, bool containNull = false) {
    if(type < 0 || type >= TYPE_COUNT) throw RuntimeException("Invalid Matrix data type value " + Util::getDataTypeString(type));
    ConstantMatrixFunc func = arrConstMatrixFactory[type];
    if(func == nullptr) throw RuntimeException("Not allowed to create a matrix with type " + Util::getDataTypeString(type));
  ...
  DATA_TYPE getDataType(const std::string& type) const {
    ...
    if(it == typeMap_.end()) return DT_VOID;
    return it->second;
  }

  DATA_FORM getDataForm(const std::string& form) const {
    auto it = formMap_.find(form);
    if(it == formMap_.end()) return (DATA_FORM)-1;
    return it->second;
  }

  char getDataTypeSymbol(DATA_TYPE type) const { return arrTypeSymbol[type]; }

  std::string getDataTypeString(DATA_TYPE type) const { return dolphindb::getDataTypeName(type); }

  std::string getDataFormString(DATA_FORM form) const {
    if(form >= 0 && form < 9) return arrFormStr[form];
  ...
  std::string getTableTypeString(TABLE_TYPE type) const {
    switch(type) {
      ...
      case COMPRESSTBL:
        return "COMPRESS";
      default:
        return "Uknown table type " + std::to_string(type);
    }
  }
  ...
  Dictionary* createDictionary(DATA_TYPE keyInternalType, DATA_TYPE keyType, DATA_TYPE valueType) {
    ...
    if(valueType != DT_ANY) {
      switch(keyInternalType) {
        ...
        default:
          throw RuntimeException("Not allowed to create Dictionary for the key type " + Util::getDataTypeString(keyType));
      }
    } else {
      switch(keyInternalType) {
        case DT_CHAR:
        ...
        case DT_FLOAT:
          return new FloatAnyDictionary(keyType);
        case DT_DOUBLE:
          return new DoubleAnyDictionary(keyType);
        case DT_INT128:
        ...
        default:
          throw RuntimeException("Not allowed to create Dictionary for this key type " + Util::getDataTypeString(keyType));
      }
    }
  }
  ...
  Set* createSet(DATA_TYPE keyInternalType, DATA_TYPE keyType, INDEX capacity) {
    switch(keyInternalType) {
      ...
      case DT_SYMBOL:
        return new StringSet(capacity, false, true);
      case DT_STRING:
        return new StringSet(capacity, false);
      case DT_INT128:
        return new Int128Set(keyType, capacity);
      case DT_BLOB:
        return new StringSet(capacity, true);
      default:
        return nullptr;
    }
  }
  ...
  std::string getPartitionTypeString(PARTITION_TYPE type) const {
    if(type >= 0 && type < 6) return arrPartitionTypeStr[type];
    return "UnknowPartition" + std::to_string(type);
  }

  std::string getCategoryString(DATA_CATEGORY type) const {
    if(type >= 0 && type < 12) return arrCategoryTypeStr[type];
 ...
 private:
  ...
  Constant* parseDoubleEnum(const std::string& word) {
    char ch = word[0];
    if(ch == 'p') return new EnumDouble(word, MC_PI);
  ...
  Vector* createVoidVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    // throw RuntimeException("Not allowed to create void vector");
    return new FastVoidVector(size, capacity, (char*)data, containNull);
  }

  Vector* createBoolVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<char>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastBoolVector(size, capacity, (char*)data, containNull);
    return nullptr;
  }
  Vector* createCharVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<char>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastCharVector(size, capacity, (char*)data, containNull);
    return nullptr;
  }
  Vector* createShortVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                            bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<short>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastShortVector(size, capacity, (short*)data, containNull);
    return nullptr;
  }
  Vector* createIntVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                          bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastIntVector(size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createLongVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastLongVector(size, capacity, (long long*)data, containNull);
    return nullptr;
  }
  Vector* createFloatVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                            bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<float>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastFloatVector(size, capacity, (float*)data, containNull);
    return nullptr;
  }
  Vector* createDoubleVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<double>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastDoubleVector(size, capacity, (double*)data, containNull);
    return nullptr;
  }
  Vector* createDateVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastDateVector(size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createMonthVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                            bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastMonthVector(size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createDateTimeVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                               bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastDateTimeVector(size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createDateHourVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                               bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastDateHourVector(size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createTimeVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastTimeVector(size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createNanoTimeVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                               bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastNanoTimeVector(size, capacity, (long long*)data, containNull);
    return nullptr;
  }
  Vector* createTimestampVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                                bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastTimestampVector(size, capacity, (long long*)data, containNull);
    return nullptr;
  }
  Vector* createNanoTimestampVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                                    bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastNanoTimestampVector(size, capacity, (long long*)data, containNull);
    return nullptr;
  }
  Vector* createMinuteVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastMinuteVector(size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createSecondVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastSecondVector(size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createStringVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    return new StringVector(size, capacity);
  }
  Vector* createSymbolVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data) {
    ...
    if(data != nullptr) return new FastSymbolVector(new SymbolBase(0), size, capacity, (int*)data, containNull);
    return nullptr;
  }
  Vector* createBlobVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    return new StringVector(size, capacity, true);
  }
  Vector* createAnyVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                          bool containNull) {
    return new AnyVector(size);
  }
  Vector* createInt128Vector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<Guid>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastInt128Vector(DT_INT128, size, capacity, (unsigned char*)data, containNull);
    return nullptr;
  }
  Vector* createUuidVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<Guid>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastUuidVector(size, capacity, (unsigned char*)data, containNull);
    return nullptr;
  }
  Vector* createIPAddrVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<Guid>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastIPAddrVector(size, capacity, (unsigned char*)data, containNull);
    return nullptr;
  }
  Vector* createDecimal32Vector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                                bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int32_t>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastDecimalVector<int32_t>(extraParam, size, capacity, (int32_t*)data, containNull);
    return nullptr;
  }
  Vector* createDecimal64Vector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                                bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<int64_t>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastDecimalVector<int64_t>(extraParam, size, capacity, (int64_t*)data, containNull);
    return nullptr;
  }

  Vector* createDecimal128Vector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                                 bool containNull) {
    if(data == nullptr && dataSegment == nullptr) {
      allocate<wide_integer::int128>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    }
    if(data != nullptr) return new FastDecimalVector<wide_integer::int128>(extraParam, size, capacity, (wide_integer::int128*)data, containNull);
    return nullptr;
  }

  Vector* createBoolArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<char>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createCharArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<char>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createShortArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                 int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<short>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createIntArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                               int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createLongArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createFloatArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                 int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<float>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createDoubleArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                  int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<double>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createDateArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createTimestampArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                     int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createDateHourArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                    int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createDatetimeArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                    int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createTimeArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createMinuteArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                  int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createMonthArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                 int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createSecondArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                  int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createNanotimeArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                    int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createNanoTimestampArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                         int segmentSizeInBit, bool containNull) {
    // if(data == nullptr && dataSegment == nullptr){
    //	allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createUuidArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                int segmentSizeInBit, bool containNull) {
    // if (data == nullptr && dataSegment == nullptr) {
    //	allocate<Guid>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
    // }
  ...
  Vector* createInt128ArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                  int segmentSizeInBit, bool containNull) {
    // if (data == nullptr && dataSegment == nullptr) {
  ...
  Vector* createIpArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                              int segmentSizeInBit, bool containNull) {
    // if (data == nullptr && dataSegment == nullptr) {
  ...
  Vector* createDecimal32ArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                     int segmentSizeInBit, bool containNull) {
  ...
  Vector* createDecimal64ArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                     int segmentSizeInBit, bool containNull) {
  ...
  Vector* createDecimal128ArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void* data, void* pindex, void** dataSegment,
                                      int segmentSizeInBit, bool containNull) {
  ...
  Vector* createVoidMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    throw RuntimeException("Not allowed to create a void matrix");
  }

  Vector* createBoolMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr) return new FastBoolMatrix(cols, rows, colCapacity, allocateZero<char>(cols, rows, colCapacity), false);
    return new FastBoolMatrix(cols, rows, colCapacity, (char*)data, containNull);
  }

  Vector* createCharMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr) return new FastCharMatrix(cols, rows, colCapacity, allocateZero<char>(cols, rows, colCapacity), false);
    return new FastCharMatrix(cols, rows, colCapacity, (char*)data, containNull);
  }

  Vector* createShortMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                            bool containNull) {
    if(data == nullptr) return new FastShortMatrix(cols, rows, colCapacity, allocateZero<short>(cols, rows, colCapacity), false);
    return new FastShortMatrix(cols, rows, colCapacity, (short*)data, containNull);
  }

  Vector* createIntMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                          bool containNull) {
    if(data == nullptr) return new FastIntMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity), false);
    return new FastIntMatrix(cols, rows, colCapacity, (int*)data, containNull);
  }

  Vector* createLongMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr) return new FastLongMatrix(cols, rows, colCapacity, allocateZero<long long>(cols, rows, colCapacity), false);
    return new FastLongMatrix(cols, rows, colCapacity, (long long*)data, containNull);
  }

  Vector* createFloatMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                            bool containNull) {
    if(data == nullptr) return new FastFloatMatrix(cols, rows, colCapacity, allocateZero<float>(cols, rows, colCapacity), false);
    return new FastFloatMatrix(cols, rows, colCapacity, (float*)data, containNull);
  }

  Vector* createDoubleMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data == nullptr) return new FastDoubleMatrix(cols, rows, colCapacity, allocateZero<double>(cols, rows, colCapacity), false);
    return new FastDoubleMatrix(cols, rows, colCapacity, (double*)data, containNull);
  }

  Vector* createDateMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr) return new FastDateMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity), false);
    return new FastDateMatrix(cols, rows, colCapacity, (int*)data, containNull);
  }

  Vector* createDateTimeMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                               bool containNull) {
    if(data == nullptr) return new FastDateTimeMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity), false);
    return new FastDateTimeMatrix(cols, rows, colCapacity, (int*)data, containNull);
  }

  Vector* createDateHourMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                               bool containNull) {
    if(data == nullptr) return new FastDateHourMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity), false);
    return new FastDateHourMatrix(cols, rows, colCapacity, (int*)data, containNull);
  }

  Vector* createMonthMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                            bool containNull) {
    if(data == nullptr) return new FastMonthMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity), false);
    return new FastMonthMatrix(cols, rows, colCapacity, (int*)data, containNull);
  }

  Vector* createTimeMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                           bool containNull) {
    if(data == nullptr) return new FastTimeMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity), false);
    return new FastTimeMatrix(cols, rows, colCapacity, (int*)data, containNull);
  }

  Vector* createSecondMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data == nullptr) return new FastSecondMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity), false);
    return new FastSecondMatrix(cols, rows, colCapacity, (int*)data, containNull);
  }

  Vector* createMinuteMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                             bool containNull) {
    if(data == nullptr) return new FastMinuteMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity), false);
    return new FastMinuteMatrix(cols, rows, colCapacity, (int*)data, containNull);
  }

  Vector* createNanoTimeMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                               bool containNull) {
    if(data == nullptr) return new FastNanoTimeMatrix(cols, rows, colCapacity, allocateZero<long long>(cols, rows, colCapacity), false);
    return new FastNanoTimeMatrix(cols, rows, colCapacity, (long long*)data, containNull);
  }

  Vector* createTimestampMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                                bool containNull) {
    if(data == nullptr) return new FastTimestampMatrix(cols, rows, colCapacity, allocateZero<long long>(cols, rows, colCapacity), false);
    return new FastTimestampMatrix(cols, rows, colCapacity, (long long*)data, containNull);
  }

  Vector* createNanoTimestampMatrix(int cols, int rows, int colCapacity, int extraParam, void* data, void** dataSegment, int segmentSizeInBit,
                                    bool containNull) {
    if(data == nullptr) return new FastNanoTimestampMatrix(cols, rows, colCapacity, allocateZero<long long>(cols, rows, colCapacity), false);
  ...
  void init() {
    ...
    arrConstFactory[DT_DICTIONARY] = nullptr;
    ...
    arrConstVectorFactory[DT_DICTIONARY] = nullptr;
    ...
    arrConstVectorFactory[DT_COMPRESS] = nullptr;
    ...
    arrConstMatrixFactory[DT_STRING] = nullptr;
    arrConstMatrixFactory[DT_DICTIONARY] = nullptr;
    arrConstMatrixFactory[DT_ANY] = nullptr;
