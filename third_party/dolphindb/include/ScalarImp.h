...
class Void : public Constant {
 public:
  ...
  bool getString(INDEX start, int len, std::string **buf) const override { return set1(start, len, buf, &Constant::EMPTY); }
...
class Int128 : public Constant {
 public:
  ...
  static Int128 *parseInt128(const char *str, size_t len) {
    ...
    if(len == 32 && Util::fromHex(str, len, Util::LITTLE_ENDIAN_ORDER, buf)) return new Int128(buf);
    return nullptr;
  }
  static bool parseInt128(const char *str, size_t len, unsigned char *buf) {
    ...
    if(len == 32 && Util::fromHex(str, len, Util::LITTLE_ENDIAN_ORDER, buf)) return true;
    return false;
  }
...
class String : public Constant {
 public:
  ...
  void nullFill(const ConstantSP &val) override {
    if(isNull()) val_ = val->getStringRef();
  }
  bool isNull(INDEX start, int len, char *buf) const override {
    char null = isNull();
    for(int i = 0; i < len; ++i) buf[i] = null;
  ...
  bool getString(INDEX start, int len, std::string **buf) const override {
    for(int i = 0; i < len; ++i) buf[i] = &val_;
    return true;
  }
  std::string **getStringConst(INDEX start, int len, std::string **buf) const override {
    for(int i = 0; i < len; ++i) buf[i] = &val_;
  ...
  int serialize(char *buf, int bufSize, INDEX indexStart, int offset, int &numElement, int &partial) const override {
    ...
    if(offset > 0) {
      ...
      if(UNLIKELY((size_t)bufSize < sizeof(int))) return 0;
...
class AbstractScalar : public Constant {
 public:
  ...
  void setBool(char val) override {
    if(val != CHAR_MIN)
      val_ = (T)val;
    else
      setNull();
  ...
  void setShort(short val) override {
    if(val != SHRT_MIN)
      val_ = (T)val;
    else
      setNull();
  ...
  void setLong(long long val) override {
    if(val != LLONG_MIN)
      val_ = (T)val;
    else
      setNull();
  }
  void setIndex(INDEX val) override {
    if(val != INDEX_MIN)
      val_ = (T)val;
    else
      setNull();
 ...
 public:
  ...
  static Char *parseChar(const std::string &str) {
    ...
    if(str == "00" || str.empty()) {
      ...
      if(val > 127 || val < -128) return nullptr;
  ...
  static std::string toString(char val) {
    if(val == CHAR_MIN) return "";
    if(val > 31 && val < 127) return std::string(1, val);
...
inline int countTemporalUnit(int days, int multiplier, int remainder) { return days == INT_MIN ? INT_MIN : (days * multiplier) + remainder; }
...
class Time : public TemporalScalar {
 public:
  ...
  void validate() override {
    if(val_ >= 86400000 || val_ < 0) val_ = INT_MIN;
  }
  static Time *parseTime(const std::string &str) {
    ...
    if(str.size() != 12) return p;
    int hour, minute, second, milliSecond = 0;
    hour = atoi(str.substr(0, 2).c_str());
    if(hour >= 24 || str[2] != ':') return p;
    minute = atoi(str.substr(3, 2).c_str());
    if(minute >= 60 || str[5] != ':') return p;
    second = atoi(str.substr(6, 2).c_str());
    if(second >= 60) return p;
    if(str[8] == '.') milliSecond = atoi(str.substr(9, str.length() - 9).c_str());
    p = new Time(hour, minute, second, milliSecond);
    return p;
  }
  static std::string toString(int val) {
    if(val < 0 || val >= 86400000) return "";
...
class NanoTime : public Long {
 public:
  ...
  std::string getString() const override { return toString(val_); }
  void validate() override {
    if(val_ >= 86400000000000LL || val_ < 0) val_ = LLONG_MIN;
  }
  ...
  static std::string toString(long long val) {
    if(val < 0 || val >= 86400000000000LL) return "";
...
class Timestamp : public Long {
 public:
  ...
  ConstantSP castTemporal(DATA_TYPE expectType) override {
    ...
    int remainder = val_ % 86400000;
    val_ == LLONG_MIN ? result = INT_MIN
                      : result = static_cast<int>((remainder + static_cast<int>((val_ < 0) && (remainder != 0)) * 86400000) / ratio);
    return Util::createObject(expectType, result);
  }
  ...
  static Timestamp *parseTimestamp(const std::string &str) {
    ...
    year = atoi(str.substr(0, 4).c_str());
    if(year == 0 || str[4] != '.') return p;
    month = atoi(str.substr(5, 2).c_str());
    if(month == 0 || str[7] != '.') return p;
    day = atoi(str.substr(8, 2).c_str());
    if(day == 0 || (str[10] != ' ' && str[10] != 'T')) return p;

    hour = atoi(str.substr(11, 2).c_str());
    if(hour >= 24 || str[13] != ':') return p;
    minute = atoi(str.substr(14, 2).c_str());
    if(minute >= 60 || str[16] != ':') return p;
...
class NanoTimestamp : public Long {
 public:
  ...
  ConstantSP castTemporal(DATA_TYPE expectType) override {
    ...
    long long remainder = val_ % 86400000000000LL;
    val_ == LLONG_MIN ? result = INT_MIN
                      : result = static_cast<int>((remainder + static_cast<long long>(val_ < 0 && (remainder != 0)) * 86400000000000LL) / ratio);
    return Util::createObject(expectType, result);
  }
...
class Minute : public TemporalScalar {
 public:
  ...
  std::string getString() const override { return toString(val_); }
  void validate() override {
    if(val_ >= 1440 || val_ < 0) val_ = INT_MIN;
  }
  static Minute *parseMinute(const std::string &str) {
    ...
    if(hour >= 24 || str[2] != ':') return p;
    minute = atoi(str.substr(3, 2).c_str());
    if(minute >= 60) return p;
    p = new Minute(hour, minute);
    return p;
  }
  static std::string toString(int val) {
    if(val < 0 || val >= 1440) return "";
...
class Second : public TemporalScalar {
 public:
  ...
  std::string getString() const override { return toString(val_); }
  void validate() override {
    if(val_ >= 86400 || val_ < 0) val_ = INT_MIN;
  }
 ...
 public:
  ...
  ConstantSP castTemporal(DATA_TYPE expectType) override {
    ...
    if(expectType == DT_NANOTIME) {
      ...
      val_ == INT_MIN ? result = LLONG_MIN
                      : result = (long long)(remainder + (static_cast<int>((val_ < 0) && (remainder != 0)) * 86400)) * 1000000000LL;
      return Util::createObject(expectType, result);
    }
...
class DateHour : public TemporalScalar {
 public:
  ...
  DateHour(int year, int month, int day, int hour) : TemporalScalar(countTemporalUnit(Util::countDays(year, month, day), 24, hour)) {}
  ...
  static DateHour *parseDateHour(const std::string &str) {
    ...
    if(UNLIKELY(str.size() < 13)) return p;
...
inline ConstantSP operator""_d(const char *s, size_t len) { return Date::parseDate(s); }

inline ConstantSP operator""_M(const char *s, size_t len) { return Month::parseMonth(s); }

inline ConstantSP operator""_t(const char *s, size_t len) { return Time::parseTime(s); }

inline ConstantSP operator""_m(const char *s, size_t len) { return Minute::parseMinute(s); }

inline ConstantSP operator""_s(const char *s, size_t len) { return Second::parseSecond(s); }

inline ConstantSP operator""_D(const char *s, size_t len) { return DateTime::parseDateTime(s); }

inline ConstantSP operator""_T(const char *s, size_t len) { return Timestamp::parseTimestamp(s); }

inline ConstantSP operator""_n(const char *s, size_t len) { return NanoTime::parseNanoTime(s); }

inline ConstantSP operator""_N(const char *s, size_t len) { return NanoTimestamp::parseNanoTimestamp(s); }
