...
class DBConnectionPoolImpl {
 public:
  struct Task {
    explicit Task(std::string script_ = "", std::shared_ptr<std::condition_variable> finished_ = nullptr, int id = 0,
                  const RpcParam& param_ = RpcParam())
        : script(std::move(script_)), finished(std::move(finished_)), identity(id), param(param_) {}
    Task(std::string function, const std::vector<ConstantSP>& args, std::shared_ptr<std::condition_variable> finished_ = nullptr, int id = 0,
         const RpcParam& param_ = RpcParam())
  ...
  ~DBConnectionPoolImpl() {
    shutDown();
    Task emptyTask;
    for(size_t i = 0; i < workers_.size(); i++) queue_->push(emptyTask);
    for(auto& work : workers_) {
      work->join();
    }
  }

  int run(std::string script, int id, std::shared_ptr<std::condition_variable> finished = nullptr, const RpcParam& param = RpcParam()) {
    taskStatus_.setResult(id, TaskResult());
    queue_->push(Task(std::move(script), std::move(finished), id, param));
    return id;
  }

  int run(std::string functionName, const std::vector<ConstantSP>& args, int id, std::shared_ptr<std::condition_variable> finished = nullptr,
  ...
  bool isFinished(int identity) { return taskStatus_.isFinished(identity); }

  ConstantSP getData(int identity) { return taskStatus_.getData(identity); }
  ...
  bool isShutDown() { return shutDownFlag_.load(); }

  int getConnectionCount() { return static_cast<int>(workers_.size()); }
