...
class OneRowInArrayVector {
 public:
  OneRowInArrayVector(const T *data, size_t length, bool isNeedCopy) : dataPtr_(data), length_(length) {
    if(isNeedCopy) {
      data_.resize(length);
...
class ResultSet {
 public:
  explicit ResultSet(const TableSP &table) : table_(table) {
    rows_ = table_->rows();
    ...
    for(int i = 0; i < cols_; i++) {
      ...
      switch(type) {
        case DT_BOOL:
          column_[i].charCol = new Column<char>(
              pVector, [=](const VectorSP &vector, INDEX position, int len, char *buf) { return vector->getBoolConst(position, len, buf); });
          break;
        case DT_CHAR:
          column_[i].charCol = new Column<char>(
              pVector, [=](const VectorSP &vector, INDEX position, int len, char *buf) { return vector->getCharConst(position, len, buf); });
          break;
        case DT_SHORT:
          column_[i].shortCol = new Column<short>(
              pVector, [=](const VectorSP &vector, INDEX position, int len, short *buf) { return vector->getShortConst(position, len, buf); });
          break;
        case DT_INT:
          if(pVector->getType() == DT_SYMBOL) {
            ...
            column_[i].intCol = new Column<int>(
                pVector, [=](const VectorSP &vector, INDEX position, int len, int *buf) { return vector->getIntConst(position, len, buf); });
          }
          break;
        case DT_LONG:
          column_[i].longCol = new Column<long long>(
              pVector, [=](const VectorSP &vector, INDEX position, int len, long long *buf) { return vector->getLongConst(position, len, buf); });
          break;
        case DT_FLOAT:
          column_[i].floatCol = new Column<float>(
              pVector, [=](const VectorSP &vector, INDEX position, int len, float *buf) { return vector->getFloatConst(position, len, buf); });
          break;
        case DT_DOUBLE:
          column_[i].doubleCol = new Column<double>(
              pVector, [=](const VectorSP &vector, INDEX position, int len, double *buf) { return vector->getDoubleConst(position, len, buf); });
          break;
        case DT_BLOB:
        case DT_STRING:
          column_[i].stringCol = new Column<std::string *>(pVector, [=](const VectorSP &vector, INDEX position, int len, std::string **buf) {
        ...
        case DT_DECIMAL64:
          ...
          break;
        case DT_DECIMAL128:
          column_[i].decimal128Col =
              new Column<wide_integer::int128>(pVector, [=](const VectorSP &vector, INDEX position, int len, wide_integer::int128 *buf) {
  ...
  ~ResultSet() { delete[] column_; }
  INDEX position() { return position_; }
  ...
  bool isFirst() { return position_ == 0; }
  bool last() {
    if(rows_ > 0) {
      position_ = rows_ - 1;
      return true;
  ...
  bool isLast() { return position_ + 1 == rows_; }
  bool isAfterLast() { return position_ >= rows_; }
  bool isBeforeFirst() { return position_ < 0; }
  DATA_TYPE getDataType(int col) { return table_->getColumnType(col); }
  ...
  const unsigned char *getBinary(int col) const {
    ...
    if(column.charCol != nullptr) return (unsigned char *)&column.charCol->getValue(position_);
    if(column.shortCol != nullptr) return (unsigned char *)&column.shortCol->getValue(position_);
    if(column.intCol != nullptr) return (unsigned char *)&column.intCol->getValue(position_);
    if(column.longCol != nullptr) return (unsigned char *)&column.longCol->getValue(position_);
    if(column.floatCol != nullptr) return (unsigned char *)&column.floatCol->getValue(position_);
    if(column.doubleCol != nullptr) return (unsigned char *)&column.doubleCol->getValue(position_);
    if(column.stringCol != nullptr) return (unsigned char *)column.stringCol->getValue(position_)->data();
    if(column.int128Col != nullptr) return (unsigned char *)&column.int128Col->getValue(position_);
    if(column.decimal32Col != nullptr) return (unsigned char *)&column.decimal32Col->getValue(position_);
    if(column.decimal64Col != nullptr) return (unsigned char *)&column.decimal64Col->getValue(position_);
    if(column.decimal128Col != nullptr) return (unsigned char *)&column.decimal128Col->getValue(position_);
    throw RuntimeException("This instance doesn't support getBinary.");
  }
  ConstantSP getObject(int col) const { return column_[col].pVector->get(position_); }

 private:
  template <class T>
  class Column {
   public:
    Column(const VectorSP &vector, std::function<const T *(const VectorSP &pVector, INDEX position, int len, T *buf)> getBufConst)
