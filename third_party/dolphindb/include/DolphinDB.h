...
class EXPORT_DECL DBConnection {
 public:
  explicit DBConnection(bool enableSSL = false, bool asyncTask = false, int keepAliveTime = 7200, bool compress = false, bool python = false,
                        bool isReverseStreaming = false, bool enableSCRAM = false);
  DBConnection(const std::string& host, int port, const std::string& userName = "", const std::string& password = "") : DBConnection(false) {
  ...
  bool connect(const std::string& hostName, int port, const std::string& userId = "", const std::string& password = "",
               const std::string& initialScript = "", bool highAvailability = false,
               const std::vector<std::string>& highAvailabilitySites = std::vector<std::string>(), int keepAliveTime = 7200, bool reconnect = false);

  bool checkVersion(const std::vector<VersionT>& vers) { return version_.check(vers); }
  ...
  /*
   * Internal function for stream subscription.
   * Server function prototype: publishTable(host, port, tableName, [actionName], [offset], [filter], [allowExists],
   * [resetOffset], [udpMulticast]) Server return value: columnNames for TCP or (columnNames, port) for UDP
   */
   ...
   */
  ConstantSP run(const std::string& funcName, std::vector<ConstantSP>& args, int priority = 4, int parallelism = 64, int fetchSize = 0,
 ...
 private:
  ...
  struct Node {
    ...
    bool isEqual(const Node& node) { return hostName_ == node.hostName_ && port_ == node.port_; }
    Node() = default;
...
class EXPORT_DECL DBConnectionPool {
 public:
  ...
  int run(std::string functionName, const std::vector<ConstantSP>& args, std::shared_ptr<std::condition_variable> finished = nullptr,
          const RpcParam& param = RpcParam());
  ...
  [[deprecated]] void run(const std::string& script, int identity, int priority = 4, int parallelism = 64, int fetchSize = 0,
                          bool clearMemory = false);
  [[deprecated]] void run(const std::string& functionName, const std::vector<ConstantSP>& args, int identity, int priority = 4, int parallelism = 64,
...
class EXPORT_DECL PartitionedTableAppender {
 public:
  ...
  PartitionedTableAppender(const std::string& dbUrl, const std::string& tableName, const std::string& partitionColName,
