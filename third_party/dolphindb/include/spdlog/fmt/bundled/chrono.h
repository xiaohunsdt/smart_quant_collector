...
struct is_same_arithmetic_type
    : public std::integral_constant<bool, (std::is_integral<Rep1>::value && std::is_integral<Rep2>::value) ||
...
auto to_time_t(std::chrono::time_point<std::chrono::system_clock, Duration> time_point) -> std::time_t {
  ...
  // overflow.
  return fmt_duration_cast<std::chrono::duration<std::time_t>>(time_point.time_since_epoch()).count();
}
}  // namespace detail
...
template <typename Duration>
inline auto gmtime(std::chrono::time_point<std::chrono::system_clock, Duration> time_point) -> std::tm {
...
struct formatter<std::chrono::time_point<std::chrono::system_clock, Duration>, Char> : formatter<std::tm, Char> {
  ...
  auto format(std::chrono::time_point<std::chrono::system_clock, Duration> val, FormatContext& ctx) const -> decltype(ctx.out()) {
    ...
    Duration subsecs = detail::fmt_duration_cast<Duration>(epoch - detail::fmt_duration_cast<std::chrono::seconds>(epoch));
    if(subsecs.count() < 0) {
...
struct formatter<std::chrono::local_time<Duration>, Char> : formatter<std::tm, Char> {
  ...
  auto format(std::chrono::local_time<Duration> val, FormatContext& ctx) const -> decltype(ctx.out()) {
    ...
    if(period::num != 1 || period::den != 1 || std::is_floating_point<typename Duration::rep>::value) {
      ...
      const auto subsecs = detail::fmt_duration_cast<Duration>(epoch - detail::fmt_duration_cast<std::chrono::seconds>(epoch));
