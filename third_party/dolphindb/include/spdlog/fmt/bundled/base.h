...
#if defined(__GXX_RTTI) || FMT_HAS_FEATURE(cxx_rtti) || defined(_CPPRTTI) || defined(__INTEL_RTTI__) || defined(__RTTI)
...
class basic_string_view {
 ...
 public:
  ...
  basic_string_view(const Char* s)
      : data_(s),
        size_(detail::const_check(std::is_same<Char, char>::value && !detail::is_constant_evaluated(false)) ? strlen(reinterpret_cast<const char*>(s))
                                                                                                            : detail::length(s)) {}
...
FMT_DEPRECATED FMT_NORETURN inline void throw_format_error(const char* message) { report_error(message); }
...
class compile_parse_context : public basic_format_parse_context<Char> {
 ...
 public:
  explicit FMT_CONSTEXPR compile_parse_context(basic_string_view<Char> format_str, int num_args, const type* types, int next_arg_id = 0)
...
class iterator_buffer<OutputIt,
                      enable_if_t<detail::is_back_insert_iterator<OutputIt>::value && is_contiguous<typename OutputIt::container_type>::value,
                                  typename OutputIt::container_type::value_type>> : public buffer<typename OutputIt::container_type::value_type> {
 private:
...
class value {
 public:
  ...
  FMT_CONSTEXPR20 FMT_ALWAYS_INLINE value(T& val) {
    ...
    if constexpr(std::is_same<decltype(&val), T*>::value) custom.value = const_cast<value_type*>(&val);
#endif
    if(!is_constant_evaluated()) custom.value = const_cast<char*>(&reinterpret_cast<const char&>(val));
...
struct arg_mapper {
  ...
  FMT_MAP_API auto map(volatile void* val) -> const void* { return const_cast<const void*>(val); }
  FMT_MAP_API auto map(const volatile void* val) -> const void* { return const_cast<const void*>(val); }
  ...
  template <typename T, typename U = remove_const_t<T>,
            FMT_ENABLE_IF((std::is_class<U>::value || std::is_enum<U>::value || std::is_union<U>::value || std::is_fundamental<U>::value) &&
                          !has_to_string_view<U>::value && !is_char<U>::value && !is_named_arg<U>::value && !std::is_integral<U>::value &&
...
struct is_output_iterator<It, T, void_t<decltype(*std::declval<It&>()++ = std::declval<T>())>> : std::true_type {};
...
struct format_arg_store {
  ...
  template <typename... T>
  FMT_MAP_API format_arg_store(T&... values) : args{{named_args, NUM_NAMED_ARGS}, make_arg<NUM_ARGS <= max_packed_args, Context>(values)...} {
    using dummy = int[];
...
class basic_format_args {
 ...
 public:
  ...
  constexpr basic_format_args(const dynamic_format_arg_store<Context>& store) : desc_(store.get_types()), args_(store.data()) {}
...
class context {
 ...
 public:
  ...
  FMT_CONSTEXPR context(iterator out, basic_format_args<context> ctx_args, detail::locale_ref loc = {}) : out_(out), args_(ctx_args), loc_(loc) {}
...
using is_formattable =
    bool_constant<!std::is_base_of<detail::unformattable, decltype(detail::arg_mapper<buffered_context<Char>>().map(std::declval<T&>()))>::value>;
...
template <typename Context = format_context, typename... T, size_t NUM_ARGS = sizeof...(T), size_t NUM_NAMED_ARGS = detail::count_named_args<T...>(),
          unsigned long long DESC = detail::make_descriptor<Context, T...>(), FMT_ENABLE_IF(NUM_NAMED_ARGS == 0)>
...
#ifndef FMT_DOC
template <typename Context = format_context, typename... T, size_t NUM_NAMED_ARGS = detail::count_named_args<T...>(),
          unsigned long long DESC = detail::make_descriptor<Context, T...>() | static_cast<unsigned long long>(detail::has_named_args_bit),
...
template <typename Char>
using unsigned_char = typename conditional_t<std::is_integral<Char>::value, std::make_unsigned<Char>, type_identity<unsigned>>::type;
...
struct format_specs {
  ...
  constexpr format_specs()
      : width(0), precision(-1), type(presentation_type::none), align(align::none), sign(sign::none), upper(false), alt(false), localized(false) {}
};
...
template <typename Char>
FMT_CONSTEXPR auto parse_dynamic_spec(const Char* begin, const Char* end, int& value, arg_ref<Char>& ref,
...
template <typename Char>
FMT_CONSTEXPR auto parse_precision(const Char* begin, const Char* end, int& value, arg_ref<Char>& ref,
                                   basic_format_parse_context<Char>& ctx) -> const Char* {
  ++begin;
  if(begin == end || *begin == '}') {
...
template <typename Char>
FMT_CONSTEXPR auto parse_format_specs(const Char* begin, const Char* end, dynamic_format_specs<Char>& specs, basic_format_parse_context<Char>& ctx,
...
FMT_CONSTEXPR inline auto check_char_specs(const format_specs& specs) -> bool {
  ...
  if(specs.align == align::numeric || specs.sign != sign::none || specs.alt) report_error("invalid format specifier for char");
  return true;
}
...
class basic_format_string {
 ...
 public:
  template <typename S, FMT_ENABLE_IF(std::is_convertible<const S&, basic_string_view<Char>>::value ||
                                      (detail::is_compile_string<S>::value && std::is_constructible<basic_string_view<Char>, const S&>::value))>
  FMT_CONSTEVAL FMT_ALWAYS_INLINE basic_format_string(const S& s) : str_(s) {
    static_assert(detail::count<(std::is_base_of<detail::view, remove_reference_t<Args>>::value && std::is_reference<Args>::value)...>() == 0,
...
FMT_INLINE void println(FILE* f, format_string<T...> fmt, T&&... args) {
  const auto& vargs = fmt::make_format_args(args...);
  return detail::use_utf8() ? vprintln(f, fmt, vargs) : detail::vprint_mojibake(f, fmt, vargs, true);
}
