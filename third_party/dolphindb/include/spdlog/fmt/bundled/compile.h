...
FMT_CONSTEXPR inline auto copy(InputIt begin, InputIt end, counting_iterator it) -> counting_iterator {
  return it + (end - begin);
}
    ...
    if constexpr(str[POS + 1] == '{') {
      ...
      if constexpr(arg_id_result.arg_id.kind == arg_id_kind::index) {
        ...
        if constexpr(arg_index >= 0) {
          ...
          return parse_tail<Args, arg_id_end_pos + 1, ID>(runtime_named_field<char_type>{arg_id_result.arg_id.val.name}, fmt);
        } else if constexpr(c == ':') {
