...
class EXPORT_DECL Util {
 ...
 public:
  ...
  static Table* createTable(const std::vector<std::string>& colNames, const std::vector<DATA_TYPE>& colTypes, INDEX size, INDEX capacity,
                            const std::vector<int>& extraParams = {});
  ...
  static Vector* createVector(DATA_TYPE type, INDEX size, INDEX capacity = 0, bool fast = true, int extraParam = 0, void* data = nullptr,
                              bool containNull = false);
  static Vector* createArrayVector(const VectorSP& index, const VectorSP& value);
  static Vector* createArrayVector(DATA_TYPE type, INDEX size, INDEX capacity = 0, bool fast = true, int extraParam = 0, void* data = nullptr,
                                   INDEX* pindex = nullptr, bool containNull = false);
  static Vector* createMatrix(DATA_TYPE type, int cols, int rows, int colCapacity, int extraParam = 0, void* data = nullptr,
                              bool containNull = false);
   ...
   * littleEndian: if true, the first byte is the least significant, i.e. the leftmost characters would be converted to
   * the rightmost byte. data: the length of buffer must be at least len/2
   */
  ...
  static Vector* createSymbolVector(const SymbolBaseSP& symbolBase, INDEX size, INDEX capacity = 0, bool fast = true, void* data = nullptr,
  ...
  static bool setValue(ConstantSP& data, DATA_TYPE dataType, const std::vector<std::nullptr_t>& val, ErrorCodeInfo& errorCodeInfo,
                       int extraParam = 0);
  ...
  static bool setValue(ConstantSP& data, DATA_TYPE dataType, const std::vector<const unsigned char*>& val, ErrorCodeInfo& errorCodeInfo,
                       int extraParam = 0);
  ...
  static ConstantSP createObject(DATA_TYPE dataType, const std::vector<std::nullptr_t>& val, ErrorCodeInfo* errorCodeInfo = nullptr,
                                 int extraParam = 0);
  ...
  static ConstantSP createObject(DATA_TYPE dataType, const std::vector<const unsigned char*>& val, ErrorCodeInfo* errorCodeInfo = nullptr,
                                 int extraParam = 0);
  ...
  static void enumBoolVector(const VectorSP& pVector, const std::function<bool(const char* pbuf, INDEX startIndex, INDEX size)>& func,
                             INDEX offset = 0) {
    enumDdbVector<char>(pVector, &Vector::getBoolConst, func, offset);
  }
  static void enumIntVector(const VectorSP& pVector, const std::function<bool(const int* pbuf, INDEX startIndex, int length)>& func,
                            INDEX offset = 0) {
    enumDdbVector<int>(pVector, &Vector::getIntConst, func, offset);
  }
  static void enumShortVector(const VectorSP& pVector, const std::function<bool(const short* pbuf, INDEX startIndex, int length)>& func,
                              INDEX offset = 0) {
    enumDdbVector<short>(pVector, &Vector::getShortConst, func, offset);
  }
  static void enumCharVector(const VectorSP& pVector, const std::function<bool(const char* pbuf, INDEX startIndex, int length)>& func,
                             INDEX offset = 0) {
    enumDdbVector<char>(pVector, &Vector::getCharConst, func, offset);
  }
  static void enumLongVector(const VectorSP& pVector, const std::function<bool(const long long* pbuf, INDEX startIndex, int length)>& func,
                             INDEX offset = 0) {
    enumDdbVector<long long>(pVector, &Vector::getLongConst, func, offset);
  }
  static void enumFloatVector(const VectorSP& pVector, const std::function<bool(const float* pbuf, INDEX startIndex, int length)>& func,
                              INDEX offset = 0) {
    enumDdbVector<float>(pVector, &Vector::getFloatConst, func, offset);
  }
  static void enumDoubleVector(const VectorSP& pVector, const std::function<bool(const double* pbuf, INDEX startIndex, int length)>& func,
                               INDEX offset = 0) {
    enumDdbVector<double>(pVector, &Vector::getDoubleConst, func, offset);
  }
  static void enumStringVector(const VectorSP& pVector, const std::function<bool(std::string** pbuf, INDEX startIndex, int length)>& func,
                               INDEX offset = 0) {
    ...
    while(leftSize > 0) {
      ...
      if(func(pbuf, startIndex, size) == false) break;
      leftSize -= size;
      startIndex += size;
    }
  }
  static void enumInt128Vector(const VectorSP& pVector, const std::function<bool(const Guid* pbuf, INDEX startIndex, int length)>& func,
                               INDEX offset = 0) {
    enumBinaryVector<Guid>(pVector, func, offset);
  }
  static void enumDecimal32Vector(const VectorSP& pVector, const std::function<bool(const int32_t* pbuf, INDEX startIndex, int length)>& func,
                                  INDEX offset = 0) {
    enumBinaryVector<int32_t>(pVector, func, offset);
  }
  static void enumDecimal64Vector(const VectorSP& pVector, const std::function<bool(const int64_t* pbuf, INDEX startIndex, int length)>& func,
                                  INDEX offset = 0) {
    enumBinaryVector<int64_t>(pVector, func, offset);
  }
  static void enumDecimal128Vector(const VectorSP& pVector,
                                   const std::function<bool(const wide_integer::int128* pbuf, INDEX startIndex, int length)>& func,
                                   INDEX offset = 0) {
 ...
 private:
  template <class T>
  static void enumBinaryVector(const VectorSP& pVector, const std::function<bool(const T* pbuf, INDEX startIndex, int length)>& func,
                               INDEX offset = 0) {
  ...
  template <class T>
  static void enumDdbVector(const VectorSP& pVector, const T* (Vector::*getConst)(INDEX, int, T*) const,
...
template <>
inline char getNullValue() {
  return CHAR_MIN;
}
template <>
inline short getNullValue() {
  return SHRT_MIN;
}
template <>
inline int getNullValue() {
  return INT_MIN;
}
template <>
inline int64_t getNullValue() {
  return LLONG_MIN;
}
#ifdef __linux__
template <>
inline long long getNullValue() {
  return LLONG_MIN;
}
#endif
template <>
inline float getNullValue() {
  return FLT_NMIN;
}
template <>
...
template <>
inline std::string getNullValue() {
  return "";
}
template <>
inline Guid getNullValue() {
  return Guid();
}
template <>
inline wide_integer::int128 getNullValue() {
