...
class EXPORT_DECL MultithreadedTableWriter {
 public:
  ...
  MultithreadedTableWriter(const std::string &host, int port, const std::string &userId, const std::string &password, const std::string &dbPath,
                           const std::string &tableName, bool useSSL, bool enableHighAvailability = false,
                           const std::vector<std::string> *pHighAvailabilitySites = nullptr, int batchSize = 1, float throttle = 0.01F,
                           int threadCount = 1, const std::string &partitionCol = "", const std::vector<COMPRESS_METHOD> *pCompressMethods = nullptr,
                           Mode mode = M_Append, std::vector<std::string> *pModeOption = nullptr,
                           const std::function<void(ConstantSP)> &callbackFunc = nullptr, bool enableStreamTableTimestamp = false);
  ...
  bool insertUnwrittenData(std::vector<std::vector<ConstantSP> *> &records, ErrorCodeInfo &errorInfo) {
    return insert(records.data(), static_cast<int>(records.size()), errorInfo, true);
  }
 ...
 private:
  ...
  DATA_TYPE getColDataType(int colIndex) {
    ...
    if(dataType >= ARRAY_TYPE_BASE) dataType = (DATA_TYPE)(dataType - ARRAY_TYPE_BASE);
    return dataType;
  }
  ...
  class SendExecutor : public dolphindb::Runnable {
   public:
    SendExecutor(MultithreadedTableWriter &tableWriter, WriterThread &writeThread) : tableWriter_(tableWriter), writeThread_(writeThread){};
    void run() override;
