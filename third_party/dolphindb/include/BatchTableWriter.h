...
class EXPORT_DECL BatchTableWriter {
 public:
  ...
  void insert(const std::string& dbName, const std::string& tableName, Targs... Fargs) {
    ...
    if(destTable->destroy) throw RuntimeException("Failed to insert into table, the table is being removed.");
 ...
 private:
  ...
  void insertRecursive(std::vector<ConstantSP>* row, DestTable* destTable, int colIndex) {
    ...
    if(destTable->finished) {
      throw RuntimeException(std::string("Failed to insert data. Error writing data in backgroud thread. Please use "
                                         "getUnwrittenData to get data not written to server and remove talbe (") +
