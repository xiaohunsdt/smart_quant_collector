...
class EXPORT_DECL StreamReplicator {
 public:
  explicit StreamReplicator(const std::vector<HostInfo> &hosts, const std::string &tableName, const ReplicatorConfig &config = ReplicatorConfig());
  ...
  void insert(TArgs... args) {
    ...
    int idx2 = 0;
    int idx3 = 0;
    bool results[] = {Util::setValue(oneRowData_[idx1++], getColDataType(idx2++), args, errorInfo, colExtras_[idx3++])...};
 ...
 private:
  ...
  BlockIt flush(const std::shared_ptr<DBConnection> &conn, ReplicatorStreamStatus &status, BlockIt front, bool flushAll);
