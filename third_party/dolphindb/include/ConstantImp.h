...
class AnyVector : public Vector {
 public:
  explicit AnyVector(int sz) : data_(sz, Constant::void_), containNull_(false) {}
  ...
  size_t getUnitLength() const override { return 0; }
  void clear() override;
  ...
  INDEX reserve(INDEX capacity) { return (INDEX)data_.size(); };
  ...
  bool getString(INDEX start, int len, std::string** buf) const override { throw RuntimeException("getString method not supported for AnyVector"); }

  bool getString(INDEX start, int len, char** buf) const override { throw RuntimeException("getString method not supported for AnyVector"); }
  ...
  void reverse(INDEX start, INDEX length) override { std::reverse(data_.begin() + start, data_.begin() + start + length); }
  void replace(const ConstantSP& oldVal, const ConstantSP& newVal) override { throw RuntimeException("replace method not supported for AnyVector"); }
...
class AbstractFastVector : public Vector {
 public:
  AbstractFastVector(int sz, int capacity, T* srcData, T nullVal, bool containNull)
      : nullVal_(nullVal), size_(sz), capacity_(capacity), containNull_(containNull) {
    if(capacity < sz) capacity_ = sz;
    data_ = srcData;
  }
  ~AbstractFastVector() override { delete[] data_; }
  ...
  ConstantSP getSubVector(INDEX start, INDEX length) const override {
    if(start < 0 || length < 0 || start + length > size_) {
      throw IllegalArgumentException("getSubVector", "index out of range: start=" + std::to_string(start) + ", length=" + std::to_string(length));
    }
  ...
  ConstantSP getSubVector(INDEX start, INDEX length, INDEX capacity) const override {
    ...
    if(data) return Util::createVector(type, std::abs(length), capacity, true, getExtraParamForType(), data, containNull_);
    ...
    if(!in->isIntegerReversed()) {
      ...
      for(; i < end; ++i) {
        if((ret = in->readBytes((char*)(data_ + i), unitLength, true)) != OK) {
          ...
          if(!containNull_) containNull_ = hasNullInRange(indexStart, i);
          return ret;
        }
      }
    }
  ...
  void initialize() override { memset((void*)data_, 0, sizeof(T) * size_); }

  bool getNullFlag() const override { return containNull_; }
  void setNullFlag(bool containNull) override { containNull_ = containNull; }

  bool hasNull() override { return hasNullInRange(0, size_); }
  ...
  bool remove(const ConstantSP& index) override {
    ...
    for(INDEX k = prevIndex + 1; k < size_; ++k) data_[cursor++] = data_[k];
  ...
  void next(INDEX steps) override {
    ...
    for(int i = size_ - steps; i < size_; ++i) data_[i] = nullVal_;
    containNull_ = true;
  }
  void prev(INDEX steps) override {
    if(steps > size_ || steps < 0) return;
    memmove(data_ + steps, data_, (size_ - steps) * sizeof(T));
    for(int i = 0; i < steps; ++i) data_[i] = nullVal_;
    containNull_ = true;
  }
  ...
  bool isNull(INDEX start, int len, char* buf) const override {
    if(containNull_) {
      for(int i = 0; i < len; ++i) buf[i] = (data_[start + i] == nullVal_);
    } else {
      memset(buf, 0, len);
    }
    return true;
  ...
  bool getBool(INDEX start, int len, char* buf) const override { return copyData<char>(start, len, DT_BOOL, CHAR_MIN, buf); }

  const char* getBoolConst(INDEX start, int len, char* buf) const override { return getDataConst<char>(start, len, DT_BOOL, CHAR_MIN, buf); }

  char* getBoolBuffer(INDEX start, int len, char* buf) const override { return getDataBuffer<char>(start, len, DT_BOOL, buf); }

  bool getChar(INDEX start, int len, char* buf) const override { return copyData<char>(start, len, DT_CHAR, CHAR_MIN, buf); }

  const char* getCharConst(INDEX start, int len, char* buf) const override { return getDataConst<char>(start, len, DT_CHAR, CHAR_MIN, buf); }

  char* getCharBuffer(INDEX start, int len, char* buf) const override { return getDataBuffer<char>(start, len, DT_CHAR, buf); }

  bool getShort(INDEX start, int len, short* buf) const override { return copyData<short>(start, len, DT_SHORT, SHRT_MIN, buf); }

  const short* getShortConst(INDEX start, int len, short* buf) const override { return getDataConst<short>(start, len, DT_SHORT, SHRT_MIN, buf); }

  short* getShortBuffer(INDEX start, int len, short* buf) const override { return getDataBuffer<short>(start, len, DT_SHORT, buf); }
  ...
  bool getIndex(INDEX start, int len, INDEX* buf) const override { return copyData<INDEX>(start, len, DT_INDEX, INDEX_MIN, buf); }

  const INDEX* getIndexConst(INDEX start, int len, INDEX* buf) const override { return getDataConst<INDEX>(start, len, DT_INDEX, INDEX_MIN, buf); }

  INDEX* getIndexBuffer(INDEX start, int len, INDEX* buf) const override { return getDataBuffer<INDEX>(start, len, DT_INDEX, buf); }

  bool getFloat(INDEX start, int len, float* buf) const override { return copyData<float>(start, len, DT_FLOAT, FLT_NMIN, buf); }

  const float* getFloatConst(INDEX start, int len, float* buf) const override { return getDataConst<float>(start, len, DT_FLOAT, FLT_NMIN, buf); }

  float* getFloatBuffer(INDEX start, int len, float* buf) const override { return getDataBuffer<float>(start, len, DT_FLOAT, buf); }

  bool getDouble(INDEX start, int len, double* buf) const override { return copyData<double>(start, len, DT_DOUBLE, DBL_NMIN, buf); }

  const double* getDoubleConst(INDEX start, int len, double* buf) const override {
    return getDataConst<double>(start, len, DT_DOUBLE, DBL_NMIN, buf);
  }

  double* getDoubleBuffer(INDEX start, int len, double* buf) const override { return getDataBuffer<double>(start, len, DT_DOUBLE, buf); }

  void* getDataBuffer(INDEX start, int len, void* buf) const override { return (T*)(data_ + start); }

  using Vector::getString;
  virtual bool getString(INDEX start, int len, std::string* buf) const {
    for(int i = 0; i < len; ++i) buf[i] = getString(i);
    return true;
  }

  bool setBool(INDEX start, int len, const char* buf) override { return setData<char>(start, len, DT_BOOL, CHAR_MIN, buf); }

  bool setChar(INDEX start, int len, const char* buf) override { return setData<char>(start, len, DT_CHAR, CHAR_MIN, buf); }

  bool setShort(INDEX start, int len, const short* buf) override { return setData<short>(start, len, DT_SHORT, SHRT_MIN, buf); }
  ...
  bool setIndex(INDEX start, int len, const INDEX* buf) override { return setData<INDEX>(start, len, DT_INDEX, INDEX_MIN, buf); }

  bool setFloat(INDEX start, int len, const float* buf) override { return setData<float>(start, len, DT_FLOAT, FLT_NMIN, buf); }

  bool setDouble(INDEX start, int len, const double* buf) override { return setData<double>(start, len, DT_DOUBLE, DBL_NMIN, buf); }

  bool setData(INDEX start, int len, void* buf) override {
    if(buf != (void*)(data_ + start)) memcpy(data_ + start, buf, sizeof(T) * len);
    return true;
  }

  bool appendBool(char* buf, int len) override { return appendData<char>(buf, len, DT_BOOL, CHAR_MIN); }

  bool appendChar(char* buf, int len) override { return appendData<char>(buf, len, DT_CHAR, CHAR_MIN); }

  bool appendShort(short* buf, int len) override { return appendData<short>(buf, len, DT_SHORT, SHRT_MIN); }
  ...
  bool appendFloat(float* buf, int len) override { return appendData<float>(buf, len, DT_FLOAT, FLT_NMIN); }

  bool appendDouble(double* buf, int len) override { return appendData<double>(buf, len, DT_DOUBLE, DBL_NMIN); }
  ...
  bool add(INDEX start, INDEX length, long long inc) override {
    ...
    if(containNull_) {
      for(INDEX i = start; i < length; ++i)
        if(data_[i] != nullVal_) data_[i] += incVal;
    } else {
      for(INDEX i = start; i < length; ++i) data_[i] += incVal;
    }
    return true;
  }
  ...
  void addIndex(INDEX start, INDEX length, INDEX offset) override {
    ...
    for(INDEX i = 0; i < length; ++i)
      if(data_[i + start] >= 0) data_[i + start] += incVal;
  }
  ...
  void neg() override {
    ...
    if(containNull_) {
      for(int i = 0; i < len; ++i)
        if(data_[i] != nullVal_) data_[i] = -data_[i];
    } else {
      for(int i = 0; i < len; ++i) data_[i] = -data_[i];
    }
  }
  ...
  void replace(const ConstantSP& oldVal, const ConstantSP& newVal) override {
    ...
    for(int i = 0; i < size_; ++i)
      if(data_[i] == ov) data_[i] = nv;
  }

  void reverse() override { reverse(0, size_ - 1); }
  ...
  using Vector::getAllocatedMemory;
  long long getAllocatedMemory() const override { return sizeof(T) * (capacity_ == 0 ? size_ : capacity_); }
 ...
 protected:
  void replaceNull(T rep) {
    for(int i = 0; i < size_; ++i)
      if(data_[i] == nullVal_) data_[i] = rep;
    containNull_ = false;
  }

  template <typename Y>
  bool copyData(int start, int len, DATA_TYPE targetType, Y targetNullVal, Y* buf) const {
    if(getType() == targetType)
      memcpy(buf, data_ + start, sizeof(Y) * len);
    else if(containNull_) {
      if(targetType == DT_BOOL) {
        for(int i = 0; i < len; ++i) buf[i] = (data_[start + i] == nullVal_) ? targetNullVal : (bool)data_[start + i];
      } else {
        for(int i = 0; i < len; ++i) buf[i] = (data_[start + i] == nullVal_) ? targetNullVal : static_cast<Y>(data_[start + i]);
      }
    } else {
      if(targetType == DT_BOOL) {
        for(int i = 0; i < len; ++i) buf[i] = data_[start + i] != 0;
      } else {
        for(int i = 0; i < len; ++i) buf[i] = static_cast<Y>(data_[start + i]);
      }
    }
    return true;
  }
  ...
  template <typename Y>
  const Y* getDataConst(int start, int len, DATA_TYPE targetType, Y targetNullVal, Y* buf) const {
    if(getType() == targetType) return (Y*)data_ + start;
    if(containNull_)
      if(targetType == DT_BOOL) {
        for(int i = 0; i < len; ++i) buf[i] = (data_[start + i] == nullVal_) ? targetNullVal : (bool)data_[start + i];
      } else {
      ...
      if(targetType == DT_BOOL) {
        for(int i = 0; i < len; ++i) buf[i] = (bool)data_[start + i];
      } else {
        for(int i = 0; i < len; ++i) buf[i] = static_cast<Y>(data_[start + i]);
      }
    }
  ...
  template <typename Y>
  Y* getDataBuffer(int start, int len, DATA_TYPE targetType, Y* buf) const {
    if(getType() == targetType) return (Y*)data_ + start;
    return buf;
  }

  template <typename Y>
  bool setData(int start, int len, DATA_TYPE sourceType, Y sourceNullVal, const Y* buf) {
    if(buf == (Y*)data_ + start) return true;
    if(getType() == sourceType)
      memcpy(reinterpret_cast<void*>(data_ + start), reinterpret_cast<const void*>(buf), sizeof(Y) * len);
    else
      for(int i = 0; i < len; ++i) data_[start + i] = (buf[i] == sourceNullVal) ? nullVal_ : static_cast<T>(buf[i]);
    return true;
  }

  template <typename Y>
  bool appendData(Y* buf, int len, DATA_TYPE sourceType, Y sourceNullVal) {
    checkCapacity(len);
    if(getType() == sourceType)
      memcpy(reinterpret_cast<void*>(data_ + size_), reinterpret_cast<const void*>(buf), sizeof(Y) * len);
    else
      for(int i = 0; i < len; ++i) data_[size_ + i] = (buf[i] == sourceNullVal) ? nullVal_ : static_cast<T>(buf[i]);
    size_ += len;
    return true;
  }
  ...
  T* getDataArray(const Vector* indexVector, bool& haveNull) const {
    ...
    if(containNull_) {
      ...
      if(indexVector->isIndexArray()) {
        ...
        for(INDEX i = 0; i < len; ++i) buf[i] = bufIndex[i] < sz ? data_[bufIndex[i]] : nullVal_;
      } else {
        ...
        while(start < len) {
          ...
          for(i = 0; i < count; ++i) buf[start + i] = pbufIndex[i] < sz ? data_[pbufIndex[i]] : nullVal_;
          start += count;
        }
      }
    } else {
  ...
  T* getDataArray(INDEX start, INDEX length) const { return getDataArray(start, length, std::abs(length)); }
  ...
  using iterator = typename std::unordered_map<T, INDEX>::const_iterator;
  T* data_;
  T nullVal_;
  int size_;
  int capacity_;
...
class FastVoidVector : public AbstractFastVector<char> {
 public:
  ...
  ~FastVoidVector() override = default;
...
class FastBoolVector : public AbstractFastVector<char> {
 public:
  ...
  ~FastBoolVector() override = default;
...
class FastCharVector : public AbstractFastVector<char> {
 public:
  ...
  ~FastCharVector() override = default;
  ...
  bool getHash(INDEX start, int len, int buckets, int* buf) const override {
    for(int i = 0; i < len; ++i) buf[i] = (data_[start + i] == CHAR_MIN) ? -1 : (((unsigned int)data_[start + i]) % buckets);
    return true;
  }
  void upper() override;
  void lower() override;
...
class FastShortVector : public AbstractFastVector<short> {
 public:
  ...
  ~FastShortVector() override = default;
  ...
  int compare(INDEX index, const ConstantSP& target) const override;
  bool getHash(INDEX start, int len, int buckets, int* buf) const override {
    for(int i = 0; i < len; ++i) buf[i] = (data_[start + i] == SHRT_MIN) ? -1 : (((unsigned int)data_[start + i]) % buckets);
    return true;
  }
};
...
class FastIntVector : public AbstractFastVector<int> {
 public:
  ...
  ~FastIntVector() override = default;
  ...
  int compare(INDEX index, const ConstantSP& target) const override;
  bool getHash(INDEX start, int len, int buckets, int* buf) const override {
    for(int i = 0; i < len; ++i) buf[i] = (data_[start + i] == INT_MIN) ? -1 : (((uint32_t)data_[start + i]) % buckets);
    return true;
  }
};
...
class FastLongVector : public AbstractFastVector<long long> {
 public:
  ...
  ~FastLongVector() override = default;
  ...
  int compare(INDEX index, const ConstantSP& target) const override;
  bool getHash(INDEX start, int len, int buckets, int* buf) const override {
    for(int i = 0; i < len; ++i) buf[i] = (data_[start + i] == LLONG_MIN) ? -1 : (((uint64_t)data_[start + i]) % buckets);
    return true;
  }
};
...
class FastFloatVector : public AbstractFastVector<float> {
 public:
  ...
  ~FastFloatVector() override = default;
  ...
  bool getChar(INDEX start, int len, char* buf) const override { return copyData<char>(start, len, CHAR_MIN, buf); }
  const char* getCharConst(INDEX start, int len, char* buf) const override {
    copyData<char>(start, len, CHAR_MIN, buf);
    return buf;
  }
  bool getShort(INDEX start, int len, short* buf) const override { return copyData<short>(start, len, SHRT_MIN, buf); }
  const short* getShortConst(INDEX start, int len, short* buf) const override {
    copyData<short>(start, len, SHRT_MIN, buf);
    return buf;
  }
  bool getInt(INDEX start, int len, int* buf) const override { return copyData<int>(start, len, INT_MIN, buf); }
  const int* getIntConst(INDEX start, int len, int* buf) const override {
    copyData<int>(start, len, INT_MIN, buf);
    return buf;
  }
  bool getLong(INDEX start, int len, long long* buf) const override { return copyData<long long>(start, len, LLONG_MIN, buf); }
  const long long* getLongConst(INDEX start, int len, long long* buf) const override {
 ...
 private:
  template <typename Y>
  Y getData(INDEX index, Y targetNullVal) const {
    float x = data_[index];
    return (x == nullVal_) ? targetNullVal : (x < 0 ? (Y)(x - 0.5) : (Y)(x + 0.5));
  }
  template <typename Y>
  bool copyData(INDEX start, int len, Y targetNullVal, Y* buf) const {
    if(containNull_)
      for(int i = 0; i < len; ++i) {
...
class FastDoubleVector : public AbstractFastVector<double> {
 public:
  ...
  ~FastDoubleVector() override = default;
  ...
  void fill(INDEX start, INDEX length, const ConstantSP& value) override;
  bool append(const ConstantSP& value, INDEX appendSize) override;
  bool append(const ConstantSP& value, INDEX start, INDEX /*length*/) override;
  int compare(INDEX index, const ConstantSP& target) const override;
...
class FastTemporalVector : public FastIntVector {
 public:
  ...
  ~FastTemporalVector() override = default;
...
class FastArrayVector : public Vector {
 public:
  FastArrayVector(int sz, int capacity, char* srcData, bool containNull, DATA_TYPE dataType, INDEX* pindex = nullptr, int extraParam = 0) {
  ...
  FastArrayVector(const VectorSP& index, const VectorSP& value) {
    ...
    rowsRead_ = 1;
    size_ = index->size();
    if(!index->isFastMode()) throw RuntimeException("The index vector for the constructor of FastArrayVector must be a regular vector.");
  ...
  ~FastArrayVector() override = default;
  ...
  size_t getUnitLength() const override { return value_->getUnitLength(); }
  ...
  ConstantSP getSourceValue() { return (ConstantSP)value_; }
  ConstantSP getSourceIndex() { return (ConstantSP)index_; }
  ...
  INDEX reserve(INDEX capacity) { return size_; };
  ...
  INDEX checkVectorSize();  //<0: The vectors in the array vector are of different size. >=0: The vectors are of the
                            // same size {x}.
  VectorSP getFlatValueArray() { return value_; }
  void reserveValue(INDEX capacity) {
    if(capacity > value_->getCapacity()) {
      value_->reserve(capacity);
    }
  }
  void reserveIndex(INDEX capacity) { index_->reserve(capacity); }
...
class FastDateVector : public FastTemporalVector {
 public:
  ...
  ~FastDateVector() override = default;
...
class FastDateTimeVector : public FastTemporalVector {
 public:
  ...
  ~FastDateTimeVector() override = default;
...
class FastDateHourVector : public FastTemporalVector {
 public:
  ...
  ~FastDateHourVector() override = default;
...
class FastMonthVector : public FastTemporalVector {
 public:
  ...
  ~FastMonthVector() override = default;
  ...
  ConstantSP castTemporal(DATA_TYPE expectType) override {
    if(expectType == DT_MONTH) return getValue();
...
class FastTimeVector : public FastTemporalVector {
 public:
  ...
  ~FastTimeVector() override = default;
...
class FastMinuteVector : public FastTemporalVector {
 public:
  ...
  ~FastMinuteVector() override = default;
...
class FastSecondVector : public FastTemporalVector {
 public:
  ...
  ~FastSecondVector() override = default;
...
class FastNanoTimeVector : public FastLongVector {
 public:
  ...
  ~FastNanoTimeVector() override = default;
...
class FastTimestampVector : public FastLongVector {
 public:
  ...
  ~FastTimestampVector() override = default;
...
class FastNanoTimestampVector : public FastLongVector {
 public:
  ...
  ~FastNanoTimestampVector() override = default;
...
class FastBoolMatrix : public Matrix, public FastBoolVector {
 public:
  FastBoolMatrix(int colNum, int rowNum, int colCapacity, char* data, bool containNull)
      : Matrix(colNum, rowNum), FastBoolVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastBoolMatrix() override = default;
  ConstantSP getValue() const override;
  ...
  bool setColumn(INDEX index, const ConstantSP& value) override {
...
class FastCharMatrix : public Matrix, public FastCharVector {
 public:
  FastCharMatrix(int colNum, int rowNum, int colCapacity, char* data, bool containNull)
      : Matrix(colNum, rowNum), FastCharVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastCharMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
...
class FastShortMatrix : public Matrix, public FastShortVector {
 public:
  FastShortMatrix(int colNum, int rowNum, int colCapacity, short* data, bool containNull)
      : Matrix(colNum, rowNum), FastShortVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastShortMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
...
class FastIntMatrix : public Matrix, public FastIntVector {
 public:
  FastIntMatrix(int colNum, int rowNum, int colCapacity, int* data, bool containNull)
      : Matrix(colNum, rowNum), FastIntVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastIntMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
...
class FastLongMatrix : public Matrix, public FastLongVector {
 public:
  FastLongMatrix(int colNum, int rowNum, int colCapacity, long long* data, bool containNull)
      : Matrix(colNum, rowNum), FastLongVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastLongMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
...
class FastFloatMatrix : public Matrix, public FastFloatVector {
 public:
  FastFloatMatrix(int colNum, int rowNum, int colCapacity, float* data, bool containNull)
      : Matrix(colNum, rowNum), FastFloatVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastFloatMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
...
class FastDoubleMatrix : public Matrix, public FastDoubleVector {
 public:
  FastDoubleMatrix(int colNum, int rowNum, int colCapacity, double* data, bool containNull)
      : Matrix(colNum, rowNum), FastDoubleVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastDoubleMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
...
class FastDateMatrix : public Matrix, public FastDateVector {
 public:
  FastDateMatrix(int colNum, int rowNum, int colCapacity, int* data, bool containNull)
      : Matrix(colNum, rowNum), FastDateVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastDateMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastDateVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastDateTimeMatrix : public Matrix, public FastDateTimeVector {
 public:
  FastDateTimeMatrix(int colNum, int rowNum, int colCapacity, int* data, bool containNull)
      : Matrix(colNum, rowNum), FastDateTimeVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastDateTimeMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastDateTimeVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastDateHourMatrix : public Matrix, public FastDateHourVector {
 public:
  FastDateHourMatrix(int colNum, int rowNum, int colCapacity, int* data, bool containNull)
      : Matrix(colNum, rowNum), FastDateHourVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastDateHourMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastDateHourVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastMonthMatrix : public Matrix, public FastMonthVector {
 public:
  FastMonthMatrix(int colNum, int rowNum, int colCapacity, int* data, bool containNull)
      : Matrix(colNum, rowNum), FastMonthVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastMonthMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastMonthVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastTimeMatrix : public Matrix, public FastTimeVector {
 public:
  FastTimeMatrix(int colNum, int rowNum, int colCapacity, int* data, bool containNull)
      : Matrix(colNum, rowNum), FastTimeVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastTimeMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastTimeVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastSecondMatrix : public Matrix, public FastSecondVector {
 public:
  FastSecondMatrix(int colNum, int rowNum, int colCapacity, int* data, bool containNull)
      : Matrix(colNum, rowNum), FastSecondVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastSecondMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastSecondVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastMinuteMatrix : public Matrix, public FastMinuteVector {
 public:
  FastMinuteMatrix(int colNum, int rowNum, int colCapacity, int* data, bool containNull)
      : Matrix(colNum, rowNum), FastMinuteVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastMinuteMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastMinuteVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastNanoTimeMatrix : public Matrix, public FastNanoTimeVector {
 public:
  FastNanoTimeMatrix(int colNum, int rowNum, int colCapacity, long long* data, bool containNull)
      : Matrix(colNum, rowNum), FastNanoTimeVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastNanoTimeMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastNanoTimeVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastTimestampMatrix : public Matrix, public FastTimestampVector {
 public:
  FastTimestampMatrix(int colNum, int rowNum, int colCapacity, long long* data, bool containNull)
      : Matrix(colNum, rowNum), FastTimestampVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastTimestampMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastTimestampVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class FastNanoTimestampMatrix : public Matrix, public FastNanoTimestampVector {
 public:
  FastNanoTimestampMatrix(int colNum, int rowNum, int colCapacity, long long* data, bool containNull)
      : Matrix(colNum, rowNum), FastNanoTimestampVector(colNum * rowNum, colCapacity * rowNum, data, containNull) {
    setForm(DF_MATRIX);
  }
  ~FastNanoTimestampMatrix() override = default;
  ConstantSP getValue() const override;
  INDEX columns() const override { return cols_; }
  INDEX rows() const override { return rows_; }
  bool reshape(INDEX colNum, INDEX rowNum) override { return Matrix::reshape(colNum, rowNum); }
  ...
  ConstantSP getColumnLabel() const override { return colLabel_; }
  void setColumnLabel(const ConstantSP& label) override { Matrix::setColumnLabel(label); }
  std::string getString(int column, int row) const override { return FastNanoTimestampVector::getString((column * rows_) + row); }
  std::string getString() const override { return Matrix::getString(); }
  std::string getString(INDEX index) const override { return Matrix::getString(index); }
  bool set(INDEX column, INDEX row, const ConstantSP& value) override;
  bool set(const ConstantSP& index, const ConstantSP& value) override { return Matrix::set(index, value); }
...
class StringVector : public AbstractStringVector {
 public:
  ...
  bool set(INDEX index, const ConstantSP& value) override {
    ...
    if(data_[index].empty()) containNull_ = true;
    return true;
  }
  bool set(const ConstantSP& index, const ConstantSP& value) override;
  bool assign(const ConstantSP& value) override;
  ...
  ConstantSP get(const ConstantSP& index) const override;
  bool isNull(INDEX index) const override { return data_[index].empty(); }
  bool isNull() const override { return false; }
  ...
  bool getHash(INDEX start, int len, int buckets, int* buf) const override {
    for(int i = 0; i < len; ++i) buf[i] = murmur32(data_[start + i].data(), data_[start + i].size()) % buckets;
    return true;
  }
...
class FastRecordVector : public FastFixedLengthVector {
 public:
  FastRecordVector(DATA_TYPE type, int sz, int capacity, unsigned char* srcData, bool containNull)
      : FastFixedLengthVector(type, sizeof(T), sz, capacity, srcData, containNull) {}
  ...
  bool isNull(INDEX index) const override { return ((T*)(data_ + (index * fixedLength_)))->isNull(); }
  void nullFill(const ConstantSP& val) override {
    if(val->getType() != type_) throw IncompatibleTypeException(type_, val->getType());
    const unsigned char* buf = val->getBinary();
    const T* data = (const T*)data_;
    for(INDEX i = 0; i < size_; ++i) {
      if(data[i].isNull()) memcpy(data_ + (i * fixedLength_), buf, fixedLength_);
    }
  }
  bool isNull(INDEX start, int len, char* buf) const override {
    const T* data = (const T*)data_;
    for(int i = 0; i < len; ++i) buf[i] = data[start + i].isNull();
    return true;
  }
  bool isValid(INDEX start, int len, char* buf) const override {
    const T* data = (const T*)data_;
    for(int i = 0; i < len; ++i) buf[i] = data[start + i].isValid();
    return true;
  }
...
class FastInt128Vector : public FastRecordVector<Guid, GuidHash> {
 public:
  ...
  ~FastInt128Vector() override = default;
  ...
  bool getHash(INDEX start, int len, int buckets, int* buf) const override {
    for(int i = 0; i < len; ++i) buf[i] = murmur32_16b(data_ + ((start + i) * 16)) % buckets;
    return true;
  }
};
...
class FastUuidVector : public FastInt128Vector {
 public:
  ...
  ~FastUuidVector() override = default;
...
class FastIPAddrVector : public FastInt128Vector {
 public:
  ...
  ~FastIPAddrVector() override = default;
...
class FastSymbolVector : public AbstractFastVector<int> {
 public:
  FastSymbolVector(const SymbolBaseSP& base, int sz, int capacity, int* srcData, bool containNull)
      : AbstractFastVector(sz, capacity, srcData, 0, containNull), base_(base) {
    if(sz > 0) {
      base_->find("");
    }
  }
  ~FastSymbolVector() override = default;
  ...
  const std::string& getStringRef(INDEX index) const override { return base_->getSymbol(data_[index]); }
  ...
  ConstantSP getSubVector(INDEX start, INDEX length, INDEX capacity) const override {
    ...
    if(data) return new FastSymbolVector(base_, length, capacity, data, false);
    throw MemoryException();
  }
  ConstantSP getInstance(INDEX sz) const override {
    INDEX capacity = (std::max)(1, sz);
  ...
  bool append(const ConstantSP& value, INDEX appendSize) override {
    ...
    if(appendSize == 1) {
      ...
      if(value->getCategory() != LITERAL || value->size() < appendSize) return false;
  ...
  bool appendString(std::string* buf, int len) override {
    checkCapacity(len);
    for(int i = 0; i < len; ++i) data_[size_ + i] = base_->findAndInsert(buf[i]);
    size_ += len;
    return true;
  }
  bool appendString(char** buf, int len) override {
    checkCapacity(len);
    for(int i = 0; i < len; ++i) data_[size_ + i] = base_->findAndInsert(std::string(buf[i]));
    size_ += len;
    return true;
  }
  ...
  void replace(const ConstantSP& oldVal, const ConstantSP& newVal) override {
    ...
    for(INDEX i = 0; i < size_; ++i) {
      if(data_[i] == oldId) data_[i] = newId;
    }
  }
  bool isIndexArray() const override { return false; }
  INDEX* getIndexArray() const override { return nullptr; }
  ...
  ConstantSP retrieve(Vector* index) const {
    ...
    if(data) return new FastSymbolVector(base_, length, length, data, haveNull);
    throw MemoryException();
  }
...
class Decimal : public Constant {
  ...
  static_assert(std::is_same<T, int32_t>::value || std::is_same<T, int64_t>::value || std::is_same<T, int128>::value,
 ...
 public:
  ...
  Decimal(int scale, raw_data_t rawData) : scale_(scale), rawData_(rawData) {
    if(scale_ < 0 || scale_ > decimal_util::MaxPrecision<T>::value) {
      throw RuntimeException("Scale out of bound (valid range: [0, " + std::to_string(decimal_util::MaxPrecision<T>::value) +
  ...
  template <typename U>
  explicit Decimal(const Decimal<U>& other) : Decimal(other.scale_) {
    if(other.isNull()) {
      this->setNull();
    } else {
  ...
  std::string toString() const { return decimal_util::toString(scale_, rawData_); }
  ...
  const unsigned char* getBinary() const override { return (unsigned char*)&rawData_; }
  ...
  bool getDecimal32(INDEX start, int len, int scale, int32_t* buf) const override { return getDecimal(start, len, scale, buf); }
  bool getDecimal64(INDEX start, int len, int scale, int64_t* buf) const override { return getDecimal(start, len, scale, buf); }

  bool getDecimal128(INDEX start, int len, int scale, wide_integer::int128* buf) const override { return getDecimal(start, len, scale, buf); }
 ...
 private:
  ...
  bool getDecimal(INDEX /*start*/, int len, int scale, R* buf) const {
    if(scale < 0 || scale > decimal_util::MaxPrecision<R>::value) {
      throw RuntimeException("Scale out of bound (valid range: [0, " + std::to_string(decimal_util::MaxPrecision<R>::value) +
                             "], but get: " + std::to_string(scale) + ")");
    }
  ...
  DATA_TYPE type() const { return decimal_util::DecimalType<T>::value; }
...
using Decimal32SP = SmartPointer<Decimal32>;
using Decimal64SP = SmartPointer<Decimal64>;
using Decimal128SP = SmartPointer<Decimal128>;
...
class FastDecimalVector : public AbstractFastVector<T> {
  ...
  using unsigned_raw_data_t = typename std::make_unsigned<T>::type;
  using unsigned_raw_data_ptr_t = typename std::add_pointer<unsigned_raw_data_t>::type;

 public:
  FastDecimalVector(int scale, int sz, int capacity, T* srcData, bool containNull)
      : AbstractFastVector<T>(sz, capacity, srcData, std::numeric_limits<T>::min(), containNull), scale_(scale) {
    if(scale_ < 0 || scale_ > decimal_util::MaxPrecision<T>::value) {
  ...
  bool append(const ConstantSP& value, INDEX start, INDEX appendSize) override {
    ...
    if(type() != value->getType() || scale_ != value->getExtraParamForType()) {
      ...
      if(false == value->getBinary(start, appendSize, sizeof(T), reinterpret_cast<unsigned char*>(data_ + size_))) {
        return false;
      }
  ...
  bool set(const ConstantSP& index, const ConstantSP& value) override {
    ...
    for(INDEX start = 0; start < len; /**/) {
      ...
      auto pval = reinterpret_cast<const T*>(value->getBinaryConst(start, count, sizeof(T), reinterpret_cast<unsigned char*>(bufVal)));
  ...
  bool validIndex(INDEX uplimit) override { return validIndex(0, size_, uplimit); }
  ...
  // IO_ERR FastFixedLengthVector::deserialize(DataInputStream* in, INDEX indexStart, INDEX targetNumElement, INDEX&
  // numElement){
  IO_ERR deserialize(DataInputStream* in, INDEX indexStart, INDEX targetNumElement, INDEX& numElement) override {
    IO_ERR ret = OK;
    INDEX end = indexStart + targetNumElement;
...
using FastDecimal32VectorSP = SmartPointer<FastDecimal32Vector>;
using FastDecimal64VectorSP = SmartPointer<FastDecimal64Vector>;
using FastDecimal128VectorSP = SmartPointer<FastDecimal128Vector>;
}  // namespace dolphindb
