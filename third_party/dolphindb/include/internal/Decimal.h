...
template <>
struct MaxPrecision<wide_integer::int128> {
...
struct DecimalType;
template <>
struct DecimalType<int32_t> {
  static constexpr DATA_TYPE value = DT_DECIMAL32;
};
template <>
struct DecimalType<int64_t> {
  static constexpr DATA_TYPE value = DT_DECIMAL64;
};
template <>
struct DecimalType<wide_integer::int128> {
...
template <typename T>
struct wrapper {
  static T getDecimal(const ConstantSP &value, INDEX index, int scale) { return getDecimal(value.get(), index, scale); }
...
inline bool parseString(const char *str, size_t str_len, T &rawData, int &scale, std::string &errMsg, bool strict = false) {
  ...
  const char *const msg[] = {"", "illegal string", "no digits", "illegal string", "the number of digits exceed scale", "decimal overflow"};
  ...
  auto buildErrorMsg = [&](ErrorCodes err) {
    ...
    return "parse `" + std::string(str, str_len) + "` to " + Util::getDataTypeString(DecimalType<T>::value) + "(" + std::to_string(scale) +
           ") failed: " + msg[err];
  };
