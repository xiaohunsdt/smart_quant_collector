...
long long Util::tmporalRatioMatrix[81] = {1,
                                          0,
                                          0,
                                          0,
                                          0,
...
char Util::escape(char original) { return Util::escapes[(int)original]; }

int Util::countDays(int year, int month, int day) {
  if(month < 1 || month > 12 || day < 0) return INT_MIN;
...
char* Util::allocateMemory(INDEX size, bool throwIfFail) {
  try {
    ...
    if(throwIfFail) throw MemoryException();
    return nullptr;
  }
}
...
Constant* Util::createConstant(DATA_TYPE type, int extraParam) { return s_constFactory->createConstant(type, extraParam); }
...
Constant* Util::createBool(bool val) { return new Bool(val); }

Constant* Util::createBool(char val) { return new Bool(val); }

Constant* Util::createChar(char val) { return new Char(val); }

Constant* Util::createShort(short val) { return new Short(val); }

Constant* Util::createInt(int val) { return new Int(val); }

Constant* Util::createLong(long long val) { return new Long(val); }

Constant* Util::createFloat(float val) { return new Float(val); }

Constant* Util::createDouble(double val) { return new Double(val); }

Constant* Util::createString(const std::string& val) { return new String(val); }

Constant* Util::createBlob(const std::string& val) { return new String(val, true); }

Constant* Util::createDate(int year, int month, int day) { return new Date(year, month, day); }

Constant* Util::createDate(int days) { return new Date(days); }

Constant* Util::createMonth(int year, int month) { return new Month(year, month); }

Constant* Util::createMonth(int months) { return new Month(months); }

Constant* Util::createNanoTime(int hour, int minute, int second, int nanosecond) { return new NanoTime(hour, minute, second, nanosecond); }

Constant* Util::createNanoTime(long long nanoseconds) { return new NanoTime(nanoseconds); }

Constant* Util::createTime(int hour, int minute, int second, int millisecond) { return new Time(hour, minute, second, millisecond); }

Constant* Util::createTime(int milliseconds) { return new Time(milliseconds); }

Constant* Util::createSecond(int hour, int minute, int second) { return new Second(hour, minute, second); }

Constant* Util::createSecond(int seconds) { return new Second(seconds); }

Constant* Util::createMinute(int hour, int minute) { return new Minute(hour, minute); }

Constant* Util::createMinute(int minutes) { return new Minute(minutes); }
...
Constant* Util::createNanoTimestamp(long long nanoseconds) { return new NanoTimestamp(nanoseconds); }
...
Constant* Util::createTimestamp(long long milliseconds) { return new Timestamp(milliseconds); }

Constant* Util::createDateTime(int year, int month, int day, int hour, int minute, int second) {
  return new DateTime(year, month, day, hour, minute, second);
}

Constant* Util::createDateTime(int seconds) { return new DateTime(seconds); }

Constant* Util::createDateHour(int hours) { return new DateHour(hours); }

Constant* Util::createDateHour(int year, int month, int day, int hour) { return new DateHour(year, month, day, hour); }
...
bool Util::isFlatDictionary(Dictionary* dict) {
  if(dict->getKeyCategory() != LITERAL || dict->size() > 1024) return false;
  int numCol = dict->size();
  ConstantSP keys = dict->keys();
  ConstantSP value;
  for(int i = 0; i < numCol; ++i) {
    value = dict->getMember(keys->get(i));
    if(value->isNull() || !value->isScalar()) return false;
    DATA_TYPE type = value->getType();
    if(type == DT_VOID || type > DT_STRING) return false;
  }
  return true;
}

Table* Util::createTable(Dictionary* dict, int size) {
  if(dict->getKeyCategory() != LITERAL || dict->size() > 1024) return nullptr;
  int numCol = dict->size();
  ConstantSP keys = dict->keys();
  ConstantSP value;
  ...
  for(int i = 0; i < numCol; ++i) {
    ...
    value = dict->getMember(keys->get(i));
    if(value->isNull() || !value->isScalar()) throw RuntimeException("Invalid column format " + names.back());
    DATA_TYPE type = value->getType();
    if(type >= ARRAY_TYPE_BASE) {
      pVec = createArrayVector(type, (INDEX)size, 0, true, value->getExtraParamForType());
    } else {
      if(type == DT_VOID || type == DT_OBJECT || type == DT_ANY) throw RuntimeException("Invalid column type " + getDataTypeString(type));
...
Table* Util::createTable(const vector<string>& colNames, const vector<DATA_TYPE>& colTypes, INDEX size, INDEX capacity,
  ...
  for(size_t i = 0; i < numCol; ++i) {
    int extraParam = 0;
    if(i < extraParams.size()) extraParam = extraParams[i];
...
Set* Util::createSet(DATA_TYPE keyType, INDEX capacity) { return s_constFactory->createSet(convertToIntegralDataType(keyType), keyType, capacity); }

Vector* Util::createVector(DATA_TYPE type, INDEX size, INDEX capacity, bool fast, int extraParam, void* data, bool containNull) {
  if(type < ARRAY_TYPE_BASE) return s_constFactory->createConstantVector(type, size, capacity, true, extraParam, data, nullptr, 0, containNull);
...
Vector* Util::createIndexVector(INDEX start, INDEX length) {
  ...
  if(index == nullptr) throw RuntimeException("Failed to create an index vector.");
...
void Util::toGuid(const unsigned char* data, char* str) {
  ...
  for(int i = 0; i < 4; ++i) charToHexPair(data[15 - i], str + (2 * i));
  ...
  for(int i = 5; i >= 0; --i) charToHexPair(data[i], str + 34 - (2 * i));
#else
  for(int i = 0; i < 4; ++i) charToHexPair(data[i], str + 2 * i);
  ...
  for(int i = 10; i < 16; ++i) charToHexPair(data[i], str + 4 + 2 * i);
#endif
}
...
inline unsigned char hexPairToChar(char a, char b) { return (hexPairToChar(a) << 4) + hexPairToChar(b); }

bool Util::fromGuid(const char* str, unsigned char* data) {
  if(str[8] != '-' || str[13] != '-' || str[18] != '-' || str[23] != '-') return false;
#ifndef BIGENDIANNESS
  for(int i = 0; i < 4; ++i) data[15 - i] = hexPairToChar(str[2 * i], str[(2 * i) + 1]);
  ...
  for(int i = 10; i < 16; ++i) data[15 - i] = hexPairToChar(str[(2 * i) + 4], str[(2 * i) + 5]);
#else
  for(int i = 0; i < 4; ++i) data[i] = hexPairToChar(str[2 * i], str[2 * i + 1]);
  ...
  for(int i = 10; i < 16; ++i) data[i] = hexPairToChar(str[2 * i + 4], str[2 * i + 5]);
#endif
  return true;
}

DATA_TYPE Util::getDataType(const std::string& typestr) { return s_constFactory->getDataType(lower(typestr)); }

DATA_FORM Util::getDataForm(const std::string& formstr) { return s_constFactory->getDataForm(lower(formstr)); }

char Util::getDataTypeSymbol(DATA_TYPE type) { return s_constFactory->getDataTypeSymbol(type); }

std::string Util::getDataTypeString(DATA_TYPE type) { return s_constFactory->getDataTypeString(type); }

std::string Util::getDataFormString(DATA_FORM form) { return s_constFactory->getDataFormString(form); }

std::string Util::getTableTypeString(TABLE_TYPE type) { return s_constFactory->getTableTypeString(type); }

DATA_TYPE Util::getDataType(char ch) {
  if(ch == 'b') return DT_BOOL;
  if(ch == 'c') return DT_CHAR;
  if(ch == 'h') return DT_SHORT;
  if(ch == 'i') return DT_INT;
  if(ch == 'l') return DT_LONG;
  if(ch == 'f') return DT_FLOAT;
  if(ch == 'F') return DT_DOUBLE;
  if(ch == 'd') return DT_DATE;
  if(ch == 'M') return DT_MONTH;
  if(ch == 'm') return DT_MINUTE;
  if(ch == 's') return DT_SECOND;
  if(ch == 't') return DT_TIME;
  if(ch == 'D') return DT_DATETIME;
  if(ch == 'T') return DT_TIMESTAMP;
  if(ch == 'n') return DT_NANOTIME;
  if(ch == 'N') return DT_NANOTIMESTAMP;
  if(ch == 'S') return DT_SYMBOL;
  if(ch == 'W') return DT_STRING;
  return DT_VOID;
}

DURATION_UNIT Util::getDurationUnit(const std::string& typestr) {
  for(int i = 0; i < 10; ++i) {
    if(duSyms[i] == typestr) return (DURATION_UNIT)i;
  }
  return (DURATION_UNIT)-1;
}

long long Util::getTemporalDurationConversionRatio(DATA_TYPE t, DURATION_UNIT du) { return tmporalDurationRatioMatrix[t - DT_DATE][du]; }

long long Util::getTemporalUplimit(DATA_TYPE t) { return tmporalUplimit[t - DT_DATE]; }

DATA_TYPE Util::convertToIntegralDataType(DATA_TYPE type) {
  if(type == DT_TIME || type == DT_SECOND || type == DT_MINUTE || type == DT_DATE || type == DT_DATETIME || type == DT_MONTH || type == DT_DATEHOUR ||
     type == DT_DATEMINUTE)
    return DT_INT;
  if(type == DT_TIMESTAMP || type == DT_NANOTIME || type == DT_NANOTIMESTAMP) return DT_LONG;
  if(type == DT_UUID || type == DT_IP) return DT_INT128;
  return type;
}
...
DATA_CATEGORY Util::getCategory(DATA_TYPE type) {
  if(type == DT_TIME || type == DT_SECOND || type == DT_MINUTE || type == DT_DATE || type == DT_DATEHOUR || type == DT_DATEMINUTE ||
    ...
    return TEMPORAL;
  if(type == DT_INT || type == DT_LONG || type == DT_SHORT || type == DT_CHAR) return INTEGRAL;
  if(type == DT_BOOL) return LOGICAL;
  if(type == DT_DOUBLE || type == DT_FLOAT) return FLOATING;
  if(type == DT_STRING || type == DT_SYMBOL || type == DT_BLOB) return LITERAL;
  if(type == DT_INT128 || type == DT_UUID || type == DT_IP) return BINARY;
  if(type == DT_ANY) return MIXED;
  if(type == DT_VOID) return NOTHING;
  if(type == DT_DECIMAL32 || type == DT_DECIMAL64 || type == DT_DECIMAL128) return DENARY;
  if(type >= ARRAY_TYPE_BASE) return ARRAY;
  return SYSTEM;
}
...
std::string Util::replace(const std::string& str, const std::string& pattern, const std::string& replacement) {
  ...
  while(true) {
    ...
    if(end > start) target.append(str.substr(start, end - start));
...
std::string Util::replace(const std::string& str, char pattern, char replacement) {
  ...
  string target(str);
  for(size_t i = 0; i < len; ++i) {
    if(str[i] == pattern) target[i] = replacement;
  }
  return target;
}
...
char Util::toUpper(char ch) { return (ch >= 'a' && ch <= 'z') ? ch - 32 : ch; }

char Util::toLower(char ch) { return (ch >= 'A' && ch <= 'Z') ? ch + 32 : ch; }
...
bool Util::endWith(const std::string& str, const std::string& end) {
  if(end.empty() || str.size() < end.size()) return false;
  return str.substr(str.size() - end.size(), end.size()) == end;
}

bool Util::startWith(const std::string& str, const std::string& start) {
  if(start.empty() || str.size() < start.size()) return false;
  return str.substr(0, start.size()) == start;
}

bool Util::isVariableCandidate(const std::string& word) {
  char cur = word.at(0);
  if((cur < 'a' || cur > 'z') && (cur < 'A' || cur > 'Z')) return false;
  for(unsigned int i = 1; i < word.length(); i++) {
    cur = word.at(i);
    if((cur < 'a' || cur > 'z') && (cur < 'A' || cur > 'Z') && (cur < '0' || cur > '9') && cur != '_') return false;
  }
  return true;
}
...
std::string Util::literalConstant(const std::string& str) {
  ...
  if(end == string::npos)
    ...
    if(start < str.length()) script.append(str.substr(start));
  }
  script.append(1, '"');
  return script;
}

long long Util::getNanoBenchmark() { return std::chrono::steady_clock::now().time_since_epoch() / std::chrono::nanoseconds(1); }

long long Util::getNanoEpochTime() { return std::chrono::system_clock::now().time_since_epoch() / std::chrono::nanoseconds(1); }

long long Util::getEpochTime() { return std::chrono::system_clock::now().time_since_epoch() / std::chrono::milliseconds(1); }
...
int* Util::toLocalDateTime(int* epochTimes, int n) {
  struct tm lt;
  for(int i = 0; i < n; ++i) {
    if(epochTimes[i] == INT_MIN) continue;
...
long long* Util::toLocalTimestamp(long long* epochTimes, int n) {
  struct tm lt;
  for(int i = 0; i < n; ++i) {
    if(epochTimes[i] == LLONG_MIN) continue;
    time_t t = epochTimes[i] / 1000;
    getLocalTime(t, lt);
    int days = countDays(lt.tm_year + 1900, lt.tm_mon + 1, lt.tm_mday);
    epochTimes[i] =
        days == INT_MIN ? LLONG_MIN : (days * 86400000LL) + (((lt.tm_hour * 60 + lt.tm_min) * 60 + lt.tm_sec) * 1000LL) + (epochTimes[i] % 1000);
  }
  return epochTimes;
}
...
long long Util::toLocalNanoTimestamp(long long epochNanoTime) {
  ...
  return days == INT_MIN
             ? LLONG_MIN
             : (days * 86400000000000LL) + (((lt.tm_hour * 60 + lt.tm_min) * 60 + lt.tm_sec) * 1000000000LL) + (epochNanoTime % 1000000000);
}

long long* Util::toLocalNanoTimestamp(long long* epochNanoTimes, int n) {
  struct tm lt;
  for(int i = 0; i < n; ++i) {
    if(epochNanoTimes[i] == LLONG_MIN) continue;
    time_t t = epochNanoTimes[i] / 1000000000;
    getLocalTime(t, lt);
    int days = countDays(lt.tm_year + 1900, lt.tm_mon + 1, lt.tm_mday);
    epochNanoTimes[i] = days == INT_MIN ? LLONG_MIN
                                        : (days * 86400000000000LL) + (((lt.tm_hour * 60 + lt.tm_min) * 60 + lt.tm_sec) * 1000000000LL) +
                                              (epochNanoTimes[i] % 1000000000);
  }
...
#ifdef _MSC_VER
  if(printDate)
    sprintf_s(buf, 100, "%04d-%02d-%02d %02d:%02d:%02d.%06d", lt.tm_year + 1900, lt.tm_mon + 1, lt.tm_mday, lt.tm_hour, lt.tm_min, lt.tm_sec,
              microsecond);
  else
...
int Util::getDataTypeSize(DATA_TYPE type) {
  ...
  if(type == DT_INT || type == DT_SYMBOL || type == DT_SECOND || type == DT_DATE || type == DT_MONTH || type == DT_TIME || type == DT_MINUTE ||
     type == DT_DATETIME || type == DT_DATEHOUR) {
    return sizeof(int);
  }
  if(type == DT_SHORT) {
    return sizeof(short);
  ...
  if(type == DT_INT128 || type == DT_IP || type == DT_UUID) return sizeof(Guid);
  return -1;
}
...
void Util::split(const char* s, char delim, vector<string>& elems) {
  ...
  if(*start != 0) elems.emplace_back(start, length);
}
  ...
  for(s = str, p = pat; *s != 0; ++s, ++p) {
    switch(*p) {
      ...
      case '%':
        ...
        goto loopStart;
      default:
        if(toLower(*s) != toLower(*p)) goto starCheck;
        break;
    }
  }
  if(*p == '%') ++p;
...
#ifdef _MSC_VER
  TCHAR buf[256];
  FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, errCode, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), buf, 256,
                nullptr);
  return tcharToString(buf);
#else
  char buf[256];
...
std::string Util::getPartitionTypeString(PARTITION_TYPE type) { return s_constFactory->getPartitionTypeString(type); }

std::string Util::getCategoryString(DATA_CATEGORY type) { return s_constFactory->getCategoryString(type); }
...
Vector* Util::createSymbolVector(const SymbolBaseSP& symbolBase, INDEX size, INDEX capacity, bool fast, void* data, void** dataSegment,
  ...
  if(data != nullptr) return new FastSymbolVector(symbolBase, size, capacity, (int*)data, containNull);
  return nullptr;
}
    ...
    case DATA_TYPE::DT_INT:
      if(val >= INT_MIN && val <= INT_MAX) {
        ...
        SetOrThrowErrorInfo(&errorCodeInfo, ErrorCodeInfo::EC_InvalidObject,
                            std::string(pTypeName) + " cannot be converted because it exceeds the range of " + getDataTypeString(dataType));
        return false;
      }
      break;
...
bool Util::setValue(ConstantSP& data, DATA_TYPE dataType, const std::vector<const unsigned char*>& val, ErrorCodeInfo& errorCodeInfo,
                    int extraParam) {
  // Only arrayVector needs vector data and it requires any vector.
...
ConstantSP Util::createValue(DATA_TYPE dataType, long long val, const char* pTypeName, ErrorCodeInfo* errorCodeInfo, int extraParam) {
  switch(dataType) {
    case DATA_TYPE::DT_DECIMAL128:
      if(extraParam > 0) return createDecimal128(extraParam, (double)val);
      return createDouble(static_cast<double>(val));
      break;
    case DATA_TYPE::DT_DECIMAL64:
      if(extraParam > 0) return createDecimal64(extraParam, (double)val);
      return createDouble(static_cast<double>(val));
      break;
    case DATA_TYPE::DT_LONG:
      ...
      break;
    case DATA_TYPE::DT_DECIMAL32:
      if(extraParam > 0) return createDecimal32(extraParam, (double)val);
      return createDouble(static_cast<double>(val));
      break;
    case DATA_TYPE::DT_INT:
      if(val >= INT_MIN && val <= INT_MAX)
...
ConstantSP Util::createObject(DATA_TYPE dataType, float val, ErrorCodeInfo* errorCodeInfo, int extraParam) {
  switch(dataType) {
    case DATA_TYPE::DT_DECIMAL32:
      if(extraParam > 0) return createDecimal32(extraParam, val);
      return createDouble(val);
      break;
    case DATA_TYPE::DT_DECIMAL64:
      if(extraParam > 0) return createDecimal64(extraParam, val);
      return createDouble(val);
      break;
    case DATA_TYPE::DT_DECIMAL128:
      if(extraParam > 0) return createDecimal128(extraParam, val);
      return createDouble(val);
      break;
    case DATA_TYPE::DT_FLOAT:
...
bool Util::checkColDataType(DATA_TYPE colDataType, bool isColTemporal, ConstantSP& constsp) {
  ...
  if(constsp->getForm() == DF_SCALAR) {
    if(constsp->getType() == colDataType) return true;
    if(colDataType == DT_SYMBOL && constsp->getType() == DT_STRING) return true;
