...
StreamDeserializer::StreamDeserializer(const std::unordered_map<std::string, DictionarySP> &sym2schema) { parseSchema(sym2schema); }
...
void StreamDeserializer::returnMessage(Message *msg) { symbol2tableInfo_[msg->getSymbol()]->returnTuple(*msg); }
...
void StreamDeserializer::create(DBConnection &conn) {
  if(!symbol2tableInfo_.empty() || sym2tableName_.empty()) return;
