...
class CheckSum {
 public:
  unsigned int crc32(unsigned int prev, const unsigned char *buf, int len) {
    ...
    for(int i = 0; i < len; i++) crc = crcTable_[*buf++ ^ (crc & 0xffU)] ^ (crc >> 8U);
    return (~crc);
  }
...
class DeltaCompressor {
 public:
  DeltaCompressor() { firstDeltaBits_ = sizeof(T) * 8; }
...
class DeltaDecompressor {
 ...
 private:
  ...
  bool readHeaderData() {
    ...
    if(!read_.readBits(5, &flag)) return false;
    if(flag == 30) {
    ...
    if(!read_.readBits(sizeof(T) * 8, (unsigned long long *)&blockData_)) return false;
  ...
  long long decodeZigZag64(unsigned long long n) { return ((n) >> 1U) ^ -((long long)(n & 1U)); }
};
...
bool DeltaBufferRead::readBits(int bits, unsigned long long *value) {
  *value = 0;
  if(position_ >= limit_ && bitsAvailable_ == 0) return false;
  if(bitsAvailable_ == 0) {
    if(!flipByte()) {
      return false;
    }
...
int DeltaDecompressor<T>::readData(long long *buf, int bufferSize, T *data, int dataSize) {
  ...
  if(!read_.readBits(1, &flag)) return count;
  while(flag == 0) {
    data[count] = nullVal_;
    count++;
    if(!read_.readBits(1, &flag) || count > dataSize) {
...
int DeltaCompressor<T>::writeData(const T *data, int DataSize, long long *buf, int bufferSize) {
  ...
  while(count < DataSize) {
    if((data[count] == INT_MIN && sizeof(T) == sizeof(int)) || (data[count] == LLONG_MIN && sizeof(T) == sizeof(long long)) ||
...
IO_ERR CompressDeltaofDelta::decode(DataInputStreamSP compressSrc, DataOutputStreamSP &decompressResult, const CompressionFactory::Header &header) {
  ...
  while(fileCursor < byteSize && start < len) {
    ...
    if(ret != OK) return ret;
    if(calcChecksum) cksum = checkSum.crc32(cksum, (const unsigned char *)&blockSize, 4);
    if(blockSize < 0) {
      containLSN = true;
    ...
    if(blockSize <= 0 || blockSize > maxCompressedSize_) {
      std::cout << "Failed to decode. streamType=" + std::to_string(compressSrc->getStreamType()) + " blockSize=" + std::to_string(blockSize) +
                       " fileCursor=" + std::to_string(fileCursor) + " fileLength=" + std::to_string(byteSize) +
                       " decodedRows=" + std::to_string(start) + " totalRows=" + std::to_string(len) + " ret=" + std::to_string(ret)
                << std::endl;
      return INVALIDDATA;
    }
    ret = compressSrc->readBytes(compressedBuf, blockSize, actualRead);
    if(ret != OK) {
      std::cout << "Failed to decode. fileCursor=" + std::to_string(fileCursor) + " fileLength=" + std::to_string(byteSize) +
                       " decodedRows=" + std::to_string(start) + " totalRows=" + std::to_string(len) + "blockSize=" + std::to_string(blockSize) +
                       " actualRead=" + std::to_string(actualRead) + " ret=" + std::to_string(ret)
                << std::endl;
      return ret;
    }
    ...
    if(readSize <= 0) {
      std::cout << "Failed to decode. LZ4 block offset=" + std::to_string(fileCursor - blockSize) + " fileLength=" + std::to_string(byteSize) +
                       " decodedRows=" + std::to_string(start) + " totalRows=" + std::to_string(len) + " blockSize=" + std::to_string(blockSize) +
                       " bufSize=" + std::to_string(decompressedBufSize)
                << std::endl;
      return INVALIDDATA;
    }
    count = readSize;
    ...
    if(containLSN && fileCursor + 8 <= byteSize) {
      ...
      if(ret != OK) return ret;
      if(calcChecksum) cksum = checkSum.crc32(cksum, (const unsigned char *)&lsn, sizeof(long long));
    }
  }
...
IO_ERR CompressDeltaofDelta::encodeContent(const VectorSP &vec, const DataOutputStreamSP &compressResult, CompressionFactory::Header &header,
                                           bool needcheckSum) {
  ...
  INDEX start = 0;
  {
    ...
    while(start < len) {
      ...
      if(needcheckSum) cksum = checkSum.crc32(cksum, (const unsigned char *)blockBuf, blockSize + sizeof(int));
...
IO_ERR CompressLZ4::decode(DataInputStreamSP compressSrc, DataOutputStreamSP &uncompressResult, const CompressionFactory::Header &header) {
  ...
  while(fileCursor < byteSize && start < len) {
    ...
    if(ret != OK || blockSize <= 0 || blockSize > MAX_COMPRESSED_SIZE || fileCursor + blockSize > byteSize) {
      std::cout << "Failed to decode. blockSize=" + std::to_string(blockSize) + " fileCursor=" + std::to_string(fileCursor) +
...
IO_ERR CompressLZ4::encodeContent(const VectorSP &vec, const DataOutputStreamSP &compressResult, CompressionFactory::Header &header,
                                  bool needcheckSum) {
  std::vector<char *> blockBufList;
  std::vector<int> blockSizeList;
  int compressedbyteSize = 0;
    ...
    if(type != DT_SYMBOL) {
      while(start < len) {
        ...
        if(type != DT_STRING) {
          ...
          if(decompressedBufSize == 0) decompressedBufSize = vec->serialize(decompressedBuf, MAX_DECOMPRESSED_SIZE, start, offset, count, offset);
          if(decompressedBufSize == 0) return TOO_LARGE_DATA;
        }
        if(ret != OK) return ret;
        ...
        if(needcheckSum) cksum = checkSum.crc32(cksum, (const unsigned char *)blockBuf, blockSize);
