...
ConstantSP Vector::getColumnLabel() const { return new String(name_); }
...
std::string Vector::getScript() const {
  ...
  if(getForm() == DF_MATRIX) {
    return name_.empty() ? "matrix()" : name_;
  }
  int len = size();
  if(len > Util::CONST_VECTOR_MAX_SIZE) return name_.empty() ? "array()" : name_;

  std::string str("[");
  if(len > 0) str.append(get(0)->getScript());
  for(int i = 1; i < len; ++i) {
    str.append(",");
    str.append(get(i)->getScript());
  }
