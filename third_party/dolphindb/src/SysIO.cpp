...
void LOG_ERR(const std::string& msg) { DLogger::Error(msg); }

void LOG_INFO(const std::string& msg) { DLogger::Info(msg); }
...
Socket::Socket(const std::string& host, int port, bool blocking, int keepAliveTime, bool enableSSL)
    : host_(host),
      port_(port),
      blocking_(blocking),
      autoClose_(true),
...
bool Socket::isValid() { return handle_ != INVALID_SOCKET; }
...
IO_ERR Socket::read(char* buffer, size_t length, size_t& actualLength, bool msgPeek) {
  if(!enableSSL_) {
    ...
    if(ret == SOCKET_ERROR) {
      ...
      if(error == WSAENOTCONN || error == WSAESHUTDOWN || error == WSAENETRESET) return DISCONNECTED;
      if(error == WSAEWOULDBLOCK) return NODATA;
      return OTHERERR;
    }
    if(ret == 0) {
      return DISCONNECTED;
  ...
  readdata:
    actualLength = recv(handle_, (void*)buffer, length,
                        (blocking_ ? 0 : MSG_DONTWAIT) | (msgPeek ? MSG_PEEK : 0));  // NOLINT(hicpp-signed-bitwise): linux header macros
    RECORD_READ(buffer, actualLength);
    if(actualLength == (size_t)SOCKET_ERROR && errno == EINTR) goto readdata;
    if(actualLength == 0) return DISCONNECTED;
    if(actualLength != (size_t)SOCKET_ERROR) return OK;
    if(errno == EAGAIN || errno == EWOULDBLOCK) return NODATA;
    actualLength = 0;
    return OTHERERR;
...
IO_ERR Socket::write(const char* buffer, size_t length, size_t& actualLength) {
  ...
  senddata:
    ...
    actualLength = 0;
    if(errno == EAGAIN || errno == EWOULDBLOCK) return NOSPACE;
    if(errno == ECONNRESET || errno == EPIPE || errno == EBADF || errno == ENOTCONN) return DISCONNECTED;
...
IO_ERR Socket::bind() {
  if(port_ < 0 || handle_ == INVALID_SOCKET) return OTHERERR;
...
IO_ERR Socket::listen() {
  ...
  if(r != SOCKET_ERROR) return OK;
  LOG_ERR("Failed to bind the socket on port " + std::to_string(port_) + " with error code " + std::to_string(getErrorCode()));
  closesocket(handle_);
  return OTHERERR;
}

void Socket::enableTcpNoDelay(bool enable) { ENABLE_TCP_NODELAY = enable; }
...
IO_ERR Socket::connect() {
  if(port_ == -1 || host_.empty()) return OTHERERR;
  ...
  for(p = servinfo; p != nullptr; p = p->ai_next) {
    if((handle_ = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == (SOCKET)SOCKET_ERROR) continue;
  ...
  freeaddrinfo(servinfo);
  if(handle_ == INVALID_SOCKET) return DISCONNECTED;

#ifdef USE_OPENSSL
  if(enableSSL_ && sslConnect() != OK) return DISCONNECTED;
#endif
  return OK;
}
...
Socket* Socket::accept() {
  ...
  if(t == INVALID_SOCKET) {
    ...
    if(errCode != WSAEWOULDBLOCK) LOG_ERR("Failed to accept one incoming connection with error code " + std::to_string(errCode));
#else
    int errCode = errno;
    if(errCode != EWOULDBLOCK && errCode != EAGAIN) LOG_ERR("Failed to accept one incoming connection with error code " + std::to_string(errCode));
#endif
    return nullptr;
  }
...
SOCKET Socket::getHandle() { return handle_; }
...
bool Socket::skipAll() {
  int oldTimeout = -2;
  if(blocking_ == false) {
    if(setBlocking() == false) return false;
  } else {
  ...
  if(blocking_ == false) {
    ...
    if(oldTimeout != -2) setTimeout(oldTimeout);
  }
  return true;
}
...
bool Socket::sslInit() {
#if defined(OPENSSL_VERSION_NUMBER) && OPENSSL_VERSION_NUMBER >= 0x10100000L
  auto sslInit = OPENSSL_INIT_LOAD_SSL_STRINGS | OPENSSL_INIT_LOAD_CRYPTO_STRINGS | OPENSSL_INIT_ADD_ALL_CIPHERS | OPENSSL_INIT_ADD_ALL_DIGESTS;
...
DataInputStream::DataInputStream(STREAM_TYPE type, std::size_t bufSize)
    : file_(nullptr),
...
DataInputStream::DataInputStream(const SocketSP& socket, std::size_t bufSize)
    : socket_(socket),
      file_(nullptr),
      buf_(new char[bufSize]),
...
DataInputStream::DataInputStream(FILE* file, std::size_t bufSize)
    : file_(file),
      buf_(new char[bufSize]),
      source_(FILE_STREAM),
      reverseOrder_(false),
...
DataInputStream::DataInputStream(const char* data, std::size_t size, bool copy)
    : file_(nullptr), source_(ARRAY_STREAM), reverseOrder_(false), externalBuf_(!copy), closed_(false), capacity_(size), size_(size), cursor_(0) {
  if(copy) {
    buf_ = new char[size];
...
DataInputStream::DataInputStream(const DataQueueSP& dataQueue)
    : file_(nullptr),
      buf_(nullptr),
      source_(QUEUE_STREAM),
      reverseOrder_(false),
      externalBuf_(false),
      closed_(false),
      capacity_(0),
...
DataInputStream::~DataInputStream() {
  if(!externalBuf_) delete[] buf_;
  if(source_ == FILE_STREAM) close();
  ...
  if(source_ == QUEUE_STREAM) {
    ...
    while(true) {
      ...
      if(isStringFinished) {
        ...
        if(lineLength > 0 && buf_[endPos - 1] == '\r') lineLength--;
...
IO_ERR DataInputStream::readBytes(char* buf, size_t length, size_t& actualLength) {
  ...
  if(source_ == FILE_STREAM) {
    ...
    actualLength += count;
    if(count == 0) {
      if(feof(file_) != 0) return END_OF_STREAM;
      return OTHERERR;
    }
    return OK;
  }
...
IO_ERR DataInputStream::readBytes(char* buf, size_t unitLength, size_t length, size_t& actualLength) {
  if(unitLength == 1) return readBytes(buf, length, actualLength);
...
IO_ERR DataInputStream::prepareBytes(size_t length) {
  if(source_ == ARRAY_STREAM) return END_OF_STREAM;
...
IO_ERR DataInputStream::close() {
  if(closed_ || source_ == ARRAY_STREAM || source_ == QUEUE_STREAM) return OK;

  if(source_ == SOCKET_STREAM) {
    IO_ERR ret = socket_->close();
    if(ret == OK) closed_ = true;
    return ret;
  }
...
DataOutputStream::DataOutputStream(const SocketSP& socket, size_t flushThreshold)
    : source_(SOCKET_STREAM),
      ...
      size_(0),
      autoClose_(false) {
  if(capacity_ > 0) buf_ = new char[capacity_];
}

DataOutputStream::DataOutputStream(FILE* file, bool autoClose)
    : source_(FILE_STREAM), flushThreshold_(0), file_(file), buf_(nullptr), capacity_(0), size_(0), autoClose_(autoClose) {}

DataOutputStream::DataOutputStream(size_t capacity)
    : source_(ARRAY_STREAM), flushThreshold_(0), file_(nullptr), buf_(new char[capacity]), capacity_(capacity), size_(0), autoClose_(false) {}

DataOutputStream::DataOutputStream(STREAM_TYPE source)
    : source_(source), flushThreshold_(0), file_(nullptr), buf_(nullptr), capacity_(0), size_(0), autoClose_(false) {}

DataOutputStream::DataOutputStream(const DataQueueSP& dataQueue)
    : source_(QUEUE_STREAM), flushThreshold_(0), file_(nullptr), buf_(nullptr), capacity_(0), size_(0), autoClose_(false), dataQueue_(dataQueue) {}
...
IO_ERR DataOutputStream::write(const char* buffer, size_t length, size_t& actualWritten) {
  ...
  switch(source_) {
    ...
    case FILE_STREAM:
      ...
      if(actualWritten < length) return OTHERERR;
      return OK;
    ...
    case ARRAY_STREAM:
      if(size_ + length > capacity_) {
        ...
        if(buf_ == nullptr) return TOO_LARGE_DATA;
...
IO_ERR DataOutputStream::write(const char* buffer, size_t length) {
  ...
  switch(source_) {
    ...
    case FILE_STREAM:
      ...
      if(actualLength < length) return OTHERERR;
      return OK;
...
IO_ERR DataOutputStream::resume() {
  if(size_ == 0 || source_ != SOCKET_STREAM) return OK;
...
DataStream::DataStream(FILE* file, bool readable, bool writable) : DataInputStream(file), outBuf_(new char[2048]) {
  isReadable(readable);
  isWritable(writable);
}

DataStream::~DataStream() { delete[] outBuf_; }

bool DataStream::isReadable() const { return (flag_ & 1U) != 0; }
...
bool DataStream::isWritable() const { return (flag_ & 2U) != 0; }
...
#ifdef MAC
    if(fseeko(file_, offset, SEEK_CUR) != 0) return OTHERERR;
#else
...
IO_ERR DataStream::writeLine(const char* obj, const char* newline) {
  if(source_ == FILE_STREAM) {
    ...
    if(fputs(newline, file_) >= 0) return OK;
    return OTHERERR;
  }
  // write to socket
  size_t actualSent;
...
IO_ERR DataStream::seek(long long offset, int mode, long long& newPosition) {
  ...
  if(mode == 1 && size_ > 0) {
    ...
    ret = OTHERERR;
  if(ret == OK) newPosition = ftello(file_);
#else
  if(mode == 1 && size_ > 0) {
    offset -= size_;
    if(offset == 0) {
    ...
    ret = OTHERERR;
  if(ret == OK) newPosition = ftello64(file_);  // NOLINT(misc-include-cleaner): the suggested header is not standard Linux API
#endif
  return ret;
}

std::string DataStream::getDescription() const {
  if(source_ == SOCKET_STREAM) return "SocketStream[" + std::to_string(socket_->getHandle()) + "]";
  if(source_ == FILE_STREAM) return "FileStream[" + std::to_string((long long)file_) + "]";
  return "ArrayStream";
}
