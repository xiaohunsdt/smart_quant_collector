...
class StreamingClientImpl {
  ...
  struct SubscribeParam {
    ...
    explicit SubscribeParam(std::string id, SubscribeInfo info, long long offset, bool resub, const VectorSP &filter, bool msgAsTable,
                            bool allowExists, int batchSize, std::string userName, std::string password, const StreamDeserializerSP &blobDeserializer,
                            bool isEvent, int resubscribeInterval, bool subOnce, bool convertMsgRowData, int resubscribeTimeout)
        : ID_(std::move(id)),
          info_(std::move(info)),
          offset_(offset),
          resub_(resub),
    ...
    bool operator<(const SubscribeParam &other) const { return ID_ < other.ID_; }

    void exit() {
      if(!socket_.isNull()) {
        socket_->close();
      }
      if(queue_.isNull()) return;
      *stopped_ = true;
      queue_->push(Message());
 ...
 public:
  ...
  void exit() {
    ...
    if(listenerSocket_.isNull() == false) listenerSocket_->close();
    if(reconnectThread_.isNull() == false) reconnectThread_->join();
    if(daemonThread_.joinable()) daemonThread_.join();
    {
  ...
  SubscribeQueue subscribeInternal(const SubscribeInfo &subInfo, int64_t offset = -1, bool resubscribe = true, const VectorSP &filter = nullptr,
                                   bool msgAsTable = false, bool allowExists = false, int batchSize = 1, const string &userName = "",
                                   const string &password = "", const StreamDeserializerSP &sdsp = nullptr,
                                   const std::vector<std::string> &backupSites = std::vector<std::string>(), bool isEvent = false,
                                   int resubscribeInterval = 100, bool subOnce = false, bool convertMsgRowData = false,
                                   int resubscribeTimeout = 3600000);
  ...
  void onSubscribeStateChange(const SubscribeCallbackT &callback) { callback_ = callback; }
 ...
 private:
  ...
  void init();
  bool isListenMode() { return listeningPort_ > 0; }
  ...
  void daemon() {
    ...
    while(isExit() == false) {
      try {
        ...
        std::thread t([this, inputStream]() { parseMessage(inputStream); });
...
class UDPStreamingImpl {
 public:
  UDPStreamingImpl(StreamingClientConfig config) : config_(config) {}
  ~UDPStreamingImpl();
  void subscribe(const SubscribeInfo &info, const SubscribeConfig &config, const std::shared_ptr<DBConnection> &conn,
                 const MessageHandler &handlerconst, const StreamDeserializerSP &blobDeserializer);
...
void UDPStreamingImpl::subscribe(const SubscribeInfo &info, const SubscribeConfig &config, const std::shared_ptr<DBConnection> &conn,
                                 const MessageHandler &handler, const StreamDeserializerSP &blobDeserializer) {
  ...
  client.receiveThread = std::thread([this, &client]() {
    aeron::fragment_handler_t h = [this, &client](aeron::concurrent::AtomicBuffer &buffer, aeron::util::index_t offset, aeron::util::index_t length,
                                                  aeron::Header &header) {
      std::ignore = header;
...
void StreamingClientImpl::init() {
  ...
  if(isListenMode()) {
    ...
    if(listenerSocket_->bind() != OK) {
      throw RuntimeException("Failed to bind the socket on port " + Util::convert(listeningPort_) + ". Couldn't start the subscription daemon.");
    }
    if(listenerSocket_->listen() != OK) {
      throw RuntimeException("Failed to listen the socket on port " + Util::convert(listeningPort_) + ". Couldn't start the subscription daemon.");
    }
  }
  exit_ = false;
  reconnectThread_ = new Thread(new Executor(std::bind(&StreamingClientImpl::reconnect, this)));
  reconnectThread_->start();
  daemonThread_ = std::thread([this]() { daemon(); });
}
...
void StreamingClientImpl::checkServerVersion(std::string host, int port, const std::vector<std::string> &backupSites) {
  ...
  if(conn.checkVersion({{2, 9, 0}, {3, 0, 0}})) {
    if(listeningPort_ != 0) {
      DLogger::Warn(
          "The server only supports subscription through reverse connection (connection initiated by the subscriber). "
          "The specified port will not take effect.");
      listeningPort_ = 0;
    }
  } else {
    if(listeningPort_ == 0) {
      throw RuntimeException(
          "The server does not support subscription through reverse connection (connection initiated by the "
          "subscriber). Specify a valid port parameter.");
    }
  }
}
  ...
  while(!isExit()) {
    topicReconn_.op([&](unordered_map<string, pair<long long, long long>> &mp) {
      ...
      for(auto &p : mp) {
        ...
        if(info.resubscribeTimeout_ > 0 &&
           (std::chrono::steady_clock::now() - info.subscribeTime_) > std::chrono::milliseconds(info.resubscribeTimeout_)) {
  ...
  while(isExit() == false) {
    ...
    if(UNLIKELY(flag != previousDataFormFlag)) {
      ...
      if(UNLIKELY(unmarshall == nullptr)) {
        cerr << "[ERROR] Invalid data from: 0x" << std::hex << flag << " , unable to continue. Will stop this parseMessage thread." << endl;
        ret = OTHERERR;
        continue;
      }
        ...
        if(topicSubInfos_.find(t, info)) {
          ...
          if(info.isEvent_) {
            ...
            for(int rowIdx = 0; rowIdx < rowSize; ++rowIdx) {
              Message m((ConstantSP)rows[rowIdx], symbols[rowIdx], info.streamDeserializer_, startOffset++);
              rows[rowIdx].clear();
              info.queue_->push(m);
            }
          } else {
          ...
          topicSubInfos_.op([&](unordered_map<string, SubscribeParam> &mp) {
            if(mp.count(t) != 0) mp[t].offset_ = offset + 1;
          });
        }
      }
    } else {
...
void StreamingClientImpl::sendPublishRequest(DBConnection &conn, SubscribeParam &info) {
  ...
  if(info.userName_.empty()) {
    re = run(conn, "publishTable", getLocalIP(), listeningPort_, info.info_.tableName, info.info_.actionName, info.offset_, info.filter_,
             info.allowExists_);
  } else {
...
std::string StreamingClientImpl::subscribeInternal(DBConnection &conn, SubscribeParam &info) {
  if(info.userName_.empty() == false) conn.login(info.userName_, info.password_, HAS_OPENSSL);
...
void StreamingClientImpl::insertMeta(SubscribeParam &info, const string &topic) {
  topicSubInfos_.upsert(topic, [&](SubscribeParam &_info) { _info = info; }, info);
  liveSubsOnSite_.upsert(getSite(topic), [&](set<string> &s) { s.insert(topic); }, {topic});
  actionCntOnTable_.upsert(stripActionName(topic), [&](int &cnt) { ++cnt; }, 1);
  topics_[info.ID_] = topic;
}

bool StreamingClientImpl::delMeta(const string &topic, bool exitFlag) {
  if(topicSubInfos_.count(topic) < 1) return false;
  ...
  actionCntOnTable_.upsert(stripActionName(topic), [&](int &cnt) { --cnt; }, 0);
  if(exitFlag) oldinfo.exit();
  return true;
}

SubscribeQueue StreamingClientImpl::subscribeInternal(const SubscribeInfo &subInfo, int64_t offset, bool resubscribe, const VectorSP &filter,
                                                      bool msgAsTable, bool allowExists, int batchSize, const string &userName,
                                                      const string &password, const StreamDeserializerSP &blobDeserializer,
                                                      const std::vector<std::string> &backupSites, bool isEvent, int resubscribeInterval,
                                                      bool subOnce, bool convertMsgRowData, int resubscribeTimeout) {
  ...
  while(isExit() == false) {
    ++attempt;
    SubscribeParam info(_id, subInfo, offset, resubscribe, filter, msgAsTable, allowExists, batchSize, userName, password, blobDeserializer, isEvent,
                        resubscribeInterval, subOnce, convertMsgRowData, resubscribeTimeout);
...
bool StreamingClientImpl::unsubscribeInternal(const SubscribeInfo &subInfo) {
  ...
  if(iter == topics_.end()) {
    ...
    auto iter1 = std::find_if(haStreamTableInfo_.begin(), haStreamTableInfo_.end(), [&subInfo](const HAStreamTableInfo &info) {
      return info.followIp == subInfo.hostName && info.followPort == subInfo.port && info.tableName == subInfo.tableName &&
             info.action == subInfo.actionName;
    });
    if(iter1 != haStreamTableInfo_.end()) {
      ...
      auto iter2 = std::find_if(haStreamTableInfo_.begin(), haStreamTableInfo_.end(), [&subInfo](const HAStreamTableInfo &info) {
        return info.leaderIp == subInfo.hostName && info.leaderPort == subInfo.port && info.tableName == subInfo.tableName &&
               info.action == subInfo.actionName;
      });
  ...
  delMeta(topic, true);
  topicReconn_.op([&topic](unordered_map<string, pair<long long, long long>> &mp) { mp.erase(topic); });
  {
    LockGuard<Mutex> _(&mtx_);
...
StreamingClient::StreamingClient(const StreamingClientConfig &config) : impl_(new StreamingClientImpl(0)) {
...
StreamingClient::~StreamingClient() { exit(); }

void StreamingClient::exit() { impl_->exit(); }

bool StreamingClient::isExit() { return impl_->isExit(); }

SubscribeQueue StreamingClient::subscribeInternal(const SubscribeInfo &info, int64_t offset, bool resubscribe, const dolphindb::VectorSP &filter,
                                                  bool msgAsTable, bool allowExists, int batchSize, const std::string &userName,
                                                  const std::string &password, const StreamDeserializerSP &blobDeserializer,
                                                  const std::vector<std::string> &backupSites, bool isEvent, int resubscribeInterval, bool subOnce,
                                                  bool convertMsgRowData, int resubscribeTimeout) {
  return impl_->subscribeInternal(info, offset, resubscribe, filter, msgAsTable, allowExists, batchSize, userName, password, blobDeserializer,
                                  backupSites, isEvent, resubscribeInterval, subOnce, convertMsgRowData, resubscribeTimeout);
}
...
size_t ThreadedClient::getQueueDepth(const ThreadSP &thread) { return impl_->getQueueDepth(thread); }

ThreadSP ThreadedClient::subscribe(string host, int port, const MessageBatchHandler &handler, string tableName, string actionName, int64_t offset,
                                   bool resub, const VectorSP &filter, bool allowExists, int batchSize, double throttle, bool msgAsTable,
                                   const std::string &userName, const std::string &password, const StreamDeserializerSP &blobDeserializer,
                                   const std::vector<std::string> &backupSites, int resubscribeInterval, bool subOnce, int resubscribeTimeout) {
  SubscribeInfo info{std::move(host), port, std::move(tableName), std::move(actionName)};
  auto subscribeQueue = subscribeInternal(info, offset, resub, filter, msgAsTable, allowExists, batchSize, userName, password, blobDeserializer,
...
void handleConvertRowData(Message &msg, const MessageHandler &handler, ConstantSP &callBackArgs, vector<ConstantSP> &callBackScalarArgs,
                          vector<DATA_TYPE> &types, vector<int> &extras) {
...
ThreadSP newHandleThread(const MessageHandler &handler, const SubscribeQueue &subscribeQueue, bool msgAsTable, bool isStreamDeserialize,
                         const std::shared_ptr<StreamingClientImpl> &impl) {
...
ThreadSP ThreadedClient::subscribe(string host, int port, const MessageHandler &handler, string tableName, string actionName, int64_t offset,
                                   bool resub, const VectorSP &filter, bool msgAsTable, bool allowExists, const std::string &userName,
                                   const std::string &password, const StreamDeserializerSP &blobDeserializer,
                                   const std::vector<std::string> &backupSites, int resubscribeInterval, bool subOnce, int resubscribeTimeout) {
  SubscribeInfo info{std::move(host), port, std::move(tableName), std::move(actionName)};
#ifdef USE_AERON
...
#endif
  auto subscribeQueue = subscribeInternal(info, offset, resub, filter, msgAsTable, allowExists, 1, userName, password, blobDeserializer, backupSites,
                                          false, resubscribeInterval, subOnce, true, resubscribeTimeout);
  if(subscribeQueue.queue_.isNull()) {
    cerr << "Subscription already made, handler loop not created." << endl;
    ThreadSP t = new Thread(new Executor([]() {}));
    t->start();
...
MessageQueueSP PollingClient::subscribe(string host, int port, string tableName, string actionName, int64_t offset, bool resub,
                                        const VectorSP &filter, bool msgAsTable, bool allowExists, const std::string &userName,
                                        const std::string &password, const StreamDeserializerSP &blobDeserializer,
                                        const std::vector<std::string> &backupSites, int resubscribeInterval, bool subOnce, int resubscribeTimeout) {
  SubscribeInfo info{std::move(host), port, std::move(tableName), std::move(actionName)};
  return subscribeInternal(info, offset, resub, filter, msgAsTable, allowExists, 1, userName, password, blobDeserializer, backupSites, false,
                           resubscribeInterval, subOnce, false, resubscribeTimeout)
      .queue_;
}
...
ThreadPooledClient::ThreadPooledClient(int listeningPort, int threadCount) : StreamingClient(listeningPort), threadCount_(threadCount) {}

bool ThreadPooledClient::unsubscribe(string host, int port, string tableName, string actionName) {
  return unsubscribeInternal(std::move(host), port, std::move(tableName), std::move(actionName));
}

size_t ThreadPooledClient::getQueueDepth(const ThreadSP &thread) { return impl_->getQueueDepth(thread); }

vector<ThreadSP> ThreadPooledClient::subscribe(string host, int port, const MessageHandler &handler, string tableName, string actionName,
                                               int64_t offset, bool resub, const VectorSP &filter, bool msgAsTable, bool allowExists,
                                               const std::string &userName, const std::string &password, const StreamDeserializerSP &blobDeserializer,
                                               const std::vector<std::string> &backupSites, int resubscribeInterval, bool subOnce,
                                               int resubscribeTimeout) {
  SubscribeInfo info{std::move(host), port, std::move(tableName), std::move(actionName)};
  auto queue = subscribeInternal(info, offset, resub, filter, msgAsTable, allowExists, 1, userName, password, blobDeserializer, backupSites, false,
                                 resubscribeInterval, subOnce, true, resubscribeTimeout);
  vector<ThreadSP> ret;
...
EventClient::EventClient(const std::vector<EventSchema> &eventSchemas, const std::vector<std::string> &eventTimeFields,
                         const std::vector<std::string> &commonFields)
    : StreamingClient(0), eventHandler_(eventSchemas, eventTimeFields, commonFields) {}

ThreadSP EventClient::subscribe(const std::string &host, int port, const EventMessageHandler &handler, const std::string &tableName,
                                const std::string &actionName, int64_t offset, bool resub, const std::string &userName, const std::string &password,
                                int resubscribeTimeout) {
  if(tableName.empty()) {
  ...
  tempConn.close();
  SubscribeInfo info{host, port, tableName, actionName};
  SubscribeQueue subscribeQueue = subscribeInternal(info, offset, resub, nullptr, false, false, 1, userName, password, nullptr,
                                                    std::vector<std::string>(), true, 100, false, false, resubscribeTimeout);
  if(subscribeQueue.queue_.isNull()) {
    cerr << "Subscription already made, handler loop not created." << endl;
    return nullptr;
  }
...
ThreadSP IPCInMemoryStreamClient::newHandleThread(const std::string &tableName, const IPCInMemoryTableReadHandler &handler,
                                                  const SmartPointer<IPCInMemTable> &memTable, const TableSP &outputTable, bool overwrite) {
...
SmartPointer<IPCInMemTable> loadIPCInMemoryTableAndCheckSchema(const std::string &tableName, const TableSP &outputTable, string &errMsg) {
  ...
  SmartPointer<IPCInMemTable> memTable = new IPCInMemTable(false, shmPath, tableName, cols, it->second.colNames_, bufSize);

  return memTable;
}

ThreadSP IPCInMemoryStreamClient::subscribe(const std::string &tableName, const IPCInMemoryTableReadHandler &handler, const TableSP &outputTable,
                                            bool overwrite) {
  if(tableName.empty()) {
