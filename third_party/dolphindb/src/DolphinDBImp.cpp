...
DBConnectionImpl::DBConnectionImpl(bool sslEnable, bool asynTask, int keepAliveTime, bool compress, bool python, bool isReverseStreaming,
    ...
    : port_(0),
      encrypted_(false),
      isConnected_(false),
      littleEndian_(Util::isLittleEndian()),
...
bool DBConnectionImpl::connect(const std::string& hostName, int port, const std::string& userId, const std::string& password, bool sslEnable,
                               bool asynTask, int keepAliveTime, bool compress, bool python) {
  hostName_ = hostName;
  port_ = port;
  userId_ = userId;
  pwd_ = password;
...
bool DBConnectionImpl::connect() {
  ...
  if(ret != OK) throw IOException("Couldn't send login message to the given host/port with IO error type " + std::to_string(ret));

  DataInputStreamSP in = new DataInputStream(conn);
  string line;
  ret = in->readLine(line);
  if(ret != OK) throw IOException("Failed to read message from the socket with IO error type " + std::to_string(ret));

  std::vector<string> headers;
  Util::split(line.c_str(), ' ', headers);
  if(headers.size() != 3) throw IOException("Received invalid header");
  string sessionId = headers[0];
  int numObject = atoi(headers[1].c_str());
  bool remoteLittleEndian = (headers[2] != "0");

  if((ret = in->readLine(line)) != OK) throw IOException("Failed to read response message from the socket with IO error type " + std::to_string(ret));
  if(line != "OK") throw IOException("Server connection response: '" + line);

  if(numObject == 1) {
    uint16_t flag;
    if((ret = in->read(flag)) != OK) throw IOException("Failed to read object flag from the socket with IO error type " + std::to_string(ret));
    ...
    unmarshall->reset();
    if(result->getBool() == 0) throw IOException("Failed to authenticate the user");
  }
...
void DBConnectionImpl::login(const std::string& userId, const std::string& password, bool enableEncryption) {
  ...
  if(result->getBool() == 0) throw IOException("Failed to authenticate the user " + userId_);
}
...
void DBConnectionImpl::scramLogin() {
  ...
  PKCS5_PBKDF2_HMAC(pwd_.c_str(), static_cast<int>(pwd_.length()), salt.data(), static_cast<int>(salt.size()), iterCount, EVP_sha256(),
                    SHA256_DIGEST_LENGTH, saltedPassword.data());

  // calculate clientKey
  std::vector<uint8_t> clientKey(SHA256_DIGEST_LENGTH);
  HMAC(EVP_sha256(), saltedPassword.data(), static_cast<int>(saltedPassword.size()), (const uint8_t*)"Client Key", 10, clientKey.data(), nullptr);
  ...
  // calculate auth
  string authMessage = "n=" + userId_ + ",r=" + clientNonce + "," + "r=" + combinedNonce + ",s=" + saltString + ",i=" + std::to_string(iterCount) +
                       "," + "c=biws,r=" + combinedNonce;

  // calculate client signature
  std::vector<uint8_t> clientSignature(SHA256_DIGEST_LENGTH);
  HMAC(EVP_sha256(), storedKey.data(), static_cast<int>(storedKey.size()), (const uint8_t*)authMessage.c_str(), authMessage.size(),
  ...
  HMAC(EVP_sha256(), saltedPassword.data(), static_cast<int>(saltedPassword.size()), (const uint8_t*)"Server Key", 10, serverKey.data(), nullptr);

  // check server signature
  std::vector<uint8_t> serverSignature(SHA256_DIGEST_LENGTH);
  HMAC(EVP_sha256(), serverKey.data(), static_cast<int>(serverKey.size()), (const uint8_t*)authMessage.c_str(), authMessage.length(),
...
ConstantSP DBConnectionImpl::run(const std::string& funcName, std::vector<ConstantSP>& args, int priority, int parallelism, int fetchSize,
                                 bool clearMemory, long seqNum) {
...
ConstantSP DBConnectionImpl::upload(const std::string& name, const ConstantSP& obj) {
  if(!Util::isVariableCandidate(name)) throw RuntimeException(name + " is not a qualified variable name.");
  if(obj.isNull()) {
...
ConstantSP DBConnectionImpl::upload(std::vector<string>& names, std::vector<ConstantSP>& objs) {
  if(names.size() != objs.size()) throw RuntimeException("the size of variable names doesn't match the size of objects.");
  if(names.empty()) return Constant::void_;

  string varNames;
  for(unsigned int i = 0; i < names.size(); ++i) {
    if(!Util::isVariableCandidate(names[i])) throw RuntimeException(names[i] + " is not a qualified variable name.");
    if(i > 0) varNames.append(1, ',');
...
ConstantSP DBConnectionImpl::run(const std::string& script, const std::string& scriptType, std::vector<ConstantSP>& args, int priority,
                                 int parallelism, int fetchSize, bool clearMemory, long seqNum) {
...
#endif
  if(!isConnected_) throw IOException("Couldn't send script/function to the remote host because the connection has been closed");

  if(fetchSize < 8192 && fetchSize != 0) throw IOException("fetchSize must be greater than 8192 and not less than 0");
  string body;
  ...
  if(argCount > 0) {
    ...
    if(ret != OK) {
      close();
      throw IOException("Couldn't send script/function to the remote host because the connection has been closed, IO error type " +
                        std::to_string(ret));
    }
  }

  if(asynTask_) return new Void();

  if(static_cast<int>(littleEndian_) != (char)Util::isLittleEndian()) inputStream_->enableReverseIntegerByteOrder();
  ...
  if(fetchSize > 0 && form == DF_VECTOR && type == DT_ANY) return new BlockReader(inputStream_);
