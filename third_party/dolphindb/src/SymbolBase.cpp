...
int SymbolBase::serialize(char* buf, int bufSize, INDEX indexStart, int offset, int& numElement, int& partial) const {
  if(indexStart >= (INDEX)syms_.size()) return -1;
...
int SymbolBase::find(const std::string& symbol) {
  if(symMap_.empty()) {
    if(!syms_.empty() && !syms_[0].empty()) throw RuntimeException("A symbol base's first key must be empty string.");
    if(syms_.empty()) {
      symMap_[""] = 0;
      syms_.emplace_back("");
    } else {
      int count = static_cast<int>(syms_.size());
      for(int i = 0; i < count; ++i) symMap_[syms_[i]] = i;
    }
  }
  int index = -1;
