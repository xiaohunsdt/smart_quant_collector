...
StreamReplicator::StreamReplicator(const std::vector<HostInfo> &hosts, const std::string &tableName, const ReplicatorConfig &config) {
...
StreamReplicator::BlockIt StreamReplicator::flush(const std::shared_ptr<DBConnection> &conn, ReplicatorStreamStatus &status, BlockIt front,
                                                  bool flushAll) {
  while(true) {
    ConstantSP data;
    int maxTry;
