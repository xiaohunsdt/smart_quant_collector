...
MTWConfig& MTWConfig::setCompression(const std::vector<COMPRESS_METHOD>& compressMethods) {
  if(compressMethods.size() != columnInfo_.size()) {
    throw RuntimeException(
        ...
        "Table column number: " +
        std::to_string(columnInfo_.size()) + ".");
  }
  inited_ = true;
...
MTWConfig& MTWConfig::setThreads(const size_t threadNum, const std::string& partitionColumnName) {
  ...
  if(partitionColumnNames_.empty()) {
    column = std::find_if(columnInfo_.begin(), columnInfo_.end(),
                          [partitionColumnName](const std::pair<std::string, int>& col) { return col.first == partitionColumnName; });
  } else {
    auto partitionColumn = std::find_if(partitionColumnNames_.begin(), partitionColumnNames_.end(),
                                        [partitionColumnName](const std::string& col) { return col == partitionColumnName; });
...
MultithreadedTableWriter::MultithreadedTableWriter(const MTWConfig& config) {
  ...
  if(callbackFunc_ != nullptr) {
    ...
    if(threadByColIndexForNonPartion_ >= 0) threadByColIndexForNonPartion_++;
    if(partitionColumnIdx_ >= 0) partitionColumnIdx_++;
  }
  ...
  for(unsigned int i = 0; i < threads_.size(); i++) {
    ...
    writerThread.conn->onConnectionStateChange(
        [this](const ConnectionState state, const std::string& host, int port) { return stateCallback(state, host, port); });
...
MultithreadedTableWriter::MultithreadedTableWriter(const std::string& hostName, int port, const std::string& userId, const std::string& password,
                                                   const std::string& dbName, const std::string& tableName, bool useSSL, bool enableHighAvailability,
                                                   const vector<string>* pHighAvailabilitySites, int batchSize, float throttle, int threadCount,
                                                   const std::string& partitionCol, const vector<COMPRESS_METHOD>* pCompressMethods,
                                                   MultithreadedTableWriter::Mode mode, vector<string>* pModeOption,
                                                   const std::function<void(ConstantSP)>& callbackFunc, bool enableStreamTableTimestamp)
    : dbName_(dbName),
      tableName_(tableName),
    ...
    if(isPartionedTable_) {
      ...
      if(partColNames->isScalar()) {
        if(partColNames->getString() != partitionCol) {
          throw RuntimeException("The parameter partionCol must be the partitioning column '" + partColNames->getString() +
        ...
        if(index < 0) throw RuntimeException("The parameter partionCol must be the partitioning columns in the partitioned table");
...
void MultithreadedTableWriter::setError(const ErrorCodeInfo& errorInfo) {
  if(hasError_.exchange(true)) return;
...
bool MultithreadedTableWriter::insert(std::vector<ConstantSP>** records, int recordCount, ErrorCodeInfo& errorInfo, bool isNeedReleaseMemory) {
  ...
  for (auto &row : vectorOfVector) {
      if (row.size() != colNames_.size()) {
          errorInfo.set(ErrorCodeInfo::EC_InvalidObject, "Invalid vector size " + std::to_string(row.size()) + ", expect
  " + std::to_string(colNames_.size())); return false;
      }
      int index = 0;
      DATA_TYPE dataType;
      for (auto &param : row) {
          ...
          if (param->getType() != dataType && dataType != DATA_TYPE::DT_SYMBOL) {
              errorInfo.set(ErrorCodeInfo::EC_InvalidObject, "Object type mismatch " +
  Util::getDataTypeString(param->getType()) + ", expect " + Util::getDataTypeString(dataType)); return false;
          }
          index++;
      }
  }
...
void MultithreadedTableWriter::getStatus(Status& status) {
  ...
  for(size_t i = 0; i < threads_.size(); i++) {
    ...
    threadStatus.unsentRows =
        static_cast<long>(((writeThread.writeQueue_.size() - 1) * perBlockSize_) + writeThread.writeQueue_.back()->front()->size());
...
void MultithreadedTableWriter::callBack(const std::function<void(ConstantSP)>& callbackFunc, bool result, vector<ConstantSP>* block) {
  ...
  auto table = (TableSP)Util::createTable(std::vector<std::string>({"callbackId", "isSuccess"}),
                                          std::vector<ConstantSP>({(ConstantSP)callbackIds, (ConstantSP)results.createVector(DT_BOOL)}));
  ...
  while(isExit() == false) {
    {
      ...
      if(tableWriter_.batchSize_ > 1 && tableWriter_.throttleMilsecond_ > 0) {
        ...
        while(isExit() == false &&
              (writeThread_.writeQueue_.size() - 1) * tableWriter_.perBlockSize_ + writeThread_.writeQueue_.back()->front()->size() <
                  static_cast<std::size_t>(tableWriter_.batchSize_)) {  // check batchsize
