...
std::vector<int> HashDomain::getPartitionKeys(const ConstantSP& partitionColTable) const {
  if(partitionColTable->getCategory() != partitionColCategory_) throw RuntimeException("Data category incompatible.");
  ...
  if(partitionColCategory_ == TEMPORAL && partitionColType_ != partitionCol->getType()) {
    ...
    if(partitionCol == nullptr) throw RuntimeException("Can't convert partition column");
  }
...
std::vector<int> ValueDomain::getPartitionKeys(const ConstantSP& partitionColTable) const {
  ...
  if(partitionColType_ == DT_LONG) throw RuntimeException("Long type value can't be used as a partition column.");
  int rows = partitionCol->rows();
  std::vector<int> keys(rows);
  INDEX count = 0;
  INDEX start = 0;
