...
TableSP SharedMemStream::readData(TableSP& copy, const bool& overwrite, size_t& readRows, int& microCost) {
  ...
  auto* dataHeader = (DataHeader*)(dataStartBuf);

  // LOG_INFO("SharedMemStream readData, write_page:", shmpHeader_->writePage_, ", read_page:", shmpHeader_->readPage_,
  // ", data length:",
  ...
  if(!copy.isNull()) {
    if(dataHeader->rows_ + startRows > copy->size()) {
      for(int colIndex = 0; colIndex < copy->columns(); colIndex++) {
        ...
        auto colData = (VectorSP)copy->getColumn(colIndex);
  ...
  for(unsigned int i = 0; i < tableMetaIt->second.colLength_.size(); i++) {
    auto colToWrite = (VectorSP)table->getColumn(i);
    if(table->getColumnType(i) == DT_STRING || table->getColumnType(i) == DT_SYMBOL || table->getColumnType(i) == DT_BLOB) {
      ...
      for(int j = 0; j < dataHeader->rows_;) {
        ...
        void* colData = colToWrite->getDataBuffer(startRows + j, colDataLength, nullptr);
  ...
  // LOG_INFO("shared mem get data, micro cost:", IpcMemUtil::calculateMicroCost(writeTimeSeconds,
  // writeTimeMicroSeconds));
...
bool SharedMemStream::openSharedMem(std::string& errMsg) {
  ...
  void* startPos = mmap(nullptr, length, PROT_READ | PROT_WRITE, MAP_SHARED, shmFd_, 0);
...
void SharedMemStream::parseTableMeta(bool print) {
  ...
  for(unsigned int i = 0; i < shmpHeader_->tableCnt_; i++) {
    auto* tablemeta = (TableMeta*)(shmpHeader_->meta_ + offset);
    ...
    if(it == tableMeta_.end()) {
      ...
      for(int j = 0; j < tablemeta->colNum_; j++) {
        ...
        tableData.colNames_.emplace_back(colmeta->name_, colmeta->nameLen_);
...
IPCInMemTable::IPCInMemTable(bool create, std::string shmPath, std::string tableName, const std::vector<ConstantSP>& cols,
                             const std::vector<std::string>& colNames, size_t bufSize)
    : BasicTable(cols, colNames), tablename_(std::move(tableName)) {
  pShmStream_ = std::make_shared<SharedMemStream>(create, std::move(shmPath), bufSize);
...
IPCInMemTable::~IPCInMemTable() { closeShm(); }
...
int IPCInMemTable::getMicroCost() { return microCost_; }

void IPCInMemTable::closeShm() {
  if(pShmStream_) {
    pShmStream_->closeShm();
  }
}
