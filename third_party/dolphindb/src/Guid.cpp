...
uint32_t murmur32(const char* key, size_t len) {
  ...
  switch(len) {
    case 3:
      h ^= data[2] << 16;  // falls through
    /* no break */
    case 2:
      h ^= data[1] << 8;  // falls through
    /* no break */
    case 1:
      h ^= data[0];
      h *= m;
...
Guid::Guid(unsigned char* guid) { memcpy(uuid_, guid, 16); }

Guid::Guid(const std::string& guid) {
  if(guid.size() != 36 || !Util::fromGuid(guid.c_str(), uuid_)) throw RuntimeException("Invalid UUID string");
}

Guid::Guid(const Guid& copy) { memcpy(uuid_, copy.uuid_, 16); }
...
std::string Guid::getString() const { return getString(uuid_); }
...
uint64_t GuidHash::operator()(const Guid& guid) const { return murmur32_16b(guid.bytes()); }
