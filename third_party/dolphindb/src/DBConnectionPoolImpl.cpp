...
DBConnectionPoolImpl::DBConnectionPoolImpl(const std::string& hostName, int port, int threadNum, const std::string& userId,
                                           const std::string& password, bool loadBalance, bool highAvailability, bool compress, bool reConnect,
                                           bool python)
  ...
  if(!loadBalance) {
    for(int i = 0; i < threadNum; i++) {
      ...
      if(!ret) throw IOException("Failed to connect to " + hostName + ":" + std::to_string(port));
    ...
    for(int i = 0; i < nodeCount; i++) {
      ...
      if(p == std::string::npos) throw RuntimeException("Invalid data node address: " + fields);
    ...
    for(int i = 0; i < threadNum; i++) {
      ...
      if(!ret) throw IOException("Failed to connect to " + curhost + ":" + std::to_string(curport));
