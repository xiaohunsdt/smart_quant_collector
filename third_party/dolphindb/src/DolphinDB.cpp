...
#ifdef INDEX64
namespace std {
int min(int a, INDEX b) { return a < b ? a : (int)b; }

int min(INDEX a, int b) { return a < b ? (int)a : b; }
}  // namespace std
#endif
...
ConstantSP Constant::getRowLabel() const { return void_; }

ConstantSP Constant::getColumnLabel() const { return void_; }
...
DBConnection::DBConnection(bool enableSSL, bool asyncTask, int keepAliveTime, bool compress, bool python, bool isReverseStreaming, bool enableSCRAM)
    : conn_(new DBConnectionImpl(enableSSL, asyncTask, keepAliveTime, compress, python, isReverseStreaming, enableSCRAM)),
      ha_(false),
      enableSSL_(enableSSL),
      enableSCRAM_(enableSCRAM),
      asynTask_(asyncTask),
      compress_(compress),
      nodes_({}),
      lastConnNodeIndex_(0),
      python_(python),
      reconnect_(false),
      closed_(true),
      runSeqNo_(0) {
...
DBConnection::DBConnection(DBConnection&& oth) noexcept
    : conn_(std::move(oth.conn_)),
      ...
      ha_(oth.ha_),
      keepAliveTime_(oth.keepAliveTime_),
...
bool DBConnection::connect(const std::string& hostName, int port, const std::string& userId, const std::string& password, const std::string& startup,
  ...
  closed_ = false;
  if(ha_) {
    for(const auto& one : highAvailabilitySites) nodes_.emplace_back(one);
    {
      ...
      if(!foundfirst) nodes_.push_back(firstnode);
    }
    std::random_device rd;
    std::mt19937 g(rd());
    ...
    for(auto& one : nodes_) {
      if(pMinNode == nullptr || (one.load_ >= 0 && pMinNode->load_ > one.load_)) {
        pMinNode = &one;
      }
    }
    ...
    if(reconnect_) {
      ...
      if(!connectNode(hostName, port, keepAliveTime)) return false;
    }
  }
...
bool DBConnection::reconnect() {
  ...
  state_ = ConnectionState::Initializing;
  closed_ = false;
  if(!connectNode(host_, port_, keepAliveTime_)) return false;
...
void DBConnection::switchDataNode(const string& host, int port) {
  ...
  if(isConnected && initialScript_.empty() == false) run(initialScript_);
}
...
int DBConnection::publishTable(const SubscribeInfo& info, const SubscribeConfig& config, TransportationProtocol protocol) {
  ...
  auto ret = call("publishTable", hostName, port, info.tableName, info.actionName, config.offset, ConstantSP(Util::createConstant(DT_VOID)),
                  config.allowExists, config.resetOffset, isUDP);
  if(isUDP) {
...
ConstantSP DBConnection::run(const std::string& funcName, vector<dolphindb::ConstantSP>& args, int priority, int parallelism, int fetchSize,
                             bool clearMemory) {
  if(parallelism <= 0) {
    throw IllegalArgumentException(DDB_FUNCNAME, "Invalid parallelism.");
  }
...
const std::string& DBConnection::getInitScript() const { return initialScript_; }

DataInputStreamSP DBConnection::getDataInputStream() { return conn_->getDataInputStream(); }

void DBConnection::setInitScript(const std::string& script) { initialScript_ = script; }

BlockReader::BlockReader(const DataInputStreamSP& in) : in_(in), total_(0), currentIndex_(0) {
  int rowNum, colNum;
  if(in->read(rowNum) != OK) throw IOException("Failed to read rows for data block.");
  if(in->read(colNum) != OK) throw IOException("Faield to read col for data block.");
  total_ = (long long)rowNum * (long long)colNum;
}

ConstantSP BlockReader::read() {
  if(currentIndex_ >= total_) return nullptr;
  IO_ERR ret;
  uint16_t flag;
  if((ret = in_->read(flag)) != OK) throw IOException("Failed to read object flag from the socket with IO error type " + std::to_string(ret));
  ...
  if(unmarshall == nullptr) throw IOException("Failed to parse the incoming object" + std::to_string(form));
...
void BlockReader::skipAll() { while(read().isNull() == false); }
...
int DBConnectionPool::run(std::string script, std::shared_ptr<std::condition_variable> finished, const RpcParam& param) {
  auto id = id_++;
  pool_->run(std::move(script), id, std::move(finished), param);
  return id;
}

int DBConnectionPool::run(std::string functionName, const std::vector<ConstantSP>& args, std::shared_ptr<std::condition_variable> finished,
                          const RpcParam& param) {
  auto id = id_++;
...
void DBConnectionPool::run(const std::string& script, int identity, int priority, int parallelism, int fetchSize, bool clearMemory) {
  if(identity < 0) throw RuntimeException("Invalid identity: " + std::to_string(identity) + ". Identity must be a non-negative integer.");
  if(parallelism <= 0) {
    throw IllegalArgumentException(DDB_FUNCNAME, "Invalid parallelism.");
  }
...
void DBConnectionPool::run(const std::string& functionName, const vector<ConstantSP>& args, int identity, int priority, int parallelism,
...
bool DBConnectionPool::isFinished(int identity) { return pool_->isFinished(identity); }

ConstantSP DBConnectionPool::getData(int identity) { return pool_->getData(identity); }

void DBConnectionPool::shutDown() { pool_->shutDown(); }

bool DBConnectionPool::isShutDown() { return pool_->isShutDown(); }

int DBConnectionPool::getConnectionCount() { return pool_->getConnectionCount(); }

PartitionedTableAppender::PartitionedTableAppender(const std::string& dbUrl, const std::string& tableName, const std::string& partitionColName,
...
PartitionedTableAppender::PartitionedTableAppender(const std::string& dbUrl, const std::string& tableName, const std::string& partitionColName,
                                                   const std::string& appendFunction, DBConnectionPool& pool) {
  pool_ = pool.pool_;
  init(dbUrl, tableName, partitionColName, appendFunction);
}

void PartitionedTableAppender::init(const std::string& dbUrl, const std::string& tableName, const std::string& partitionColName,
                                    const std::string& appendFunction) {
  ...
  try {
    ...
    if(partColNames->isNull()) throw RuntimeException("Can't find specified partition column name.");

    if(partColNames->isScalar()) {
      if(partColNames->getString() != partitionColName) throw RuntimeException("Can't find specified partition column name.");
      ...
      if(index < 0) throw RuntimeException("Can't find specified partition column name.");
...
int PartitionedTableAppender::append(const TableSP& table) {
  if(cols_ != table->columns()) throw RuntimeException("The input table doesn't match the schema of the target table.");
  for(int i = 0; i < cols_; ++i) {
  ...
  for(int i = 0; i < threadCount_; ++i) chunkIndices_[i].clear();
  ...
  for(int i = 0; i < threadCount_; ++i) {
    if(chunkIndices_[i].empty()) continue;
...
void PartitionedTableAppender::checkColumnType(int col, DATA_CATEGORY category, DATA_TYPE type) {
  if(columnTypes_[col] != type) {
    ...
    if(category != expectCategory) {
      throw RuntimeException("column " + std::to_string(col) + ", expect category " + Util::getCategoryString(expectCategory) + ", got category " +
                             Util::getCategoryString(category));
    }  // else if (category == TEMPORAL && type != expectType) {
    //    throw  RuntimeException("column " + std::to_string(col) + ", temporal column must have exactly the same type,
    //    expect " + Util::getDataTypeString(expectType) + ", got " + Util::getDataTypeString(type));
    //}
  }
}
...
int AutoFitTableAppender::append(const TableSP& table) {
  if(cols_ != table->columns()) throw RuntimeException("The input table columns doesn't match the columns of the target table.");
...
int AutoFitTableUpsert::upsert(const TableSP& table) {
  ...
  if(res->getType() == DT_INT && res->getForm() == DF_SCALAR) return res->getInt();
  return 0;
}
...
void ErrorCodeInfo::set(int apiCode, const string& info) { set(formatApiCode(apiCode), info); }
...
void ErrorCodeInfo::set(const ErrorCodeInfo& src) { set(src.errorCode, src.errorInfo); }
