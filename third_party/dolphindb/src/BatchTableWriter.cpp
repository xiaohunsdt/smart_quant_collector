...
BatchTableWriter::BatchTableWriter(std::string hostName, int port, std::string userId, std::string password, bool acquireLock)
    : hostName_(std::move(hostName)), port_(port), userId_(std::move(userId)), password_(std::move(password)), acquireLock_(acquireLock) {}
...
void BatchTableWriter::addTable(const std::string& dbName, const std::string& tableName, bool partitioned) {
  ...
  if(!ret) throw RuntimeException("Failed to connect to server.");
  ...
  destTable->writeThread = std::thread([this, partitioned, destTable, destTableRawPtr]() {
    while(destTableRawPtr->destroy == false) {
      while(true) {
        if(writeTableAllData(destTable, partitioned) == false) break;
      }
...
bool BatchTableWriter::writeTableAllData(const SmartPointer<DestTable>& destTable, bool partitioned) {
  DestTable* destTableRawPtr = destTable.get();
  if(destTableRawPtr->finished) return false;
  bool ret;
  ...
  destTableRawPtr->writeQueue.pop(items, static_cast<int>(size));

  size = items.size();

  bool writedataok = true;
  {
    destTableRawPtr->writeTable = Util::createTable(destTableRawPtr->colNames, destTableRawPtr->colTypes, 0, static_cast<int>(size));
    INDEX insertedRows;
    std::string errMsg;
    for(std::size_t i = 0; i < size; i++) {
      ret = destTableRawPtr->writeTable->append(items[i], insertedRows, errMsg);
      if(!ret) {
        std::cerr << Util::createTimestamp(Util::getEpochTime())->getString() << " Backgroud thread of table (" << destTableRawPtr->dbName << " "
                  << destTableRawPtr->tableName << "). Failed to create table, with error: " << errMsg << std::endl;
        writedataok = false;
        break;
      }
    }
  ...
  if(writedataok) {
    try {
      ...
      args.reserve(1);
      if(destTableRawPtr->createTmpSharedTable.empty() == false) destTableRawPtr->conn->run(destTableRawPtr->createTmpSharedTable);
      args.emplace_back(destTableRawPtr->writeTable);
      destTableRawPtr->conn->run(destTableRawPtr->tableInsert, args);
      if(!partitioned) destTableRawPtr->conn->run(destTableRawPtr->saveTable);
      destTableRawPtr->sendedRows += static_cast<int>(size);
    } catch(std::exception& e) {
      writedataok = false;
      std::cerr << Util::createTimestamp(Util::getEpochTime())->getString() << " Backgroud thread of table (" << destTableRawPtr->dbName << " "
                << destTableRawPtr->tableName << "). Failed to send data to server, with exception: " << e.what() << std::endl;
    }
  }
  if(writedataok == false) {
    destTableRawPtr->finished = true;
    for(std::size_t i = 0; i < size; i++) destTableRawPtr->saveQueue.push(items[i]);
  }
  return writedataok;
}
...
TableSP BatchTableWriter::getAllStatus() {
  ...
  for(int i = 0; i < columnNum; i++) columnVecs.emplace_back(table->getColumn(i));
  int i = 0;
...
TableSP BatchTableWriter::getUnwrittenData(const std::string& dbName, const std::string& tableName) {
  ...
  for(int i = 0; i < size; i++) {
    ...
    if(!ret) throw RuntimeException("Failed to create table, with error: " + errMsg);
  }
  return table;
}
...
ConstantSP BatchTableWriter::createObject(int dataType, unsigned char val[]) { return createObject(dataType, (const unsigned char*)val); }
