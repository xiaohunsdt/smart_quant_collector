...
Uuid::Uuid(const unsigned char* uuid) { memcpy(uuid_, uuid, 16); }
...
Uuid::Uuid(const Uuid& copy) : Int128() { memcpy(uuid_, copy.uuid_, 16); }

Uuid* Uuid::parseUuid(const char* str, size_t len) { return new Uuid(str, len); }
...
IPAddr::IPAddr(const char* ip, int len) {
  if(len == 0 || !parseIPAddr(ip, len, uuid_)) memset(uuid_, 0, 16);
}
...
IPAddr* IPAddr::parseIPAddr(const char* str, size_t len) {
  ...
  if(parseIPAddr(str, len, buf)) return new IPAddr(buf);
  return nullptr;
}
...
bool IPAddr::parseIPAddr(const char* str, size_t len, unsigned char* buf) {
  ...
  int i = 0;
  for(i = 0; i < 4 && str[i] != '.'; ++i);
  if(i >= 4) return parseIP6(str, len, buf);
...
bool IPAddr::parseIP4(const char* str, size_t len, unsigned char* buf) {
  ...
  for(size_t i = 0; i <= len; ++i) {
    if(i == len || str[i] == '.') {
      if(curByte < 0 || curByte > 255 || byteIndex > 3) return false;
    ...
    if(i == len || str[i] == ':') {
      ...
      if(lastColonPos == (int)i - 1) {
        ...
        if(Util::LITTLE_ENDIAN_ORDER) {
          for(int k = 0; k < consecutiveZeros; ++k) buf[15 - byteIndex++] = 0;
        } else {
          for(int k = 0; k < consecutiveZeros; ++k) buf[byteIndex++] = 0;
        }
      } else {
        if(curByte > 65535 || byteIndex > 15) return false;
        if(Util::LITTLE_ENDIAN_ORDER) {
