...
bool Runnable::isRunning() { return status_.load() == 1; }

bool Runnable::isStarted() { return status_.load() >= 1; }

bool Runnable::isComplete() { return status_.load() == 2; }
...
RWLock::RWLock() {
  ...
  if(rc != 0) throw RuntimeException("Failed to initialize read write lock with errCode " + std::to_string(rc));
#endif
}
...
void RWLock::releaseRead() {
  ...
  if(rc != 0) throw RuntimeException("Failed to release shared lock with errCode " + std::to_string(rc));
#endif
}
...
#else
  int rc = pthread_rwlock_unlock(&lock_);
  if(rc != 0) throw RuntimeException("Failed to release exclusive lock with errCode " + std::to_string(rc));
#endif
}
...
bool CountDownLatch::resetCount(int count) {
  LockGuard<Mutex> lock(&mutex_);
  if(count_ > 0 || count <= 0) return false;
  count_ = count;
  return true;
}
...
Semaphore::Semaphore(int resources) {
  ...
  if(sem_ == nullptr) throw RuntimeException("Failed to create semaphore with error code " + std::to_string(GetLastError()));
#elif defined MAC
...
void Semaphore::acquire() {
  ...
  if(ret != WAIT_OBJECT_0) throw RuntimeException("Failed to acquire semaphore with error code " + std::to_string(GetLastError()));
#elif defined MAC
...
#ifdef _WIN32
  if(!ReleaseSemaphore(sem_, 1, nullptr)) throw RuntimeException("Failed to release semaphore with error code " + std::to_string(GetLastError()));
#elif defined MAC
