...
#ifdef BIT32
  char** buf = (char**)input;
  for(int i = 1; i < count; ++i) buf[i] = input[i].pointer;
...
void stringU8ScalarWriter(const U8& value, const ConstantSP& output) { output->setString(value.pointer); }

void doubleU8VectorReader(const ConstantSP& value, int start, int count, U8* output) { value->getDouble(start, count, (double*)output); }

void doubleU8ScalarReader(const ConstantSP& value, U8& output) { output.doubleVal = value->getDouble(); }

void doubleU8VectorWriter(U8* input, const ConstantSP& output, int start, int count) { output->setDouble(start, count, (double*)input); }

void doubleU8ScalarWriter(const U8& value, const ConstantSP& output) { output->setDouble(value.doubleVal); }
...
void floatU8VectorReader(const ConstantSP& value, int start, int count, U8* output) {
  ...
  for(int i = 0; i < count; i++) output[i].floatVal = pbuf[i];
}

void floatU8ScalarReader(const ConstantSP& value, U8& output) { output.floatVal = value->getFloat(); }
...
void floatU8VectorWriter(U8* input, const ConstantSP& output, int start, int count) {
  ...
  for(int i = 0; i < count; ++i) pbuf[i] = input[i].floatVal;
  output->setFloat(start, count, pbuf);
}

void floatU8ScalarWriter(const U8& value, const ConstantSP& output) { output->setFloat(value.floatVal); }

void longU8VectorReader(const ConstantSP& value, int start, int count, U8* output) { value->getLong(start, count, (long long*)output); }

void longU8ScalarReader(const ConstantSP& value, U8& output) { output.longVal = value->getLong(); }

void longU8VectorWriter(U8* input, const ConstantSP& output, int start, int count) { output->setLong(start, count, (long long*)input); }

void longU8ScalarWriter(const U8& value, const ConstantSP& output) { output->setLong(value.longVal); }
...
void intU8VectorReader(const ConstantSP& value, int start, int count, U8* output) {
  ...
  for(int i = 0; i < count; i++) output[i].intVal = pbuf[i];
}

void intU8ScalarReader(const ConstantSP& value, U8& output) { output.intVal = value->getInt(); }

void intU8VectorWriter(U8* input, const ConstantSP& output, int start, int count) {
  int* buf = (int*)input;
  int* pbuf = output->getIntBuffer(start, count, buf);
  for(int i = 0; i < count; ++i) pbuf[i] = input[i].intVal;
  output->setInt(start, count, pbuf);
}

void intU8ScalarWriter(const U8& value, const ConstantSP& output) { output->setInt(value.intVal); }
...
void shortU8VectorReader(const ConstantSP& value, int start, int count, U8* output) {
  ...
  for(int i = 0; i < count; i++) output[i].shortVal = pbuf[i];
}

void shortU8ScalarReader(const ConstantSP& value, U8& output) { output.shortVal = value->getShort(); }
...
void shortU8VectorWriter(U8* input, const ConstantSP& output, int start, int count) {
  ...
  for(int i = 0; i < count; ++i) pbuf[i] = input[i].shortVal;
  output->setShort(start, count, pbuf);
}

void shortU8ScalarWriter(const U8& value, const ConstantSP& output) { output->setShort(value.shortVal); }
...
void charU8VectorReader(const ConstantSP& value, int start, int count, U8* output) {
  ...
  for(int i = 0; i < count; i++) output[i].charVal = pbuf[i];
}

void charU8ScalarReader(const ConstantSP& value, U8& output) { output.charVal = value->getChar(); }
...
void charU8VectorWriter(U8* input, const ConstantSP& output, int start, int count) {
  ...
  for(int i = 0; i < count; ++i) pbuf[i] = input[i].charVal;
  output->setChar(start, count, pbuf);
}

void charU8ScalarWriter(const U8& value, const ConstantSP& output) { output->setChar(value.charVal); }
...
void boolU8ScalarReader(const ConstantSP& value, U8& output) { output.charVal = value->getBool(); }
...
void boolU8ScalarWriter(const U8& value, const ConstantSP& output) { output->setBool(value.charVal); }
...
ConstantSP AbstractDictionary::createValues(const ConstantSP& key) const {
  if(key->isScalar()) return Util::createConstant(type_);
  return Util::createVector(type_, key->size());
}

IntDictionary::IntDictionary(const unordered_map<int, U8>& dict, DATA_TYPE keyType, DATA_TYPE type) : AbstractDictionary(keyType, type), dict_(dict) {
  if(type_ != DT_STRING) return;
  auto it = dict_.begin();
  auto end = dict_.end();
  size_t len;
  char* tmp;
...
IntDictionary::~IntDictionary() {
  if(type_ != DT_STRING && type_ != DT_BLOB) return;
  auto it = dict_.begin();
  auto end = dict_.end();
  while(it != end) {
...
bool IntDictionary::remove(const ConstantSP& key) {
  if(key->isScalar())
    ...
    while(start < len) {
      ...
      for(int i = 0; i < counts; ++i) dict_.erase(pbuf[i]);
      start += counts;
    }
  }
  return true;
...
bool IntDictionary::set(const ConstantSP& key, const ConstantSP& value) {
  ConstantSP newKey;
  if(keyCategory_ == TEMPORAL && key->getType() != keyType_) return false;
  newKey = key;
  ...
  if(dict_.empty()) dict_.reserve((int)(len * 1.33));
  const int bufSize = Util::BUF_SIZE;
  int buf[bufSize];
  const int* pbuf;
  U8 tmp[bufSize];
  ...
  while(start < len) {
    ...
    if(type_ == DT_STRING) {
      ...
      for(int i = 0; i < counts; ++i) dict_[pbuf[i]] = tmp[i];
    }
    start += counts;
  }
  return true;
...
ConstantSP IntDictionary::getMember(const ConstantSP& key) const {
  ConstantSP newKey;
  if(keyCategory_ == TEMPORAL && key->getType() != keyType_) throw IncompatibleTypeException(keyType_, key->getType());
  newKey = key;
...
void IntDictionary::contain(const ConstantSP& target, const ConstantSP& resultSP) const {
  if(target->isScalar()) {
    ...
    while(start < len) {
      ...
      for(int i = 0; i < counts; ++i) pret[i] = static_cast<char>(dict_.find(pbuf[i]) != end);
...
std::string IntDictionary::getString() const {
  ...
  if(len < (int)dict_.size()) content.append("...\n");
  return content;
}
...
FloatDictionary::FloatDictionary(const unordered_map<float, U8>& dict, DATA_TYPE type) : AbstractDictionary(DT_FLOAT, type), dict_(dict) {
  if(type_ != DT_STRING) return;
  auto it = dict_.begin();
  auto end = dict_.end();
  size_t len;
...
DoubleDictionary::DoubleDictionary(const unordered_map<double, U8>& dict, DATA_TYPE type) : AbstractDictionary(DT_DOUBLE, type), dict_(dict) {
  if(type_ != DT_STRING) return;
  auto it = dict_.begin();
  auto end = dict_.end();
  size_t len;
...
bool StringDictionary::remove(const ConstantSP& key) {
  if(key->getCategory() != LITERAL && key->getType() != DT_BLOB) throw RuntimeException("Key data type incompatible. Expecting literal/BLOB data");
  if(key->isScalar())
...
bool AnyDictionary::set(const ConstantSP& key, const ConstantSP& value) {
  ...
  if(key->isScalar()) {
    ...
    int len = key->size();
    if(len != value->size()) throw RuntimeException("Key size doesn't match value size.");
...
ConstantSP AnyDictionary::getMember(const std::string& key) const {
  auto it = dict_.find(key);
  if(it == dict_.end()) return Constant::void_;
  return it->second;
}

ConstantSP AnyDictionary::getMember(const ConstantSP& key) const {
  if(key->getCategory() != LITERAL) throw RuntimeException("Dictionary with 'ANY' data type must use string as key");
  if(key->isScalar()) {
...
std::string AnyDictionary::getString() const {
  ...
  while(counts < len) {
    ...
    if(form == DF_DICTIONARY) content.append("}");
    content.append(1, '\n');
    ++counts;
    ++it;
  }
...
long long AnyDictionary::getAllocatedMemory() const {
  ...
  while(it != end) {
    bytes = it->first.size();
    if(it->second.count() == 1) bytes += it->second->getAllocatedMemory();
    ++it;
  }
  return bytes;
}
...
bool AnyDictionary::containNotMarshallableObject() const {
  ...
  auto end = dict_.end();
  while(it != end) {
    if((*it).second->containNotMarshallableObject()) return true;
    ++it;
  }
  return false;
}
...
bool FloatAnyDictionary::set(const ConstantSP& key, const ConstantSP& value) {
  if(key->getRawType() != DT_FLOAT) throw RuntimeException("Key data type incompatible. Expecting Float");
  if(key->isScalar()) {
  ...
  int len = key->size();
  if(len != value->size() && value->size() != 1) return false;

  if(dict_.empty()) dict_.reserve((std::size_t)(len * 1.33));
...
void FloatAnyDictionary::contain(const ConstantSP& target, const ConstantSP& resultSP) const {
  if(target->getRawType() != DT_FLOAT) throw RuntimeException("Key data type incompatible. Expecting Float");
  if(target->isScalar()) {
    resultSP->setBool(static_cast<char>(dict_.find(target->getFloat()) != dict_.end()));
    return;
  }
...
bool DoubleAnyDictionary::set(const ConstantSP& key, const ConstantSP& value) {
  if(key->getRawType() != DT_DOUBLE) throw RuntimeException("Key data type incompatible. Expecting Double");
  if(key->isScalar()) {
...
void DoubleAnyDictionary::contain(const ConstantSP& target, const ConstantSP& resultSP) const {
  if(target->getRawType() != DT_DOUBLE) throw RuntimeException("Key data type incompatible. Expecting Double");
  if(target->isScalar()) {
    resultSP->setBool(static_cast<char>(dict_.find(target->getDouble()) != dict_.end()));
    return;
  }
...
bool LongAnyDictionary::set(const ConstantSP& key, const ConstantSP& value) {
  if(key->getRawType() != DT_LONG) throw RuntimeException("Key data type incompatible. Expecting LONG or the like");
  if(key->isScalar()) {
...
void LongAnyDictionary::contain(const ConstantSP& target, const ConstantSP& resultSP) const {
  if(target->getRawType() != DT_LONG) throw RuntimeException("Key data type incompatible. Expecting LONG or the like");
  if(target->isScalar()) {
    resultSP->setBool(static_cast<char>(dict_.find(target->getLong()) != dict_.end()));
    return;
  }
...
bool Int128AnyDictionary::remove(const ConstantSP& key) {
  if(key->getRawType() != DT_INT128) throw RuntimeException("Key data type incompatible. Expecting INT128 or the like.");
  if(key->isScalar())
    dict_.erase(key->getInt128());
  else {
    int len = key->size();
...
void Int128AnyDictionary::contain(const ConstantSP& target, const ConstantSP& resultSP) const {
  if(target->getRawType() != DT_INT128) throw RuntimeException("Key data type incompatible. Expecting INT128 or the like.");
  if(target->isScalar()) {
    resultSP->setBool(static_cast<char>(dict_.find(target->getInt128()) != dict_.end()));
  } else {
    int len = target->size();
