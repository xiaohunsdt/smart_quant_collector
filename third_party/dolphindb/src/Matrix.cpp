...
bool Matrix::reshape(INDEX cols, INDEX rows) {
  if(cols_ == cols && rows_ == rows) return true;
  if(cols_ * rows_ != cols * rows && rows != rows_) return false;
  cols_ = cols;
  rows_ = rows;
  if(!colLabel_->isNothing() && colLabel_->size() != cols_) colLabel_ = Constant::void_;
  if(!rowLabel_->isNothing() && rowLabel_->size() != rows_) rowLabel_ = Constant::void_;
  return true;
}

void Matrix::calculateInvalidLength(INDEX colStart, int colLength, INDEX rowStart, int rowLength, int& invalidLenBeginning,
                                    int& invalidLenEnding) const {
  std::ignore = colStart;
...
std::string Matrix::getString() const {
  ...
  while(length < static_cast<std::size_t>(Util::DISPLAY_WIDTH) && curCol < cols_) {
    ...
    if(maxColWidth > static_cast<std::size_t>(limitColMaxWidth)) maxColWidth = limitColMaxWidth;
    if(listTmp[0].size() > maxColWidth) maxColWidth = (std::min)(limitColMaxWidth, (int)listTmp[0].size());
    separator.append(maxColWidth, '-');
    if(curCol < cols_ - 1) {
      maxColWidth++;
      separator.append(1, ' ');
    ...
    if(length + maxColWidth > static_cast<std::size_t>(Util::DISPLAY_WIDTH) && curCol + 1 < cols_) break;
  ...
  if(curCol < cols_) {
    for(i = 0; i <= rows; i++) list[i].append("...");
    separator.append(3, '-');
  }
  ...
  if(rows < rows_) resultStr.append("...\n");
  return resultStr;
}
...
std::string Matrix::getString(INDEX index) const {
  ...
  if(len > 0) str.append(getString(index, 0));
  for(int i = 1; i < len; ++i) {
    str.append(",");
    str.append(getString(index, i));
  }
  if(rows_ > len) str.append("...");
  str.append("}");
  return str;
}
...
ConstantSP Matrix::get(const ConstantSP& index) const {
  if(index->isScalar()) {
    ...
    if(col < 0 || col >= cols_) throw OperatorRuntimeException("matrix", "The column index " + Util::convert(col) + " is out of range.");
  ...
  if(index->isPair()) {
    ...
    if(colStart <= colEnd) {
      for(int i = 0; i < length; ++i) data[i] = colStart + i;
    } else {
      --colStart;
      for(int i = 0; i < length; ++i) data[i] = colStart - i;
    }
  ...
  if(!rowLabel_.isNull()) result->setRowLabel(rowLabel_->getValue());
  if(!colLabel_.isNull()) {
    result->setColumnLabel(colLabel_->get(indexCols));
  }
  for(int i = 0; i < cols; ++i) {
    int cur = indexCols->getInt(i);
    if(cur < 0 || cur >= cols_) throw OperatorRuntimeException("matrix", "The column index " + Util::convert(cur) + " is out of range.");
...
bool Matrix::set(const ConstantSP& index, const ConstantSP& value) {
  ...
  if(value->size() != rows_ * cols && !scalar) throw OperatorRuntimeException("matrix", "matrix and assigned value are not compatible");
  if(cols == 1) {
    int cur = index->getInt(0);
    if(cur >= cols_ || cur < 0) throw OperatorRuntimeException("matrix", "The column index " + Util::convert(cur) + " is out of range.");
    setColumn(cur, value);
    return true;
  }
