...
void CharSet::contain(const ConstantSP& target, const ConstantSP& resultSP) const {
  if(target->isScalar()) {
    ...
    while(start < len) {
      ...
      for(int i = 0; i < count; ++i) pret[i] = static_cast<char>(data_.find(pbuf[i]) != end);
...
bool CharSet::manipulate(const ConstantSP& value, bool deletion) {
  ...
  if(form == DF_SCALAR) {
    ...
    while(start < len) {
      ...
      pbuf = source->getCharConst(start, count, buf);
      if(deletion) {
        for(int i = 0; i < count; ++i) data_.erase(pbuf[i]);
      } else
...
bool CharSet::inverse(const ConstantSP& value) {
  if(!value->isSet() || value->getCategory() != getCategory()) return false;
...
bool CharSet::isSuperset(const ConstantSP& target) const {
  ...
  while(start < len) {
    ...
    pbuf = source->getCharConst(start, count, buf);
    for(int i = 0; i < count; ++i) {
      if(data_.find(pbuf[i]) == end) return false;
    }
    start += count;
  }
  return true;
...
ConstantSP CharSet::interaction(const ConstantSP& value) const {
  ...
  while(start < len) {
    ...
    pbuf = source->getCharConst(start, count, buf);
    for(int i = 0; i < count; ++i) {
      if(data_.find(pbuf[i]) != end) result->data_.insert(pbuf[i]);
    }
    start += count;
  }
  return resultSP;
...
ConstantSP CharSet::getSubVector(INDEX start, INDEX length) const {
  ...
  while(start < length) {
    ...
    for(int i = 0; i < count; ++i) pbuf[i] = *it++;
...
ConstantSP StringSet::getSubVector(INDEX start, INDEX length) const {
  ...
  while(start < length) {
    count = (std::min)(bufSize, length - start);
    for(int i = 0; i < count; ++i) buf[i] = (char*)(*it++).c_str();
