...
bool TaskStatusMgmt::isFinished(int identity) {
  ...
  if(results.count(identity) == 0) throw RuntimeException("Task [" + std::to_string(identity) + "] does not exist.");
...
ConstantSP TaskStatusMgmt::getData(int identity) {
  std::unique_lock<std::mutex> guard(mutex_);
  if(results.count(identity) == 0) throw RuntimeException("Task [" + std::to_string(identity) + "] does not exist, the result may be fetched yet.");
