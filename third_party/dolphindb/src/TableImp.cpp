...
ConstantSP AbstractTable::getColumn(const std::string& qualifier, const std::string& name) const {
  auto it = colMap_->find(Util::lower(name));
  if(it != colMap_->end()) {
    if(Util::equalIgnoreCase(qualifier, name_)) return getColumn(it->second);
  }
...
ConstantSP AbstractTable::getColumn(const std::string& name, const ConstantSP& rowFilter) const {
  if(rowFilter.isNull()) return getColumn(name);
...
ConstantSP AbstractTable::getColumn(const std::string& qualifier, const std::string& name, const ConstantSP& rowFilter) const {
  if(rowFilter.isNull()) return getColumn(qualifier, name);
...
ConstantSP AbstractTable::getColumn(INDEX index, const ConstantSP& rowFilter) const {
  if(!rowFilter.isNull()) return getColumn(index)->get(rowFilter);
  return getColumn(index);
}

ConstantSP AbstractTable::get(INDEX col, INDEX row) const { return getColumn(col)->get(row); }
...
int AbstractTable::getColumnIndex(const std::string& name) const {
  auto it = colMap_->find(Util::lower(name));
  if(it == colMap_->end()) return -1;
  return it->second;
}

bool AbstractTable::contain(const std::string& name) const { return colMap_->find(Util::lower(name)) != colMap_->end(); }
...
ConstantSP AbstractTable::getColumnLabel() const {
  ...
  for(unsigned int i = 0; i < colNames_.size(); ++i) colNames->setString(i, colNames_.at(i));
...
ConstantSP AbstractTable::values() const {
  ...
  for(int i = 0; i < sz; ++i) result->set(i, getColumn(i));
  return result;
}
...
std::string AbstractTable::getString() const {
  ...
  while(length < Util::DISPLAY_WIDTH && curCol < columns()) {
    ...
    for(i = 0; i < rowNum; i++) {
      ...
      if((int)listTmp[i + 1].size() > maxColWidth) maxColWidth = static_cast<int>(listTmp[i + 1].size());
    }
    maxColWidth = std::min(maxColWidth, strColMaxWidth);
    if((int)listTmp[0].size() > maxColWidth) maxColWidth = (std::min)(strColMaxWidth, (int)listTmp[0].size());

    if(length + maxColWidth > Util::DISPLAY_WIDTH && curCol + 1 < columns()) break;
    ...
    for(i = 0; i <= rowNum; i++) {
      ...
      if(curSize <= maxColWidth) {
        ...
        if(curSize < maxColWidth) list[i].append(maxColWidth - curSize, ' ');
      } else {
        if(maxColWidth > 3) list[i].append(listTmp[i].substr(0, maxColWidth - 3));
        list[i].append("...");
      }
    }
    length += maxColWidth;
  ...
  if(curCol < columns()) {
    for(i = 0; i <= rowNum; i++) list[i].append("...");
    separator.append("---");
  }
  ...
  if(rowNum < size()) resultStr.append("...\n");
  return resultStr;
}

COMPRESS_METHOD AbstractTable::getColumnCompressMethod(INDEX index) {
  if(index < (INDEX)colCompresses_.size()) return colCompresses_[index];
    ...
    if(colCompresses[i] == COMPRESS_DELTA) {
      ...
      if(dataType != DT_SHORT && dataType != DT_INT && dataType != DT_LONG && dataType != DT_DECIMAL32 && dataType != DT_DECIMAL64) {
        throw RuntimeException("Cannot apply compression method DELTA to column " + colNames_.at(i) +
                               ", Only integral and temporal and Decimal32/Decimal64 data supports DELTA compression");
      }
...
ConstantSP AbstractTable::getInternal(INDEX index) const {
  ...
  int numCol = columns();
  for(int i = 0; i < numCol; ++i) dict->set(colNames_.at(i), getColumn(i)->get(index));
  return resultSP;
}
...
ConstantSP AbstractTable::getInternal(const ConstantSP& index) const {
  if(index->getCategory() == LITERAL) return getMember(index);
  if(index->isScalar()) {
    Dictionary* dict = Util::createDictionary(DT_STRING, DT_ANY);
    ConstantSP resultSP(dict);
...
ConstantSP AbstractTable::getWindowInternal(int columnStart, int columnLength, int rowStart, int rowLength) const {
  ...
  if(rowStart == 0 && rowLength == size()) {
    for(int i = 0; i < columnLength; i++) newCols.push_back(getColumn(columnStart + (sign * i))->getValue());
  } else {
    for(int i = 0; i < columnLength; i++) newCols.push_back(((Vector*)getColumn(columnStart + (sign * i)).get())->getSubVector(rowStart, rowLength));
  }

  if(columnStart == 0 && columnLength == columns()) return new BasicTable(newCols, colNames_);
  vector<string> names;
  names.reserve(columnLength);
  for(int i = 0; i < columnLength; i++) names.push_back(colNames_.at(columnStart + (sign * i)));
  return new BasicTable(newCols, names);
}

ConstantSP AbstractTable::getMemberInternal(const ConstantSP& key) const {
  if(key->isScalar()) return getColumn(key->getString(0));
  int sz = key->size();
  ConstantSP result = Util::createVector(DT_ANY, sz);
  for(int i = 0; i < sz; ++i) result->set(i, getColumn(key->getString(i)));
  return result;
}
...
ConstantSP AbstractTable::getValue() const { throw TableRuntimeException(getTableTypeName() + " can't be copied."); }
...
BasicTable::BasicTable(const vector<ConstantSP>& cols, const vector<string>& colNames, const vector<int>& key)
    : AbstractTable(vector<string>(colNames)) {
  std::ignore = key;
  initData(cols, colNames);
}
...
void BasicTable::initData(const vector<ConstantSP>& cols, const vector<string>& colNames) {
  ...
  if(len == 0) throw TableRuntimeException("A table has at least one column.");
  if(len != colNames.size()) throw TableRuntimeException("Number of column names must be the same as number of column vectors.");

  int rowNum = -1;
  for(std::size_t i = 0; i < len; i++) {
    if(cols[i].isNull()) throw TableRuntimeException("Column vector cannot be null.");
    if(cols[i]->isScalar()) continue;
    if(rowNum < 0)
      rowNum = cols[i]->size();
...
ConstantSP BasicTable::getColumn(INDEX index) const { return cols_[index]; }

bool BasicTable::set(INDEX index, const ConstantSP& value) {
  if(!value->isDictionary() || ((Dictionary*)value.get())->getKeyCategory() != LITERAL) return false;

  int numCol = columns();
  for(int i = 0; i < numCol; ++i) cols_[i]->set(index, ((Dictionary*)value.get())->getMember(colNames_.at(i)));
  return true;
}

ConstantSP BasicTable::get(INDEX index) const { return getInternal(index); }

ConstantSP BasicTable::get(const ConstantSP& index) const { return getInternal(index); }
...
ConstantSP BasicTable::getMember(const ConstantSP& key) const { return getMemberInternal(key); }
...
ConstantSP BasicTable::getValue(INDEX capacity) const {
  ...
  for(unsigned int i = 0; i < cols_.size(); i++) newCols.push_back(((Vector*)cols_[i].get())->getValue(capacity));
...
ConstantSP BasicTable::getInstance(int sz) const {
  vector<ConstantSP> newCols;
  newCols.reserve(cols_.size());
  for(unsigned int i = 0; i < cols_.size(); i++) newCols.push_back(((Vector*)cols_[i].get())->getInstance(sz));
...
bool BasicTable::append(vector<ConstantSP>& valueVec, INDEX& insertedRows, string& errMsg) {
  ...
  if(num == 1 && valueVec[0]->isTable()) {
    ...
    rowNum = tbl->rows();
    if(size_ + rowNum > capacity_ && !increaseCapacity(size_ + rowNum, errMsg)) return false;
    int i = 0;
    try {
      for(; i < num; ++i) {
        if(!((Vector*)cols_[i].get())->append(tbl->getColumn(i))) {
          errMsg = "data type " + Util::getDataTypeString(tbl->getColumn(i)->getType()) + ", expect " + Util::getDataTypeString(cols_[i]->getType());
          break;
        }
      }
      if(i >= num) {
      ...
      for(int k = 0; k < i; ++k) ((Vector*)cols_[k].get())->remove(rowNum);
    ...
    try {
      for(; i < num; ++i) {
        ...
        if(col->size() != rowNum || !((Vector*)cols_[i].get())->append(col)) {
          errMsg = "data type " + Util::getDataTypeString(col->getType()) + ", expect " + Util::getDataTypeString(cols_[i]->getType());
          break;
        }
      }
      if(i >= num) {
    ...
    try {
      for(; i < num; i++) {
        if(!((Vector*)cols_[i].get())->append(valueVec[i])) {
          errMsg = "data type " + Util::getDataTypeString(valueVec[i]->getType()) + ", expect " + Util::getDataTypeString(cols_[i]->getType());
          break;
        }
      }
      if(i >= num) {
    ...
    if(it == colMap_->end()) {
      ...
      if(!allowNewCol || (!valueVec[i]->isScalar() && valueVec[i]->size() != rowNum)) {
        errMsg.append("The column " + colNames[i] +
                      " does not exist. To add a new column, the table shouldn't be shared and the value size must "
                      "match the table.");
        return false;
      }
      ...
      if(cols_[colIndex[i]]->getCategory() != valueVec[i]->getCategory() && (!valueVec[i]->isNumber() || !cols_[colIndex[i]]->isNumber()) &&
         valueVec[i]->getCategory() != NOTHING) {
...
void BasicTable::drop(vector<int>& columnNum) {
  if(isReadOnly()) throw RuntimeException("Can't drop columns of a read only in-memory table.");
...
void BasicTable::internalDrop(vector<int>& columnNum) {
  ...
  for(int i = 0; i < numCol; ++i) {
    if(dropColumns.find(i) != dropColumns.end()) continue;
    newCols.push_back(cols_[i]);
    newColNames.push_back(colNames_.at(i));
    if(!colCompresses_.empty()) newColCompresses.push_back(colCompresses_.at(i));
...
bool BasicTable::join(vector<ConstantSP>& columnNum) {
  ...
  for(std::size_t i = 0; i < num; ++i) {
    ...
    if(!col->isArray() || col->size() != size_ || name.empty() || colMap_->find(Util::lower(name)) != colMap_->end()) return false;
  }
  for(std::size_t i = 0; i < num; ++i) {
    ConstantSP& col = columnNum[i];
...
bool BasicTable::clear() {
  if(isReadOnly()) return false;
  int num = columns();
  for(int i = 0; i < num; ++i) VectorSP(cols_[i])->clear();
  size_ = 0;
  return true;
}
...
long long BasicTable::getAllocatedMemory() const {
  ...
  for(unsigned int i = 0; i < cols_.size(); ++i) sz += cols_[i]->getAllocatedMemory();
  return sz;
}
...
void BasicTable::updateSize() {
  ...
  for(unsigned int i = 1; i < cols_.size(); ++i) {
    if(cols_[i]->size() != newSize) throw TableRuntimeException("The length of all columns are inconsistent in table " + name_);
  }
  size_ = newSize;
}
