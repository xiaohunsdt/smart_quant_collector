...
bool EventHandler::checkSchema(const std::vector<EventSchema>& eventSchemas, const std::vector<std::string>& expandTimeKeys,
                               const std::vector<std::string>& commonFields, std::string& errMsg) {
  int index = 0;
...
EventHandler::EventHandler(const std::vector<EventSchema>& eventSchemas, const std::vector<std::string>& eventTimeFields,
...
bool EventHandler::deserializeEvent(const ConstantSP& obj, std::vector<std::string>& eventTypes, std::vector<std::vector<ConstantSP>>& attributes,
...
bool EventHandler::serializeEvent(const std::string& eventType, const std::vector<ConstantSP>& attributes, std::vector<ConstantSP>& serializedEvent,
                                  std::string& errMsg) {
  auto iter = eventInfos_.find(eventType);
  if(iter == eventInfos_.end()) {
  ...
  for(unsigned i = 0; i < attributes.size(); ++i) {
    if(info.eventSchema_->schema_.fieldTypes_[i] != attributes[i]->getType()) {
      ...
      errMsg = "Expected type for the field " + info.eventSchema_->schema_.fieldNames_[i] + " of " + eventType + " : " +
               Util::getDataTypeString(info.eventSchema_->schema_.fieldTypes_[i]) + ", but now it is " +
               Util::getDataTypeString(attributes[i]->getType());
      return false;
    }
    if(info.eventSchema_->schema_.fieldForms_[i] != attributes[i]->getForm()) {
      errMsg = "Expected form for the field " + info.eventSchema_->schema_.fieldNames_[i] + " of " + eventType + " : " +
               Util::getDataFormString(info.eventSchema_->schema_.fieldForms_[i]) + ", but now it is " +
               Util::getDataFormString(attributes[i]->getForm());
      return false;
    }
    if(Util::getCategory(attributes[i]->getRawType()) == DENARY) {
      if(info.eventSchema_->schema_.fieldExtraParams_[i] != attributes[i]->getExtraParamForType()) {
        errMsg = "Expected extraParams for the field " + info.eventSchema_->schema_.fieldNames_[i] + " of " + eventType + " : " +
                 std::to_string(info.eventSchema_->schema_.fieldExtraParams_[i]) + ", but now it is " +
                 std::to_string(attributes[i]->getExtraParamForType());
        return false;
      }
    }
  }
...
EventSender::EventSender(const DBConnectionSP& conn, const std::string& tableName, const std::vector<EventSchema>& eventSchema,
