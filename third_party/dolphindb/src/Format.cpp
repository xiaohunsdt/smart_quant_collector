...
int NumberFormat::printFraction(char* buf, int digitCount, bool optional, double& fraction) {
  ...
  while(digitCount != 0) {
    ...
    if(cursor - start < count && (!optional || (digitCount != 0) || notZero)) {
      for(int i = count - cursor + start; i > 0; --i) buf[cursor++] = '0';
    }
...
void NumberFormat::initialize(const std::string& form) {
  if(form.empty()) throw RuntimeException("The format string can't be empty.");
  int firstSymPos = -1;
  int lastSymPos = 0;
  ...
  if(digitCount == 0) throw RuntimeException("The number format doesn't contain '0' or '#'.");
  if(percentCount > 0) {
    percent_ = true;
    if(form[lastSymPos] != '%') throw RuntimeException("The percent sign(%) must be the last symbol of a number format.");
  }
  if(sciencePos >= 0) {
    ...
    for(int i = sciencePos + 1; i <= lastSymPos; ++i) {
      if(form[i] != '0') throw RuntimeException("The format symbol after scientific notation (E) must be 0");
    }
    science_ = lastSymPos - sciencePos;
  }

  int lastIntegerPos = lastSymPos;
  if(percent_) --lastIntegerPos;
  if(commaPos >= 0) {
    ...
    if(pointPos >= 0) {
      if(pointPos < commaPos) throw RuntimeException("The decimal separator(,) must appear before decimal point(.) in a number format.");
      lastIntegerPos = pointPos - 1;
    }
    segmentLength_ = lastIntegerPos - commaPos;
  }

  if(pointPos >= 0 && pointPos <= lastIntegerPos) lastIntegerPos = pointPos - 1;
  integerMinDigits_ = 0;
  ...
  if(science_ > 0 && integerMinDigits_ == 0) integerMinDigits_ = 1;

  fractionMinDigits_ = 0;
  fractionOptionalDigits_ = 0;
  if(pointPos >= 0) {
    int lastFractionPos = lastSymPos;
    if(percent_) --lastFractionPos;
    if(sciencePos >= 0) lastFractionPos = sciencePos - 1;
    int i = pointPos + 1;
  ...
  if(epsilon_ < rounding_ / 10) rounding_ += epsilon_;
...
std::string NumberFormat::format(double x) const {
  ...
  for(int i = 0; i < headSize_; ++i) buf[cursor++] = head_[i];
  ...
  int digits = 0;
  while(integral != 0) {
    if((digits != 0) && digits % segmentLength_ == 0) buf[cursor++] = ',';
    buf[cursor++] = integral % 10 + '0';
    ++digits;
    integral /= 10;
  }
  for(; digits < integerMinDigits_; ++digits) buf[cursor++] = '0';
  ...
  if(enableSciNotation) {
    ...
    for(; digits < science_; ++digits) buf[cursor++] = '0';
    // reverse the digits of integer part
  ...
  if(percent_) buf[cursor++] = '%';
  for(int i = 0; i < tailSize_; ++i) buf[cursor++] = tail_[i];
  buf[cursor] = 0;
  return buf;
}
...
DecimalFormat::~DecimalFormat() {
  ...
  if(negFormat_ != nullptr) delete negFormat_;
}

std::string DecimalFormat::format(double x) const {
  if(negFormat_ != nullptr && x < 0) return negFormat_->format(-x);
