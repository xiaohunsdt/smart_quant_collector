...
DFSChunkMeta::DFSChunkMeta(std::string path, const Guid& id, int version, int sz, CHUNK_TYPE chunkType, const std::vector<std::string>& sites,
                           long long cid)
    : Constant(2051),
      type_(static_cast<char>(chunkType)),
      replicaCount_(static_cast<char>(sites.size())),
      version_(version),
      size_(sz),
      sites_(nullptr),
      path_(std::move(path)),
      cid_(cid),
      id_(id) {
  if(replicaCount_ == 0) return;
  sites_ = new std::string[replicaCount_];
  for(int i = 0; i < replicaCount_; ++i) sites_[i] = sites[i];
}

DFSChunkMeta::DFSChunkMeta(std::string path, const Guid& id, int version, int sz, CHUNK_TYPE chunkType, const std::string* sites, int siteCount,
                           long long cid)
    : Constant(2051),
      type_(static_cast<char>(chunkType)),
      replicaCount_(static_cast<char>(siteCount)),
      version_(version),
      size_(sz),
      sites_(nullptr),
      path_(std::move(path)),
...
DFSChunkMeta::DFSChunkMeta(const DataInputStreamSP& in) : Constant(2051), sites_(nullptr), id_(false) {
  ...
  if(ret != OK) throw RuntimeException("Failed to deserialize DFSChunkMeta object.");
  ...
  if(ret != OK) throw RuntimeException("Failed to deserialize DFSChunkMeta object.");
  if(replicaCount_ > 0) sites_ = new std::string[replicaCount_];
  for(int i = 0; i < replicaCount_; ++i) {
    std::string site;
    if(in->readString(site) != OK) throw RuntimeException("Failed to deserialize DFSChunkMeta object.");
    sites_[i] = site;
  }
  id_ = Guid((unsigned char*)guid);
  if(in->read(cid_) != OK) throw RuntimeException("Failed to deserialize DFSChunkMeta object.");
}

DFSChunkMeta::~DFSChunkMeta() {
  if(sites_ != nullptr) delete[] sites_;
}
...
long long DFSChunkMeta::getAllocatedMemory() const {
  ...
  for(int i = 0; i < replicaCount_; ++i) length += sites_[i].size();
  return length;
}
...
ConstantSP DFSChunkMeta::getMember(const ConstantSP& key) const {
  ...
  if(key->isScalar()) return getAttribute(key->getString());
...
ConstantSP DFSChunkMeta::getSiteVector() const {
  ...
  for(int i = 0; i < replicaCount_; ++i) vec->setString(i, sites_[i]);
  return vec;
}

ConstantSP DFSChunkMeta::getAttribute(const std::string& attr) const {
  if(attr == "path") return new String(path_);
  if(attr == "id") return new String(id_.getString());
  if(attr == "cid") return new Long(cid_);
  if(attr == "version") return new Int(version_);
  if(attr == "sites") return getSiteVector();
  if(attr == "size") {
    ConstantSP obj = Util::createConstant(DT_INDEX);
    obj->setIndex(size_);
    return obj;
  }
  if(attr == "isTablet") return new Bool(isTablet());
  if(attr == "splittable") return new Bool(isSplittable());
