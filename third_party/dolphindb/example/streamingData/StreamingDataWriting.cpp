...
void showUsage() {
  ...
  cout << " --t=5 Optional,Threads number,default is 1,max is " << MAX_THREAD_NUM << endl;
  cout << " --help Print this help." << endl;
  return;
}

TableSP createDemoTable(long rows) {
  vector<string> colNames = {"id",      "cbool",   "cchar",   "cshort",    "cint",       "clong",     "cdate",          "cmonth",
                             "ctime",   "cminute", "csecond", "cdatetime", "ctimestamp", "cnanotime", "cnanotimestamp", "cfloat",
                             "cdouble", "csymbol", "cstring", "cuuid",     "cip",        "cint128"};

  vector<DATA_TYPE> colTypes = {DT_LONG,   DT_BOOL,   DT_CHAR,   DT_SHORT,    DT_INT,       DT_LONG,     DT_DATE,          DT_MONTH,
                                DT_TIME,   DT_MINUTE, DT_SECOND, DT_DATETIME, DT_TIMESTAMP, DT_NANOTIME, DT_NANOTIMESTAMP, DT_FLOAT,
                                DT_DOUBLE, DT_SYMBOL, DT_STRING, DT_UUID,     DT_IP,        DT_INT128};
  ...
  for(int i = 0; i < colNum; i++) columnVecs.push_back(table->getColumn(i));
