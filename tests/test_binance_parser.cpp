...
void ParseDoc(simdjson::ondemand::parser& parser, std::string& json, simdjson::ondemand::document& doc) {
  json.resize(json.size() + simdjson::SIMDJSON_PADDING, '\0');
  auto error = parser.iterate(json.data(), json.size() - simdjson::SIMDJSON_PADDING, json.size()).get(doc);
  if(error) {
...
TEST(BinanceParserTest, ParseTradeEvent) {
  std::string json =
      R"({"e":"aggTrade","E":1716844800000000,"s":"BTCUSDT","a":123456789,"p":"50000.00","q":"1.50000000","f":123,"l":456,"T":1716844800001000,"m":true})";
...
TEST(BinanceParserTest, ParseHandlesBookTicker) {
  std::string json =
      R"({"e":"bookTicker","u":400900217,"s":"BNBUSDT","b":"25.19000000","B":"31.21000000","a":"25.20000000","A":"40.66000000","E":1716844800000000})";
...
TEST(BinanceParserTest, ParseSpotPartialDepth) {
  ...
  std::string json =
      R"({"lastUpdateId":160,"bids":[["50000.00","1.50000000",[]],["49990.00","0.50000000",[]]],"asks":[["50010.00","2.00000000",[]],["50020.00","1.00000000",[]]]})";
...
TEST(BinanceParserTest, ParseFuturesDepthEvent) {
  ...
  std::string json =
      R"({"e":"depthUpdate","E":1716844800000,"T":1716844800001,"s":"BTCUSDT","U":1001,"u":1005,"pu":999,"b":[["50000.00","1.50000000"]],"a":[["50010.00","2.00000000"]]})";
