...
TEST(LocalLOBTest, UpdateDepthDelete) {
  ...
  // Depth guard: count=2 <= depth_level_=10, so the deleted level is
  // preserved with qty=0 instead of being physically removed.
  PriceLevel top_bids[10];
  EXPECT_EQ(lob.TopBids(top_bids, 10), 2u);
  EXPECT_DOUBLE_EQ(top_bids[0].price, 50000.0);
  EXPECT_DOUBLE_EQ(top_bids[0].quantity, 0.0);   // zeroed, not removed
  EXPECT_DOUBLE_EQ(top_bids[1].price, 49900.0);
  EXPECT_DOUBLE_EQ(top_bids[1].quantity, 2.0);
}
