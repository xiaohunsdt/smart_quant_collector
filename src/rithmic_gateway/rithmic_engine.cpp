...
RithmicEngine::RithmicEngine(const Config& config, shm_layout::TickQueue& tick_queue, shm_layout::DepthQueue& depth_queue,
...
bool RithmicEngine::CreateEngineAndCallbacks() {
  ...
  try {
    pCallbacks_ = std::make_unique<MyCallbacks>(tick_queue_, depth_queue_, book_ticker_queue_, channel_map_, converter_, sqc::kMaxOrderbookLevels);
  } catch(OmneException& oEx) {
...
bool RithmicEngine::LoginRepository() {
  ...
  if(g_iUnacceptedMandatoryAgreements > 0) {
    LOG_ERROR(quill::Frontend::get_logger("stdout"), "Unaccepted mandatory agreements: {}. Please log in via R|Trader and accept them.",
...
bool RithmicEngine::LoginMarketData() {
  ...
  const std::string md_cnnct_pt =
      (config_.use_aggregated_md && !config_.md_connect_pt_agg.empty()) ? config_.md_connect_pt_agg : config_.md_connect_pt;
