...
bool ParseBookTickerEvent(simdjson::ondemand::document& doc, BookTickerEvent& out,
  ...
  try {
    ...
    if (bid_sz.error() == simdjson::SUCCESS) SvToDouble(bid_sz.value_unsafe(), out.best_bid_qty);
    else out.best_bid_qty = static_cast<double>(result["B"].get_int64());
    if (!SvToDouble(result["a"].get_string(), out.best_ask_price)) return false;
    auto ask_sz = result["A"].get_string();
    if (ask_sz.error() == simdjson::SUCCESS) SvToDouble(ask_sz.value_unsafe(), out.best_ask_qty);
    else out.best_ask_qty = static_cast<double>(result["A"].get_int64());
