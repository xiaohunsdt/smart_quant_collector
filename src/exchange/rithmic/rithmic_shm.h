...
struct ShmHeader {
  ...
  ShmHeader() : magic(kMagic), version(1), heartbeat_ns(0), child_ready(0), parent_ready(0) {}
};

static_assert(std::is_standard_layout_v<ShmHeader>, "ShmHeader must be standard layout for shared memory");
...
inline ShmSetup::ShmSetup(const char* name, size_t total_size, bool create) : name_(name), size_(total_size), created_(create) {
  if(create) {
...
inline ShmSetup::ShmSetup(const char* name) : ShmSetup(name, 0, false) {
...
inline shm_layout::TickQueue* ShmSetup::tick_queue() noexcept {
  return reinterpret_cast<shm_layout::TickQueue*>(static_cast<char*>(addr_) + shm_layout::kTickQueueOffset);
}

inline shm_layout::DepthQueue* ShmSetup::depth_queue() noexcept {
  return reinterpret_cast<shm_layout::DepthQueue*>(static_cast<char*>(addr_) + shm_layout::kDepthQueueOffset);
}

inline shm_layout::BookTickerQueue* ShmSetup::book_ticker_queue() noexcept {
  return reinterpret_cast<shm_layout::BookTickerQueue*>(static_cast<char*>(addr_) + shm_layout::kBookTickerQueueOffset);
}

inline const shm_layout::TickQueue* ShmSetup::tick_queue() const noexcept { return const_cast<ShmSetup*>(this)->tick_queue(); }

inline const shm_layout::DepthQueue* ShmSetup::depth_queue() const noexcept { return const_cast<ShmSetup*>(this)->depth_queue(); }

inline const shm_layout::BookTickerQueue* ShmSetup::book_ticker_queue() const noexcept { return const_cast<ShmSetup*>(this)->book_ticker_queue(); }
