  ...
  try {
    switch(event_type) {
      case EventType::TICK:
        ...
        if(!binance::ParseTradeEvent(doc, result.tick, channel_id, symbol)) result.type = ParsedType::NONE;
        break;
      case EventType::DEPTH:
        // Spot uses partial depth format (no "e", "bids"/"asks", "lastUpdateId")
        result.type = ParsedType::DEPTH;
        if(!ParsePartialDepth(doc, result.depth, channel_id, symbol)) result.type = ParsedType::NONE;
        break;
      case EventType::BOOK_TICKER:
        // Spot @bookTicker omits "e" and "E" fields
        doc.rewind();
        result.type = ParsedType::BOOK_TICKER;
        if(!binance::ParseSpotBookTickerEvent(doc, result.book_ticker, channel_id, symbol)) result.type = ParsedType::NONE;
        break;
      default:
