...
void LocalLOB::ApplySnapshot(const OrderbookSnapshot& snapshot) {
  ...
  // Compact adjacent same-price levels (defense: REST snapshot may return
  // individual orders at identical prices for high-liquidity pairs).
  if (bid_count_ > 1) {
    uint32_t w = 0;
    for (uint32_t r = 1; r < bid_count_; ++r) {
      if (bids_[w].price == bids_[r].price)
        bids_[w].quantity += bids_[r].quantity;
      else
        bids_[++w] = bids_[r];
    }
    bid_count_ = w + 1;
  ...
  if (ask_count_ > 1) {
    uint32_t w = 0;
    for (uint32_t r = 1; r < ask_count_; ++r) {
      if (asks_[w].price == asks_[r].price)
        asks_[w].quantity += asks_[r].quantity;
      else
        asks_[++w] = asks_[r];
    }
    ask_count_ = w + 1;
...
void LocalLOB::UpdateSide(PriceLevel* levels, uint32_t& count, uint32_t capacity,
  ...
  for (; idx < count; ++idx) {
    if (levels[idx].price == price) {
      ...
      if (qty <= 1e-12) {
        // If removal would drop below the configured output depth, preserve
        // the price level with zero quantity instead of physically removing.
        // This prevents CSV zero-padding while correctly showing no liquidity.
        if (count <= depth_level) {
          levels[idx].quantity = 0.0;
          return;
        }
        // Remove by shifting
...
void LocalLOB::UpdateDepth(const DepthUpdateEvent& event) {
  ...
  for (uint32_t i = 0; i < event.bid_count; ++i)
    UpdateSide(bids_, bid_count_, kMaxOrderbookLevels,event.bids[i].price, event.bids[i].quantity, true, depth_level_);
  for (uint32_t i = 0; i < event.ask_count; ++i)
    UpdateSide(asks_, ask_count_, kMaxOrderbookLevels,event.asks[i].price, event.asks[i].quantity, false, depth_level_);
}
...
void LocalLOB::ForceAlignWithEvent(const DepthUpdateEvent& event) {
  ...
  // Compact adjacent same-price levels (defense against unaggregated sources).
  if (bid_count_ > 1) {
    uint32_t w = 0;
    for (uint32_t r = 1; r < bid_count_; ++r) {
      if (bids_[w].price == bids_[r].price)
  ...
  // Warn if the forced alignment leaves the book shallower than the
  // configured output depth — CSV will zero-pad missing levels.
  if (bid_count_ < depth_level_ || ask_count_ < depth_level_) {
    LOG_WARNING(GetLogger(),
                "ForceAlign: sparse event (bids={}, asks={}) < depth_level={} — "
                "CSV output will contain zero-padded levels until depth recovers",
                bid_count_, ask_count_, depth_level_);
  }
}
