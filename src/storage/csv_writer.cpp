...
bool CsvWriter::RotateIfNeeded(uint64_t exchange_ts) {
  ...
  // seekp+0 → file is newly created. On stream error, tellp() returns
  // pos_type(-1) which != 0, so header is safely skipped — we'd rather
  // miss a header row than write data to a corrupt stream.
  trade_file_.seekp(0, std::ios::end);
  if(trade_file_.tellp() == 0) trade_file_ << "exchange_timestamp,local_diff,price,quantity,direction\n";
  ...
  if(!orderbook_file_.is_open()) {
    ...
    trade_file_.close();  // rollback — avoid partial state
    return false;
  }
  orderbook_file_.seekp(0, std::ios::end);
  if(orderbook_file_.tellp() == 0) header_written_ = false;
  ...
  if(!bookticker_file_.is_open()) {
    ...
    trade_file_.close();      // rollback — avoid partial state
    orderbook_file_.close();  // rollback
    return false;
  }
...
void CsvWriter::AppendTick(const TickData& tick) {
  ...
  fmt::format_to(std::back_inserter(buf), "{},{},{},{},{}\n", tick.exchange_timestamp, tick.local_diff, tick.price, tick.quantity,
                 tick.is_buyer_maker ? -1 : 1);
  trade_file_ << std::string_view(buf.data(), buf.size());
}

void CsvWriter::AppendOrderbook(const DepthUpdateEvent& event, uint64_t local_ts, uint32_t depth_level) {
  ...
  if(!header_written_) {
    ...
    for(uint32_t i = 0; i < depth_level_; ++i)
      orderbook_file_ << ",ask_price" << (i + 1) << ",ask_size" << (i + 1) << ",bid_price" << (i + 1) << ",bid_size" << (i + 1);
    orderbook_file_ << "\n";
  }

  fmt::memory_buffer row;
  fmt::format_to(std::back_inserter(row), "{},{}", event.exchange_timestamp, local_ts);
  for(uint32_t i = 0; i < depth_level_; ++i) {
    fmt::format_to(std::back_inserter(row), ",{},{},{},{}", i < event.ask_count ? event.asks[i].price : 0.0,
                   i < event.ask_count ? event.asks[i].quantity : 0.0, i < event.bid_count ? event.bids[i].price : 0.0,
...
void CsvWriter::AppendBookTicker(const BookTickerEvent& event) {
  ...
  fmt::memory_buffer buf;
  fmt::format_to(std::back_inserter(buf), "{},{},{},{},{},{},{}\n", event.exchange_timestamp, event.local_diff, event.symbol, event.best_bid_price,
                 event.best_bid_qty, event.best_ask_price, event.best_ask_qty);
