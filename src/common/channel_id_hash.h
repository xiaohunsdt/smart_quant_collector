...
[[nodiscard]] constexpr uint32_t ComputeChannelId(
  ...
  uint64_t h = kBasis;
  for (unsigned char c : exchange) { h ^= c; h *= kPrime; }
  h ^= static_cast<uint64_t>(':'); h *= kPrime;
  for (unsigned char c : type_name) { h ^= c; h *= kPrime; }
  h ^= static_cast<uint64_t>(':'); h *= kPrime;
  for (unsigned char c : symbol)    { h ^= c; h *= kPrime; }
